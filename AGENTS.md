...
6. 완료 후 임시 QA 엔트리만 제거한다. QA 캡처 이미지는 사용자가 결과를 직접 확인할 수 있도록 보존하고 최종 답변에 절대 경로 링크를 제공한다. 캡처 이미지는 최종 커밋 직전에만 정리하며, 커밋하지 않는 작업에서는 삭제하지 않는다. 도구가 만든 브라우저 프로필과 프로세스는 자동 정리된다.
