...
export function normalizedRegionToPixelRect(
  bbox: BBox,
  pageSize: PageSize,
  minSizePx = 2,
): PixelRect {
  ...
  const x = Math.max(
    0,
    Math.min(width - 1, Math.floor((safe.x / 1000) * width)),
  );
  const y = Math.max(
    0,
    Math.min(height - 1, Math.floor((safe.y / 1000) * height)),
  );
...
export function mapCropNormalizedBboxToPageBbox(
  cropRect: PixelRect,
  pageSize: PageSize,
  cropBbox: BBox,
): BBox {
