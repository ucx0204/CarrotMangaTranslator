...
export const McpChapterTextSearchSchema = z.object({
  chapterId,
  ...
  reviewStatus: z.enum(["all", "draft", "needs_review", "reviewed"]).default("all"),
...
export const McpTranslationBatchPreviewSchema = z.object({
  chapterId,
...
export type McpTranslationBatchPreview = z.infer<typeof McpTranslationBatchPreviewSchema>;
export const McpTranslationBatchGetSchema = z.object({
  batchId: z.uuid(),
  ...window,
}).strict();
export const McpTranslationBatchActionSchema = z.object({
  batchId: z.uuid(),
  requestId: z.uuid(),
}).strict();
export type McpTranslationBatchAction = z.infer<typeof McpTranslationBatchActionSchema>;
export type McpTranslationBatchDirection = "apply" | "undo" | "redo";

const snippet = z.object({
  text: z.string().max(800), start: count, end: count,
  totalLength: count, truncated: z.boolean(),
}).strict();
const neighbor = z.object({
  blockId, source: z.string().max(160), translation: z.string().max(160),
}).strict().nullable();
const hit = z.object({
  pageId, pageNumber: count, blockId, revision,
  source: snippet, translation: snippet,
  matches: z.array(z.object({
    field: z.enum(["source", "translation"]), start: count, end: count,
  }).strict()).max(40),
...
const searchResult = z.object({
  chapterId, snapshot: fingerprint, contextRevision: fingerprint,
  total: count, excludedGenerated: count,
  offset: count, limit: count, nextOffset: count.nullable(),
  matches: z.array(hit).max(25),
  note: z.string(),
}).strict();
export type McpChapterTextHit = z.infer<typeof hit>;

const status = z.enum(["proposed", "running", "completed", "partial", "failed", "cancelled"]);
const pageState = z.enum(["pending", "applied", "undone", "unchanged", "excluded"]);
const pageSummary = z.object({
  pageId, expectedRevision: revision,
  state: pageState, changedBlocks: count,
  result: z.enum(["not_started", "saved", "failed", "cancelled"]),
  errorCode: z.string().nullable(),
}).strict();
const summary = z.object({
  batchId: z.uuid(), chapterId, contextRevision: fingerprint,
  reason: z.string(), expiresAt: count,
  status, direction: z.enum(["apply", "undo", "redo"]).nullable(),
  activeRequestId: z.uuid().nullable(), cancellationRequested: z.boolean(),
  pages: z.array(pageSummary).max(50),
  totalChanges: count, excludedChanges: count,
  canApply: z.boolean(), canUndo: z.boolean(), canRedo: z.boolean(),
  warnings: z.array(z.string()),
}).strict();
const inspectedChange = z.object({
  pageId, blockId, sourceText: z.string(),
  previousText: z.string(), proposedText: z.string().max(8192),
  reason: z.string(), excludedReason: z.string().nullable(), changed: z.boolean(),
}).strict();
...
const receipt = z.object({
  batchId: z.uuid(), requestId: z.uuid(),
