...
const characterFields = z.object({
  ...
  aliases: aliases.optional(),
  speechStyle: z.enum(CharacterProfileSchema.shape.speechStyle.options).optional(),
...
const glossaryChange = z.object({
  ...common, entity: z.literal("glossary"), entryId: entryId.optional(), values: glossaryFields,
}).strict();
const characterChange = z.object({
  ...common, entity: z.literal("character"), entryId: entryId.optional(), values: characterFields,
}).strict();
export const McpContextChangeSchema = z.discriminatedUnion("entity", [
  glossaryChange,
  characterChange,
  z.object({ ...common, entity: z.literal("rules"), values: rulesFields }).strict(),
  z.object({
    ...common, entity: z.literal("memory"), pageId: id,
    pageRevision: z.string().regex(/^page-v1:[a-f0-9]{16}$/), values: memoryFields,
  }).strict(),
]);
...
export const McpContextPreviewSchema = z.object({
  ...target, changes: z.array(McpContextChangeSchema).min(1).max(100),
}).strict();
...
export const McpExternalResearchSchema = z.object({
  ...target,
  changes: z.array(z.object({
    change: z.discriminatedUnion("entity", [glossaryChange, characterChange]),
    reason: z.string().min(1).max(2000),
    sources: z.array(source).min(1).max(20),
  }).strict()).min(1).max(100),
}).strict();
export const McpContextApplySchema = z.object({
  proposalId: z.string().uuid(), requestId: z.string().uuid(),
  selectedChangeIds: z.array(id).min(1).max(100),
}).strict();
export const McpContextInspectSchema = z.object({
  proposalId: z.string().uuid(),
  offset: z.number().int().min(0).max(1_000_000).default(0),
  limit: z.number().int().min(1).max(25).default(10),
}).strict();
export const McpContextResearchTargetSchema = z.object({
  ...target, researchTitle: z.string().trim().min(1).max(200),
  engine: z.enum(["tavily", "codex-web"]),
}).strict();
export type McpContextResearchTarget = z.infer<typeof McpContextResearchTargetSchema>;

const proposalMetadata = z.object({
  proposalId: z.string().uuid(), chapterId: id, workId: id,
  revision, source: z.enum(["edit", "external-research", "app-research"]),
  changeIds: z.array(id).max(100), expiresAt: z.number().int().nonnegative(),
  warnings: z.array(z.string().max(2000)).max(100),
}).strict();
export const McpContextResearchResultSchema = proposalMetadata.extend({
  queryCount: z.number().int().nonnegative(),
  sourceCount: z.number().int().nonnegative(),
  tavilyCreditsUsed: z.number().nonnegative(),
}).strict();
const summary = z.object({
  changeId: id, entity: z.enum(["glossary", "character", "rules", "memory"]),
  targetId: entryId, changed: z.boolean(),
...
export const mcpContextOutputSchemas = {
  ...
  carrot_get_context_proposal: proposalMetadata.extend({
  ...
  carrot_apply_context_proposal: z.object({
    proposalId: z.string().uuid(), requestId: z.string().uuid(),
...
export function mcpContextRevision(context: {
  workId: string; workTitle: string; styleGuide: WorkStyleGuide; storyMemory: ChapterStoryMemory;
}): string {
  const { createdAt: _created, updatedAt: _updated, ...guide } = context.styleGuide;
  const { updatedAt: _memoryUpdated, ...memory } = context.storyMemory;
  return hashStableValue({ workId: context.workId, workTitle: context.workTitle, guide, memory });
}
