...
export const redactionPreferencesSchema = z
  .object({
...
const pageView = z
  ...
  .strict();
export const redactionViewSchema = z
  .object({
...
export const redactionPresetSchema = z
  .object({
    id: z.string().uuid(),
...
export const openRedactionWorkspaceSchema = z.discriminatedUnion("kind", [
  ...
  z
    .object({
      ...
      chapterId: id,
      pageIds: z.array(id).min(1).max(10000).optional(),
    })
    .strict(),
  z.object({ kind: z.literal("work"), workId: id }).strict(),
]);
export type OpenRedactionWorkspace = z.infer<
  typeof openRedactionWorkspaceSchema
>;
export const redactionWorkspaceSchema = z
  .object({
    sessionId: z.string().uuid(),
...
export const saveRedactionWorkspaceSchema = z
  .object({
    sessionId: z.string().uuid(),
  ...
  .strict();
export type SaveRedactionWorkspace = z.infer<
  typeof saveRedactionWorkspaceSchema
>;
export const redactionPreviewRequestSchema = z
  .object({
    sessionId: z.string().uuid(),
    pageId: id,
...
export type RedactionPreviewRequest = z.infer<
  typeof redactionPreviewRequestSchema
>;
