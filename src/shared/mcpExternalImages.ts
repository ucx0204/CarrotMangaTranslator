...
const rect = z
  .object({
    x: coordinate,
    y: coordinate,
    w: z.number().int().positive().max(16_000_000),
    h: z.number().int().positive().max(16_000_000),
  })
  .strict();
const images = {
  imageUploadId: z.uuid(),
  maskUploadId: z.uuid().optional(),
...
export const McpExternalImagePreviewSchema = z
  .object({
    chapterId,
    pageId,
    revision,
  ...
  .strict();
export type McpExternalImagePreview = z.infer<
  typeof McpExternalImagePreviewSchema
>;
export const McpExternalImageGetPreviewSchema = z
...
export const mcpExternalImageOutputs = {
  ...
  carrot_get_external_image_preview: z
    .object({
      batchId: z.uuid(),
      chapterId,
      pageId,
