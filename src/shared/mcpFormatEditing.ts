...
import {
  McpEditableFieldsSchema,
  McpBlockPatchSchema,
} from "./mcpBlockEditing";
...
const condition = z
  ...
  .superRefine((item, ctx) => {
    if (!McpFormatFieldsSchema.shape[item.field].safeParse(item.value).success)
      ctx.addIssue({
        code: "custom",
        message: "Value must match this format field.",
        path: ["value"],
      });
    if (
      !["equals", "notEquals"].includes(item.operator) &&
      typeof item.value !== "number"
    )
      ctx.addIssue({
        code: "custom",
        message: "Ordered comparisons require a numeric field.",
      });
  });
export const McpFormatFilterSchema = z
  .object({
...
const rect = z
  ...
  .strict();
export const McpFormatViewSchema = z
  .object({
...
export function projectMcpFormat(page: MangaPage, block: TranslationBlock) {
  ...
  const effective = Object.fromEntries(
    keys
      .map((key) => [
        key,
        readConditionalBatchField(block, key, {
          page,
          pageIndex: 0,
          blockIndex: 0,
        }),
...
export function matchesMcpFormat(
  page: MangaPage,
  block: TranslationBlock,
  filter?: McpFormatFilter,
) {
  if (!filter) return true;
  const manual = block.fontSizeIntent === "manual";
  if (
    filter.manualFontSize !== "all" &&
    manual !== (filter.manualFontSize === "only")
  )
    return false;
  return filter.conditions.every((item) => {
    const actual =
      filter.basis === "stored"
        ? block[item.field]
        : readConditionalBatchField(block, item.field, {
            page,
            pageIndex: 0,
            blockIndex: 0,
          });
...
function compareFormatValue(
  actual: unknown,
  operator: McpFormatFilter["conditions"][number]["operator"],
...
export const McpFormatEditSchema = z
  .object({
  ...
  .strict()
  .refine(
    (item) => !!item.renderRect || Object.keys(item.fields ?? {}).length > 0,
    "Specify at least one format field or a render rectangle.",
  );
