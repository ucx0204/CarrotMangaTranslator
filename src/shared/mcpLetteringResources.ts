...
export const McpLetteringResourceReferenceSchema = z
  .object({
    resourceKind: kind,
    id,
    snapshot,
  ...
  .strict();
export const McpLetteringResourceCommandSchema =
  McpLetteringResourceReferenceSchema.extend({
    kind: z.literal("resource"),
    groupIds: groups.optional(),
  }).strict();
...
export const McpLetteringResourceListSchema = z
  ...
  .strict();
export const McpLetteringResourceGetSchema =
  McpLetteringResourceReferenceSchema.extend({
    offset: z.number().int().nonnegative().default(0),
...
export const mcpLetteringResourceOutputs = {
  carrot_list_lettering_resources: z
    .object({ ...window, snapshot, resources: z.array(summary).max(25) })
    .strict(),
  carrot_get_lettering_resource: summary
    .extend({
      ...window,
        ...
        .array(
          z
            .object({
              ...
              schemeJson: z
                .string()
                .max(MCP_LETTERING_RULE_JSON_LIMIT)
                .nullable(),
            })
            .strict(),
        )
