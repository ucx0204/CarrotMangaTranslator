...
export function registerSettingsIpc(context: IpcContext): void {
  ...
  trustedHandle(context, "settings:save", async (_event, settings: unknown) =>
    saveAppSettings(parseIpcPayload(AppSettingsSchema, settings, "설정 저장")),
  );
  trustedHandle(context, "settings:reset", async () => resetAppSettings());
  trustedHandle(
    context,
    "settings:pick-local-model",
