...
export function readMcpConfiguration(
  ...
  return {
    ...
    token,
    publicOrigin,
    allowImages: images === "1",
    oauthPassword,
  };
}

function readOAuthPassword(
  env: NodeJS.ProcessEnv,
  ...
  if (enabled !== "1")
    throw new Error("CARROT_MCP_OAUTH_ENABLED must be 0 or 1.");
  const password = env.CARROT_MCP_OAUTH_PASSWORD ?? "";
  if (
    !origin ||
    !/^[A-Za-z0-9_-]{43,128}$/.test(password) ||
    password === token
  )
    throw new Error(
      "Web OAuth requires an exact HTTPS origin and a separate 43-128 character connection password.",
    );
  return password;
}
