import { createMcpJobFileTool } from "./mcpJobFileTool";
...
export function createMcpOperationTools(
  ...
  if (assertFileAvailable)
    tools.push(
      createMcpJobFileTool(operations, {
        available: assertFileAvailable,
        allowImages: output?.allowImages ?? Boolean(executors.exportPng),
        disclosed: output?.disclosed,
      }),
    );
  return tools;
}
