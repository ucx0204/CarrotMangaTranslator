...
import type { SelectionPlanningPorts, SelectionSnapshotRequest } from "../application/mcpSelectionEditPolicy";
...
export function createMcpSelectionEditAdapter(
  ...
  const planning: SelectionPlanningPorts = {
    prepare: async (_saved, input, access) => {
      ...
      if (!["selectionOcr", "selectionTranslation"].includes(job.kind) ||
          job.status !== "completed" || job.result?.selectionAnalysis?.analysisId !== id)
        throw new McpEditError("not_found", "An owned completed selection analysis is required.");
      const evidence = analyses.require(access.owner, id, access.guard);
      if (evidence.binding.chapterId !== input.chapterId ||
          evidence.kind !== (job.kind === "selectionOcr" ? "ocr" : "translation"))
        throw new McpEditError("invalid_edit", "Analysis identity or chapter does not match this request.");
      await verifyMcpSelectionEvidence(evidence.binding, access.guard);
      const format = input.pages.some((page) => page.edits.some((edit) => edit.kind === "append"))
        ? await defaults() : undefined;
      access.guard();
  ...
  const ports = createMcpPageBatchPorts<SelectionSnapshotRequest>(
    (request, membership, guard, committed, scope) => {
      const authorize = () => {
        guard();
        if (request.direction !== "undo" && request.evidenceExpiresAt !== null &&
            request.evidenceExpiresAt <= Date.now())
          throw new McpEditError("not_found", "Selection evidence expired; prepare a new analysis.");
      };
...
async function withSelectionEvidence<T>(
  request: SelectionSnapshotRequest, guard: () => void, run: () => Promise<T>,
): Promise<T> {
  guard();
  ...
  const resources: AppActivityResource[] = [
    ...
    ...request.binding.pages.map((page) => ({ kind: "page-content" as const,
      scope: `${request.chapterId}/${page.pageId}`, access: "read" as const })),
  ];
