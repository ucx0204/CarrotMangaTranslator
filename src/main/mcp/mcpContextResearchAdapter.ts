...
import { libraryStructureResource, pageContentResource, type AppActivityResource } from "../../shared/appActivityTypes";
...
export function createMcpContextResearchExecutor(
  ...
  return async (owner: string, target: McpContextResearchTarget, operation: McpOperationContext) => {
    ...
    if (target.engine === "tavily" && settings.internetResearch.tavilyAnalysisProvider === "api")
      assertMcpRemoteTextApi(buildBaseOptions(operation.id, app.appPaths.dataRoot, { ...settings, modelProvider: "openai-api" }, app.appPaths));
    const work = (await listLibrary()).works.find((entry) => entry.id === snapshot.workId);
    if (!work) throw new McpEditError("not_found", "Research work no longer exists.");
    ...
    const resources: AppActivityResource[] = [
      ...
      ...chapters.flatMap((id) => [libraryStructureResource("chapter", id, "read"), { ...pageContentResource(id, "**"), access: "read" as const }]),
    ...
    operation.assertAuthorized();
    return runMcpAppJob(app, operation, "internet-research", (context) =>
      withExecutionSettings(settings, async () => {
        const latest = (await listLibrary()).works.find((entry) => entry.id === snapshot.workId);
        if (JSON.stringify(latest?.chapters.map((entry) => entry.id)) !== JSON.stringify(chapters))
          throw new McpEditError("revision_conflict", "Research chapter membership changed before execution.");
        return new McpContextResearchService({
          read: readWorkContextForEdit, proposals,
          research: (request, job) => research(request, job.signal, (progress) => {
            job.assertAuthorized();
            job.progress({ phase: progress.research?.stage ?? progress.phase ?? "researching" });
          }),
