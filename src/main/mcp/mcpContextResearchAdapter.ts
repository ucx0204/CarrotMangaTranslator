...
import type { researchWorkContext } from "../workContextResearch";
...
export function createMcpContextResearchExecutor(
  ...
  research?: typeof researchWorkContext,
) {
  return async (
    owner: string,
    target: McpContextResearchTarget,
    operation: McpOperationContext,
    expectedReferenceSnapshot?: string,
  ) => {
    operation.assertAuthorized();
    const executeResearch =
      research ??
      (await import("../workContextResearch.js")).researchWorkContext;
    operation.assertAuthorized();
    ...
    const verifyWork = await researchWorkVerifier(
      target.chapterId,
      operation.assertAuthorized,
    ...
    // The retained builder binds the original snapshot under its native transaction.
        ...
        runMcpAppJob(
          ...
          (context) =>
            withExecutionSettings(settings, async () => {
              ...
              return executeResearch(request, context.signal, (progress) => {
                context.assertAuthorized();
                context.progress({
                  phase:
...
/** The anchor context revision does not cover other chapters' saved dialogue. */
async function researchWorkVerifier(
  chapterId: string,
  guard: () => void,
  ...
  const read = async () => {
    ...
    return contextMigrationSnapshot(graph, chapterId, guard).snapshot;
  };
  const expected = preparedSnapshot ?? (await read());
  return async () => {
    guard();
    if ((await read()) !== expected)
      throw new McpEditError(
        "revision_conflict",
        "Saved work changed during research. No proposal is returned; review current context before retrying.",
      );
    guard();
    return expected;
  };
...
async function verifyResearchStart(
  workId: string,
  chapters: string[],
  target: McpContextResearchTarget,
  verifyWork: () => Promise<string>,
) {
