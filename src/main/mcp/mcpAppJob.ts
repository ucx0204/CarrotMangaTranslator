...
type McpAppJobScope<T = unknown> = {
  ...
  /** Preserve a truthful native completion when execution returns partial effects. */
  completionStatus?: (result: T) => "completed" | "failed" | "cancelled";
};
...
export async function runMcpAppJob<T>(
  ...
  try {
    ...
    emitStatus(
      ...
      operation.id,
      kind,
      scope?.completionStatus?.(result) ?? "completed",
    );
    return result;
  } catch (error) {
