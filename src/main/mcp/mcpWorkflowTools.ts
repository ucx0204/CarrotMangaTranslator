...
import {
  ...
  McpWorkflowGetSchema,
  McpWorkflowRunSchema,
  McpWorkflowExternalSchema,
  McpWorkflowListSchema,
  McpWorkflowDiscardSchema,
...
export function createMcpWorkflowTools(
  ...
  for (const direction of ["pause", "cancel"] as const)
    tools.push(
      createMcpBatchTool({
        name: `carrot_${direction}_workflow`,
        ...
        execute: (args, owner, guard) =>
          service.control(
            owner,
            McpWorkflowGetSchema.parse(args).id,
            direction,
            guard,
          ),
      }),
  ...
  tools.push(
    createMcpBatchTool({
      name: "carrot_accept_workflow_external",
      schema: McpWorkflowExternalSchema,
      scopes: editScopes,
      write: true,
      description:
        "Acknowledge that the currently waiting external page was saved with the existing editing/upload tools. Requires its exact CURRENT page/review revisions and workflow version. Does not accept raw text/images or write the page. Original identity, chapter order, context and other fixed pages must match. Acceptance marks only that step complete and leaves the workflow paused for explicit resume. A filename or claimed AI result is not evidence of a saved page.",
      execute: (args, owner, guard) =>
        service.acceptExternal(
          owner,
          McpWorkflowExternalSchema.parse(args),
          guard,
        ),
    }),
    createMcpBatchTool({
      name: "carrot_discard_workflow",
      schema: McpWorkflowDiscardSchema,
      scopes: editScopes,
      write: true,
      description:
        "Discard one owned settled workflow plan with confirm=true. Does not delete page data, native change history or retained output. Running work must first settle after cancellation. A discarded plan cannot resume.",
      execute: (args, owner, guard) =>
        service.discard(owner, McpWorkflowDiscardSchema.parse(args).id, guard),
    }),
  );
  return tools;
}
function runTool(name: string, service: McpWorkflowService): McpTool {
  return {
    ...
    invoke: async (args, context) => {
      ...
      return textContent(
        await service.run(context.principalId, parsed.data, authorize),
      );
    },
  };
}
