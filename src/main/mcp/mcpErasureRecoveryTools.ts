...
import {
  mcpErasureRecoveryLookup,
  mcpErasureRecoveryAction,
} from "../../shared/mcpErasureRecoverySchemas";
...
export function createMcpErasureRecoveryTools(
  ...
  return [
    ...
    ...(["undo", "redo"] as const).map(
      (direction): McpTool => ({
        name:
          direction === "undo" ? "carrot_undo_erasure" : "carrot_redo_erasure",
        readOnly: false,
        destructive: true,
        idempotent: true,
        openWorld: false,
        ...
        invoke: async (args, context) => {
          ...
          return textContent(
            await service.apply(
              {
                ...parsed.data,
                revision: parsed.data.revision as PageRevision,
                direction,
              },
              owner,
              authorize,
  ...
  const authorize = () => {
    if (!context?.principalId || !context.assertScopes)
      throw new McpEditError(
        "access_denied",
        "An approved OAuth connection and scope verification are required.",
      );
