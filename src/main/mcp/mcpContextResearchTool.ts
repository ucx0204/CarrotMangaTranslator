...
export function createMcpContextResearchTool(
  ...
  return {
    ...
    description:
      "Run the app's configured Tavily/Gemma-or-API or Codex-web research for ONE existing work selected by chapterId. Read carrot_get_work_context for revision first and provide the explicit researchTitle and engine. Sends bounded saved work text and context to the configured research services, may use multiple searches/model requests and charge within the app's configured budgets. The existing Codex compatibility fallback can run if necessary. Saved text across the work may contain spoilers. Holds shared work-content/context leases; local model workloads remain exclusive through cleanup. Returns a job receipt, NOT completed research. Poll carrot_get_job, inspect the returned context proposal, and explicitly apply selected changes. NEVER changes glossary, characters, memories, translations or images during research. With native retention available, completed reviews are encrypted and kept for seven days; inspect retention and expiresAt. After restart use retainedContextProposal.proposalId or carrot_list_context_proposals, not a fresh research request. Without retention, reviews remain session-only for 30 minutes. Lookup never researches again; expired, discarded or changed-work results cannot be forced into the work. No attachments.",
    ...
    invoke: async (args, context) => {
      ...
      if (
        ...
        throw new McpEditError(
          "access_denied",
          "An approved processing connection is required for research.",
        );
