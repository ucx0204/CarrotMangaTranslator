...
export function authorizeMcpWorkflow(context: Context, record: McpWorkflowRecord) {
  const scopes = [
    "carrot.read", "carrot.edit", "carrot.process",
    ...(record.input.stages.some((stage) => stage.kind === "export-png") ? ["carrot.images"] : []),
  ];
  if (context.assertJobAuthorized) context.assertJobAuthorized(scopes);
  else {
    context.assertAuthorized();
    if (!context.assertScopes)
      throw new McpEditError("access_denied", "Scope verification is required.");
