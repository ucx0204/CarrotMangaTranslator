...
export function readTailscaleOrigin(value: unknown): string {
  ...
  const hostname = typeof self.DNSName === "string"
...
export function assertTailscaleListenerFree(value: unknown, port = 443): void {
  ...
  while (queue.length) {
    ...
    if (++examined > 256)
      throw new Error("Tailscale sharing configuration is too large to inspect safely.");
    const tcp = config.TCP === undefined ? {} : object(config.TCP);
    const web = config.Web === undefined ? {} : object(config.Web);
    if (Object.hasOwn(tcp, String(port)) || Object.keys(web).some((key) => key.endsWith(`:${port}`)))
      throw new Error("Tailscale HTTPS port 443 is already used. Carrot will not replace or expose another application's route.");
    for (const group of [config.Foreground, config.Services]) {
      if (group === undefined || group === null) continue;
      for (const child of Object.values(object(group))) queue.push(object(child));
    }
  }
}
...
export function readTailscaleSetupUrl(text: string): string | undefined {
  const match = text.match(/https:\/\/login\.tailscale\.com\/[A-Za-z0-9_/?=&.%+-]+/);
