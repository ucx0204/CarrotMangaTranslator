...
import { allowArguments, argumentObject, identifierSchema, readIdentifier, readQuery, readWindow, windowProperties } from "./mcpArguments";

type McpToolContent = { type: "text"; text: string } | { type: "image"; data: string; mimeType: "image/png" };
export type McpTool = {
  name: string;
  description: string;
...
export function createMcpReadTools(service: McpLibraryReadService, imageTransfer = false, oauth = false): McpTool[] {
  return [
    {
    ...
    },
    {
      ...
      inputSchema: objectSchema({ ...windowProperties, query: { type: "string", maxLength: 200 } }),
      invoke: async (args) => {
