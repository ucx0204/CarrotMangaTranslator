...
import {
  McpChapterTextSearchSchema, McpTranslationBatchPreviewSchema,
  McpTranslationBatchGetSchema, McpTranslationBatchActionSchema,
...
export function createMcpTranslationBatchTools(ports: Ports, allowEditing: boolean, lifetime?: AbortSignal): McpTool[] {
  ...
  for (const direction of ["apply", "undo", "redo"] as const) tools.push(makeTool({
    ...
    scopes: editScopes, write: true, background: true,
    execute: async (args, owner, guard) => service.start(owner, args, direction, guard),
  }));
  tools.push(makeTool({
    name: "carrot_cancel_translation_batch", schema: McpTranslationBatchActionSchema,
    description: "Cancel ONLY the batch action requestId currently shown by get_translation_batch. Wait for terminal status before any undo or next action. An older cancellation cannot cancel a newer action. Cancellation stops future page commits and never rolls back already saved pages. No files, models or rendering.",
    scopes: editScopes, write: true,
    execute: async (args, owner, guard) => service.cancel(owner, args, guard),
  }));
  return tools;
}
function makeTool(options: {
  name: string; description: string; schema: z.ZodType; scopes: string[];
  write: boolean; background?: boolean;
  execute: (args: unknown, owner: string, guard: () => void) => Promise<unknown>;
}): McpTool {
  return { name: options.name, description: options.description,
    inputSchema: z.toJSONSchema(options.schema), oauth: true,
    requiredScopes: options.scopes, readOnly: !options.write,
    destructive: options.write && !options.name.includes("cancel"), idempotent: true,
    invoke: async (args, context) => {
      if (!context?.principalId) throw new McpEditError("access_denied", "An approved connection is required.");
      const guard = () => {
        if (options.background && context.assertJobAuthorized) context.assertJobAuthorized(options.scopes);
      ...
      return textContent(await options.execute(parsed.data, context.principalId, guard));
    },
  };
}
