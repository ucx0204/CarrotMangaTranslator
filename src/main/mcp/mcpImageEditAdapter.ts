...
import {
  produceMcpImageEdit,
  type McpImageEditRuntime,
  type McpImageProduct,
} from "./mcpImageEditExecution";
import {
  publishMcpImageEdit,
  replayMcpImageEdit,
  discardMcpImageProduct,
  ...
  const ports = createMcpPageBatchPorts<McpImageEditRequest>(
    async (request, membership, guard, committed, contextScope) => {
      const erasing =
        ...
        "expectedEngine" in request.change.command;
      const pageScope = createMcpPageEditScope(
        app,
        openChapter,
        lifetime,
        erasing ? [{ kind: "model-runtime", scope: "*", access: "write" }] : [],
      );
      await pageScope(request, guard, (authorize, signal) =>
        contextScope(async () => {
          ...
          if (request.direction === "apply") {
            if (references.size >= 64)
              throw new McpEditError(
                "editor_busy",
                "This session has 64 retained image edits. Existing recovery remains available; no new image was created.",
              );
            await forwardImageEdit(
              app,
              history,
              runtime,
              request,
            ...
            await replayMcpImageEdit(
              ...
              authorize,
              async (revision) => {
                const updated = await readMcpImageEditPage(
                  { ...request, revision },
                  () => {},
                );
                committed(updated);
...
async function forwardImageEdit(
  ...
  if (prepared.evidence.mask.snapshot !== request.change.evidence.mask.snapshot)
    throw new McpEditError(
      "revision_conflict",
      "Reviewed mask or original/cleaned image changed. Prepare a new plan.",
    );
  let product: McpImageProduct | undefined;
  try {
    product = await produceMcpImageEdit(
      app,
      page,
      request.change.command,
      prepared,
