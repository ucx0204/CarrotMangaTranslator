...
export class McpWorkflowCalls {
  ...
  async call(name: string, args: Record<string, unknown>): Promise<unknown> {
    ...
    if (!tool)
      throw new McpEditError(
        "invalid_edit",
        "A required native workflow operation is unavailable.",
      );
  ...
  async job(
    ...
    if (!tool)
      throw new McpEditError(
        "invalid_edit",
        "Required native job is unavailable.",
      );
    this.guard();
    const result = mcpToolResult(
      tool,
    ...
    try {
      ...
      await this.operations.waitForCompletion(
        jobId,
        this.owner,
    ...
    const completed = await this.operations.waitForCompletion(
      jobId,
      this.owner,
      signal,
    );
    ...
    if (completed.status !== "completed" || completed.result?.cleanupFailed)
      throw new McpEditError(
        "invalid_edit",
        "Native workflow job failed, was cancelled or requires cleanup. Inspect its recorded job ID before retrying.",
      );
    if (!completed.result)
      throw new Error("Native job completed without result metadata.");
