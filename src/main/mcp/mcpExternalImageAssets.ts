...
export function withExternalImageAssets<T>(
  ...
  guard: () => void,
  run: (assets: ExternalImageAssets) => Promise<T>,
) {
  const command = input.command;
  const references = [
    { key: "image" as const, id: command.imageUploadId, purpose: "image" },
    ...(command.maskUploadId
      ? [{ key: "mask" as const, id: command.maskUploadId, purpose: "mask" }]
      : []),
    ...(command.protectedMaskUploadId
      ? [
          {
            key: "protectedMask" as const,
            id: command.protectedMaskUploadId,
            purpose: "mask",
          },
        ]
      : []),
  ...
  const visit = (index: number, check: () => void): Promise<T> => {
    ...
    return store.use(owner, reference.id, check, async (asset) => {
      ...
      if (
        binding.chapterId !== input.chapterId ||
        binding.pageId !== input.pageId ||
        binding.revision !== input.revision ||
        binding.contextRevision !== input.contextRevision ||
        binding.purpose !== reference.purpose
      )
        throw new McpEditError(
          "revision_conflict",
          "Upload purpose or saved page/context binding differs from this plan.",
        );
      if (!result)
        result = {
          image: asset.bytes,
        ...
        if (
          binding.width !== result.width ||
          binding.height !== result.height ||
          hashStableValue(asset.files) !== hashStableValue(result.files)
        )
          throw new McpEditError(
            "revision_conflict",
            "Image and masks must have identical dimensions and original/cleaned evidence.",
          );
