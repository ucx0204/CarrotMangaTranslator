...
import { assertMcpSelectionFreshness, type McpSelectionBinding } from "../application/mcpSelectionAnalysisService";
...
export async function captureMcpSelectionEvidence(saved: McpContextSnapshot, input: McpSelectionInput, guard: () => void): Promise<McpSelectionBinding> {
  ...
  if (!("expectedEngine" in input)) {
    for (const target of binding.pages) {
      ...
      if (!page) throw new McpEditError("not_found", "Selected page is missing.");
...
export async function verifyMcpSelectionEvidence(binding: McpSelectionBinding, guard: () => void): Promise<void> {
  guard();
  const saved = await readWorkContextForEdit(binding.chapterId);
  guard(); assertMcpSelectionFreshness(saved, binding);
  for (const target of binding.pages) {
    if (!target.sourceHash) continue;
    const page = saved.chapter.pages.find((page) => page.id === target.pageId);
    if (!page || await hashMcpOriginalImage(page.imagePath, guard) !== target.sourceHash)
      throw new McpEditError("revision_conflict", "Selected original image changed. Prepare new evidence before applying.");
  }
  guard();
}
