...
import type { McpWorkflowRecord, McpWorkflowOutcome, McpWorkflowStep } from "../application/mcpWorkflowPolicy";
...
async function reconcileOutput(
  ...
  if (
    output.targets.length !== 1 || output.targets[0].pageId !== expected.pageId ||
...
async function reconcileChange(
  ...
  if (
    change.pages.length !== 1 || saved.chapterId !== expected.chapterId ||
