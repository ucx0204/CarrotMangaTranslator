    ...
    open: async (record: McpWorkflowRecord, guard: () => void) => {
      ...
      if (workflowSettingsFingerprint(settings) !== record.settingsFingerprint)
        throw new McpEditError(
          "revision_conflict",
          "Workflow execution settings changed. Restore them or prepare a new remaining-target plan; no model fallback was started.",
        );
...
function validateWorkflowSettings(
  ...
  for (const stage of input.stages) {
    ...
    if (stage.expectedEngine !== settings.modelProvider)
      throw new McpEditError(
        "revision_conflict",
        "Workflow translation provider differs from the configured provider.",
      );
    if (
      stage.expectedEngine === "gemma"
        ? !stage.allowAssetDownloads
        : !stage.allowExternal
    )
      throw new McpEditError(
        "access_denied",
        "Explicit permission for local assets or external text processing is required.",
      );
  }
}
async function executeStage(
  ...
  if (step.stage === "translate")
    return translateWorkflowPage(record, step, calls, signal, onJob, guard);
  const target = record.pages[step.pageIndex];
  const { page } = await readWorkflowPage(target, guard);
  if (
    (step.stage === "ocr" && page.blocks.length > 0) ||
    (step.stage === "erase" &&
      !page.blocks.some(
  ...
  if (step.stage === "await-external" || !step.attemptId)
    throw new Error("External steps cannot invoke native engines.");
  const result = await calls.job(
    names[step.stage],
    {
  ...
  if (typeof result.revision !== "string")
    throw new Error("Native workflow result has no exact revision.");
  return {
    revision: result.revision,
    ...(result.retainedOutputId ? { outputId: result.retainedOutputId } : {}),
  };
}
