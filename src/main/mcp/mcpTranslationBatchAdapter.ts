...
export function createMcpTranslationBatchPorts(edits: McpPageEditService) {
  const commit: BatchTextCommit = async (request, expected, guard, onCommitted) => {
    await edits.commitTranslationBatch(request, expected.membership, guard, onCommitted, async (run) => {
      ...
      try {
        ...
        guard();
        if (saved.workId !== expected.workId ||
            (expected.contextRevision !== null && mcpContextRevision(saved) !== expected.contextRevision))
          throw new McpEditError("revision_conflict", "Saved context changed. Re-read context and prepare a new plan for remaining pages.");
        return await run();
      } finally { release(); }
    });
  };
  return { read: readWorkContextForEdit, commit,
    reportError: (error: unknown) => logError("MCP translation batch stopped", error) };
}
