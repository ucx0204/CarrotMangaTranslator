...
<dl><dt>요청 클라이언트</dt><dd>${escapeMcpHtml(consent.clientName)}</dd><dt>연결할 서버</dt><dd>${escapeMcpHtml(consent.resource)}</dd><dt>권한</dt><dd>${escapeMcpHtml(consent.scope)}</dd></dl>
...
<p>승인하면 이 실행 중인 앱에 최대 24시간 동안 접근할 수 있습니다. 앱 종료·재시작으로 권한이 폐기됩니다. 외부 프록시를 통해 전송되므로 공개 가능한 시험 자료만 사용하세요.</p>
