...
export function createMcpSelectionEditSession(
  app: InpaintingJobContext, operations: McpOperationService,
  analyses: McpSelectionAnalysisService, editing: Editing,
) {
  ...
  const adapter = createMcpSelectionEditAdapter(edits, operations, analyses,
    async () => (await getAppSettings(app.appPaths)).blockFormatDefaults);
  const service = new McpPageBatchService(adapter.ports,
    createMcpSelectionEditPolicy(adapter.planning), Date.now, lifetime.signal);
  return {
    tools: selectionEditTools(service),
    stop: () => lifetime.abort(),
    close: async () => { lifetime.abort(); await service.close(); },
  };
}

function selectionEditTools(service: McpPageBatchService<
...
>) {
  ...
  for (const direction of ["apply", "undo", "redo"] as const) {
    tools.push(createMcpBatchTool({
      name: `carrot_${direction}_selection_batch`, schema: McpTranslationBatchActionSchema,
  ...
  tools.push(createMcpBatchTool({
    name: "carrot_cancel_selection_batch", schema: McpTranslationBatchActionSchema,
    scopes, write: true,
    description: "Cancel only the current selection action requestId shown by inspection. Wait for terminal status before undo or another action. Earlier cancellations cannot cancel a newer action. Already committed pages remain saved and require explicit undo. No model, file or rendering.",
