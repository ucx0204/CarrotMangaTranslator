...
const client = z
  ...
  .strict()
  .refine(
    (value) => (value.method === "none") === (value.secretHash === undefined),
    "Client authentication metadata is inconsistent.",
  );
const grant = z
  .object({
    id: z.string().uuid(),
...
const snapshot = z
  .object({
    ...
    refresh: entries(
      z.object({ grantId: z.string().uuid(), used: z.boolean() }).strict(),
      8192,
    ),
  })
  .strict();
...
export function parseMcpOAuthSnapshot(
  ...
  for (const item of result.grants) {
    if (
      !clients.has(oauthDigest(item.clientId)) ||
      item.resource !== `${issuer}/mcp`
    )
      throw new Error(
        "The saved OAuth grant has an invalid client or audience.",
      );
  }
  for (const item of result.access)
    if (!grants.has(item.value))
      throw new Error("Unknown saved OAuth access grant.");
...
function uniqueKeys(keys: string[]): Set<string> {
  ...
  if (unique.size !== keys.length)
    throw new Error("Duplicate OAuth records are not accepted.");
  return unique;
}
