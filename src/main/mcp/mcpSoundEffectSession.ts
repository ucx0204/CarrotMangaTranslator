...
export function createMcpSoundEffectSession(
  ...
  return {
    ...
    readOwnedPlan: batches.readOwnedPlan.bind(batches),
    tools: [
      ...
      ...(enabled
        ? [
            ...
            ...actionTools(batches),
            ...(allowImages
              ? [ownedSoundEffectImageTool(batches, lifetime.signal)]
              : []),
          ]
        : []),
    ],
...
>;
function ownedSoundEffectImageTool(batches: Batches, signal: AbortSignal) {
  return createMcpSoundEffectImageTool(async (owner, batchId, guard) => {
    const view = await batches.inspect(owner, { batchId, limit: 1 }, guard);
    return {
