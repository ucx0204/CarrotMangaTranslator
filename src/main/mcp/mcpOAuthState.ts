...
export class McpOAuthState<T> {
  ...
  constructor(private readonly now: () => number, private readonly capacity = 256) {}
  ...
  snapshot<U>(mapValue: (value: T) => U): { digest: string; value: U; expiresAt: number }[] {
    this.prune();
    return [...this.entries].map(([digest, item]) => ({ digest, expiresAt: item.expiresAt, value: mapValue(item.value) }));
  }
  restore<U>(records: readonly { digest: string; value: U; expiresAt: number }[], mapValue: (value: U) => T): void {
    if (records.length > this.capacity) throw new Error("OAuth snapshot exceeds capacity.");
    const entries = new Map<string, { value: T; expiresAt: number }>();
    for (const item of records) {
      if (!/^[A-Za-z0-9_-]{43}$/.test(item.digest) || entries.has(item.digest) || !Number.isSafeInteger(item.expiresAt))
        throw new Error("Invalid OAuth state record.");
      entries.set(item.digest, { value: mapValue(item.value), expiresAt: item.expiresAt });
    }
    this.entries.clear();
