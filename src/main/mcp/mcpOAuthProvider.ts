...
type Grant = { clientId: string; scope: string; resource: string; expiresAt: number; revoked: boolean };
...
export class McpOAuthProvider {
  ...
  resourceMetadata() {
    return { resource: this.resource, authorization_servers: [this.issuer], scopes_supported: ["carrot.read"], bearer_methods_supported: ["header"] };
  }
  ...
  begin(input: Record<string, unknown>) {
    ...
    const pending: Pending = {
      ...
      code: { redirect, challenge: readPkceChallenge(input.code_challenge, input.code_challenge_method), used: false,
        grant: { clientId, scope, resource: this.resource, expiresAt: this.now() + DAY, revoked: false } },
    };
  ...
  approve(input: Record<string, unknown>, cookie: string) {
    ...
    if (!pending || !oauthEqual(pending.cookie, cookie))
      throw new McpOAuthError("invalid_request", "Approval expired or browser session changed. Start linking again.");
  ...
  accepts(header: string): boolean {
    ...
    return !!grant && !grant.revoked && grant.expiresAt > this.now() && grant.resource === this.resource && grant.scope.split(" ").includes("carrot.read");
  }
  ...
  private exchangeCode(input: Record<string, unknown>, clientId: string) {
    ...
    if (code.used) {
      ...
      throw new McpOAuthError("invalid_grant", "Authorization code was already used. Reconnect.");
    }
    code.used = true;
  ...
  private exchangeRefresh(input: Record<string, unknown>, clientId: string) {
    ...
    if (entry.used) {
      ...
      throw new McpOAuthError("invalid_grant", "Refresh replay detected. Reconnect.");
    }
    if (input.scope !== undefined && readOAuthScope(input.scope) !== entry.grant.scope)
  ...
  private issueTokens(grant: Grant) {
    if (grant.revoked || grant.expiresAt <= this.now())
      throw new McpOAuthError("invalid_grant", "Authorization expired or was revoked. Reconnect.");
    ...
    return {
      ...
      refresh_token: this.refresh.issue({ grant, used: false }, grant.expiresAt - this.now()),
      scope: grant.scope,
    };
  }
}
