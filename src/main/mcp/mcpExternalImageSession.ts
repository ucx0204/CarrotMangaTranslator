...
export function createMcpExternalImageSession(
  ...
  const service = new McpPageBatchService(
    adapter.ports,
    createMcpExternalImagePolicy(adapter.planning),
    Date.now,
    lifetime.signal,
  );
  const stop = () => {
  ...
  return {
    tools: enabled
      ? [
          ...uploads.tools,
          ...externalTools(service),
          ...(allowImages
            ? [
                createMcpExternalImageReadTool(
                  uploads.store,
...
function externalTools(service: Service): McpTool[] {
  ...
  const tools = [
    createMcpBatchTool({
      name: "carrot_preview_external_image",
      schema: McpExternalImagePreviewSchema,
      scopes,
      write: false,
      description:
    ...
    createMcpBatchTool({
      name: "carrot_get_external_image",
  ...
  for (const direction of ["apply", "undo", "redo"] as const)
    tools.push(
      createMcpBatchTool({
        name: `carrot_${direction}_external_image`,
  ...
  tools.push(
    createMcpBatchTool({
      name: "carrot_cancel_external_image",
      schema: McpTranslationBatchActionSchema,
      scopes,
      write: true,
      description:
