...
type Service = {
  ...
  file: (owner: string, id: string, guard: () => void) => Promise<unknown>;
  apply: (
    owner: string,
  ...
  if (images)
    tools.push(
      createMcpBatchTool({
        ...
        background: true,
        description:
          "Issue a NEW ten-minute file link for owned retained native PNG/ZIP bytes. No rendering, model, implicit export or page save. Rechecks owner, same profile, page/source hashes, image permission, redaction, content bytes and retention expiry. Revocation, stop, page/source changes or explicit discard block the link. The historical bytes are not silently regenerated or resized.",
        execute: (args, owner, guard) =>
          service.file(owner, McpRetainedGetSchema.parse(args).id, guard),
      }),
    );
  if (enabled) {
      ...
      createMcpBatchTool({
        ...
        description:
          selected === "change"
            ...
            : "Inspect an owned retained PNG/ZIP and its original page bindings without rendering or transferring image bytes. Stored output and source/page integrity must remain valid. Download additionally requires current image permission and redaction checks. Use carrot_get_output_file to issue a fresh short-lived link; old links never revive.",
        execute: (args, owner, guard) =>
