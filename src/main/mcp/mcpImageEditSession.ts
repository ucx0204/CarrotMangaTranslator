...
export function createMcpImageEditSession(
  app: InpaintingJobContext, editing: Editing, enabled: boolean, allowImages: boolean,
  runtime: McpImageEditRuntime = productionInpaintingJobRuntime,
) {
  const lifetime = new AbortController();
  const history = app.inpaintingRevisionStore;
  if (!enabled || !history?.inspectSinglePageTransaction ||
      !history.applySinglePageTransaction || !history.releaseTransactions)
    return { tools: [] as McpTool[], stop: () => lifetime.abort(), close: async () => { lifetime.abort(); } };
  const adapter = createMcpImageEditAdapter(app, history as McpImageHistory, editing, runtime, lifetime.signal);
  const service = new McpPageBatchService(adapter.ports, createMcpImageEditPolicy(adapter.planning), Date.now, lifetime.signal);
  return {
    tools: [
      ...imageEditTools(service),
      ...(allowImages ? createMcpImageEditReadTools((owner, args, guard) => service.inspect(owner, args, guard)) : []),
    ],
    stop: () => lifetime.abort(),
    close: async () => {
      lifetime.abort();
...
function imageEditTools(service: ImageService): McpTool[] {
  ...
  for (const direction of ["apply", "undo", "redo"] as const) {
    tools.push({
      ...createMcpBatchTool({
        name: `carrot_${direction}_image_edit`, schema: McpTranslationBatchActionSchema,
