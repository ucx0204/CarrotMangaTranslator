...
import { reconstructPythonSealedJsonWithoutNestedKey } from "./preservedJsonRecordSeal";
...
/** Preserve Python json.dumps float spellings while sorting the sealed core. */
function canonicalRecordCoreFromJson(text: string): string | null {
  try {
    const root = new PreservedJsonParser(text).parse();
    if (root.type !== "object") return null;
    if (!root.entries.some(([key]) => key === "record_sha256")) return null;
    return canonicalPreservedJson(root, "record_sha256");
  } catch (_error) {
    return null;
  }
}
