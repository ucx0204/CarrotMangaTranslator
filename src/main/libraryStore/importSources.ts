...
export async function listNestedImageFolders(rootPath: string): Promise<string[]> {
  ...
  async function walk(currentPath: string, depth: number): Promise<void> {
    ...
    const imageCount = entries.filter((entry) => entry.isFile() && isSupportedImagePath(entry.name)).length;

    if (currentPath !== rootPath && imageCount > 0) {
      discoveredPages += imageCount;
      if (found.length >= MAX_NESTED_IMAGE_FOLDERS || discoveredPages > MAX_NESTED_IMAGE_FOLDER_PAGES) {
