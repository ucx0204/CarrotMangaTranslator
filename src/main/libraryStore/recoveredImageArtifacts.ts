...
export function isRecoveredImageArtifact(
  chapterDir: string,
  kind: "inpainted" | "mask",
  path: string,
) {
  ...
  return (
    parts.length === 2 &&
    /^\.mcp-recovered-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(
      parts[0],
    ) &&
    parts[1] ===
      (kind === "mask" ? "inpaintMaskPath.png" : "inpaintedImagePath.png")
  );
}
const isRecovered = (root: string, path: string) =>
  isRecoveredImageArtifact(root, "inpainted", path) ||
  isRecoveredImageArtifact(root, "mask", path);

/** Candidate-only cleanup: current references and active artifact leases were checked by the caller. */
export async function removeManagedImageCandidate(
  chapterDir: string,
  path: string,
) {
...
export async function retireReplacedRecoveryImages(
  ...
  const live = new Set(
    after.flatMap((page) =>
      fields.flatMap((field) => (page[field] ? [key(page[field])] : [])),
    ),
  );
  ...
  for (const page of before)
    for (const field of ["inpaintedImagePath", "inpaintMaskPath"] as const) {
      ...
      if (
        !path ||
        !isRecovered(chapterDir, path) ||
        live.has(key(path)) ||
        isLibraryArtifactRetained(path)
      )
        continue;
  ...
  for (const [directory, files] of groups) {
    ...
    const ownsAll =
      ...
      contents.every(
        (item) =>
          item.isFile() && candidates.has(key(join(directory, item.name))),
      );
    if (ownsAll)
      await transaction.retireDirectory(directory, { required: true });
    else
      for (const path of files)
        await transaction.retireFile(path, { required: true });
  }
}
