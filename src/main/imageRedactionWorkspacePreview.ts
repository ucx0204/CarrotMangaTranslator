...
import { redactionPreviewRequestSchema, type RedactionPreviewRequest } from "../shared/imageRedactionWorkspace";
...
export async function getRedactionWorkspacePreview(input: RedactionPreviewRequest): Promise<string> {
  const request = redactionPreviewRequestSchema.parse(input);
  const { page, signal } = getRedactionWorkspacePage(request.sessionId, request.pageId);
  signal.throwIfAborted();
  const bytes = await readFile(page.imagePath);
  signal.throwIfAborted();
  if (createHash("sha256").update(bytes).digest("hex") !== page.fingerprint)
    throw new Error("원본 이미지가 변경되었습니다. 해당 페이지를 다시 준비해 주세요.");
  ...
  const image = await loadPageImageSnapshot(page.imagePath, bytes, (path) => decodeImageThroughRuntime(getAppPaths().runtimeDir, path, signal), signal);
  signal.throwIfAborted();
  const size = image.getSize();
  if (size.width !== page.width || size.height !== page.height)
    throw new Error("원본 이미지 크기가 변경되었습니다.");
  const scale = Math.min(1, request.maxEdge / Math.max(size.width, size.height));
