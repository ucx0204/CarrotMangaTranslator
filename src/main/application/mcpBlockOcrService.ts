...
type Evidence = Pick<McpBlockOcrObservation, "recognizedText" | "regions" | "sourceLanguage" | "sourceCropSha256"> & { engine: string };
type Ports = {
  openChapter: (chapterId: string) => Promise<ChapterSnapshot>;
  recognize: (page: MangaPage, cropRect: PixelRect, context: McpOperationContext) => Promise<Evidence>;
};

/** No persistence/mutation port is available. The caller retains the existing
...
export class McpBlockOcrService {
  ...
  async run(target: McpBlockOcrTarget, context: McpOperationContext) {
    ...
    return {
      ...
      revision: target.revision, status: "observed", engine, performed: ["block-ocr"],
      pagesChanged: 0, noTextDetected: !observation.recognizedText.trim(), needsReview: true,
      observationExpired: false, blockOcr: buildMcpOcrObservation(selected, observation),
    };
  }
  private async load(target: McpBlockOcrTarget): Promise<MangaPage> {
    ...
    if (chapter.id !== target.chapterId || !page) throw new McpEditError("not_found", "Page not found.");
...
export function selectMcpBlockOcr(page: MangaPage, blockId: string) {
  ...
  const raw = block.bboxSpace === "pixels" ? block.bbox : bboxToPixels(block.bbox, page.width, page.height);
  return selectMcpSourceCrop(page, raw, block.sourceText);
}

/** Region OCR and block OCR share exactly the same containment and pixel rounding. */
export function selectMcpSourceCrop(page: MangaPage, raw: PixelRect, previousSourceText = "") {
  const parsed = McpSourceRectPatchSchema.shape.sourceRect.safeParse(raw);
  if (!parsed.success || ![page.width, page.height].every((size) => Number.isSafeInteger(size) && size > 0))
  ...
  const x = Math.floor(sourceRect.x), y = Math.floor(sourceRect.y);
  const cropRect = { x, y, w: Math.ceil(sourceRect.x + sourceRect.w) - x, h: Math.ceil(sourceRect.y + sourceRect.h) - y };
  return { sourceRect, cropRect, previousSourceText };
}

export function buildMcpOcrObservation(selected: ReturnType<typeof selectMcpSourceCrop>, observation: Omit<Evidence, "engine">): McpBlockOcrObservation {
  const noTextDetected = !observation.recognizedText.trim();
  const parsed = McpBlockOcrObservationSchema.safeParse({
    ...selected, ...observation,
    differs: selected.previousSourceText !== observation.recognizedText,
    readingOrder: "app-crop-heuristic",
    warnings: ["review_before_apply", "source_evidence_retained", ...(observation.regions.length > 1 ? ["multiple_regions"] : []), ...(noTextDetected ? ["no_text_keep_existing"] : [])],
  });
  if (!parsed.success)
