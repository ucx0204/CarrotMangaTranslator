...
export class McpBlockOcrService {
  ...
  async run(target: McpBlockOcrTarget, context: McpOperationContext) {
    ...
    return {
      ...
      performed: ["block-ocr"],
      pagesChanged: 0,
      noTextDetected: !observation.recognizedText.trim(),
      needsReview: true,
      observationExpired: false,
      blockOcr: buildMcpOcrObservation(selected, observation),
    };
  }
...
export function selectMcpSourceCrop(
  page: MangaPage,
  raw: PixelRect,
  previousSourceText = "",
) {
...
export function buildMcpOcrObservation(
  selected: ReturnType<typeof selectMcpSourceCrop>,
  observation: Omit<Evidence, "engine">,
): McpBlockOcrObservation {
  ...
  const parsed = McpBlockOcrObservationSchema.safeParse({
    ...
    warnings: [
      ...
      ...(observation.regions.length > 1 ? ["multiple_regions"] : []),
