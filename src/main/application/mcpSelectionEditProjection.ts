...
export function projectMcpSelectionEdit(context: Context): SelectionProjection {
  ...
  const excluded = block.generatedLettering ? "generated_lettering" : item?.excludedReason;
  let after = structuredClone(block);
  let excludedReason = excluded ?? null;
  if (!excludedReason) {
    if (edit.kind === "references") after = projectReferences(context.saved, block, edit);
    else {
      const observation = observedText(context, block, edit.kind);
      excludedReason = observation.trim() ? null : "empty_observation_keep_existing";
      if (!excludedReason) {
        const fields = edit.kind === "source"
          ? { sourceText: observation } : { translatedText: observation };
        after = applyMcpBlockPatch(context.saved.chapter, page, [{ blockId: block.id, fields }])
          .blocks.find((candidate) => candidate.id === block.id) ?? after;
      }
    }
  }
  return { blockId: block.id, before: structuredClone(block), after,
    excludedReason, sourceRect: null, overlapBlockIds: [] };
}
