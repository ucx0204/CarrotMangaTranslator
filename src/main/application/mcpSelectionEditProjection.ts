...
export function projectMcpSelectionEdit(context: Context): SelectionProjection {
  ...
  const excludedReason = block.generatedLettering
    ? "generated_lettering"
...
function projectExisting(
  ...
  edit: Exclude<McpSelectionEdit, { kind: "append" }>,
) {
  ...
  if (!observation.trim())
    return {
      after: structuredClone(block),
      excludedReason: "empty_observation_keep_existing",
    };
  const fields =
    edit.kind === "source"
      ? { sourceText: observation }
      : { translatedText: observation };
  const after = applyMcpBlockPatch(context.saved.chapter, context.page, [
    { blockId: block.id, fields },
  ]).blocks.find((candidate) => candidate.id === block.id);
  if (!after)
    throw new McpEditError(
      "not_found",
      "Native field projection lost the selected block.",
    );
