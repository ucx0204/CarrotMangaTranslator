...
import {
  ...
  type McpWorkflowPage,
  type McpWorkflowOutcome,
...
export type McpWorkflowRuntime = {
  verify: (
    ...
    changedPage?: number,
  ) => Promise<McpWorkflowPage | undefined>;
  ...
  reconcile: (
    ...
    { page: McpWorkflowPage; outcome: McpWorkflowOutcome } | undefined
  >;
};
...
async function runStep(
  ...
  if (
    ...
    record.translationRequestsReserved + translationCost >
      record.input.maxTranslationRequests
  )
    throw new McpEditError(
      "invalid_edit",
      "Workflow attempt or translation-request budget is exhausted. No new model work started.",
    );
  guard();
  step.status = "running";
  ...
  try {
    ...
    if (!page || page.revision !== outcome.revision)
      throw new McpEditError(
        "revision_conflict",
        "Saved step outcome does not match the current page.",
      );
    ...
    if (recovered)
      ...
      step.status = "failed";
      step.errorCode = workflowError(error);
    }
    record.lastError = workflowError(error);
    record.status = active.controller.signal.aborted ? "cancelled" : "failed";
    await persistWorkflow(port, record);
...
async function reconcileWorkflowAttempt(
  ...
  for (const step of record.steps) {
    ...
    if (!retryFailed)
      throw new McpEditError(
        "invalid_edit",
        "An interrupted/failed attempt has no conclusive saved receipt. Inspect it and explicitly set retryFailed=true to permit another model request.",
      );
    await runtime.verify(record);
    step.status = "pending";
    step.errorCode = null;
...
function completeStep(
  record: McpWorkflowRecord,
  step: McpWorkflowStep,
  page: McpWorkflowPage,
  outcome: McpWorkflowOutcome,
) {
