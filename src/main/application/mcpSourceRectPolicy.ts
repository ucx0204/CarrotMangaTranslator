...
import { bboxToPixels, resolveBlockRenderBbox, resolveEffectiveRenderBbox } from "../../shared/geometry";
import { parseRichText } from "../../shared/richTextMarkup";
import { getActiveGeneratedLettering } from "../../shared/generatedLettering";
import { McpSourceRectPatchSchema, type McpSourceRectPatch, type McpSourceRectWarning } from "../../shared/mcpSourceRect";
...
export function applyMcpSourceRect(page: MangaPage, request: McpSourceRectPatch) {
  const bbox = checkedBbox(page, request.sourceRect);
  const selected = page.blocks.filter((block) => block.id === request.blockId);
  if (!selected.length) throw new McpEditError("not_found", "Source block not found.");
  if (selected.length !== 1) throw new McpEditError("invalid_edit", "The page has duplicate block IDs. Repair it before editing.");
  ...
  const next = changed ? {
    ...
    ...(pinned ? { renderBbox: existingRenderFrame(block, page), renderBboxSpace: "normalized_1000" as const } : {}),
  } : block;
  return {
    ...
    result: {
      ...
      sourceRect: bboxToPixels(changed ? bbox : previous, page.width, page.height),
...
function checkedBbox(page: MangaPage, value: BBox): BBox {
  ...
  if (!parsed.success || !Number.isSafeInteger(page.width) || !Number.isSafeInteger(page.height) || page.width <= 0 || page.height <= 0)
    throw new McpEditError("invalid_edit", "Source rectangle and page dimensions must be finite positive pixel bounds.");
  const rect = parsed.data;
  if (rect.x > page.width || rect.y > page.height || rect.w > page.width - rect.x || rect.h > page.height - rect.y)
    throw new McpEditError("invalid_edit", "Source rectangle must fit entirely inside the original page. It is never clipped silently.");
  const bbox = normalizeBboxTo1000(rect, page, "pixels");
  if (!sameBox(rect, bboxToPixels(bbox, page.width, page.height)))
    throw new McpEditError("invalid_edit", "Source rectangle is below the app's normalized geometry minimum. Enlarge it to at least 1/1000 of the page on each axis.");
  return bbox;
}
function sameBox(a: BBox, b: BBox) {
  return (["x", "y", "w", "h"] as const).every((key) => Math.abs(a[key] - b[key]) <= 1e-8);
}
function existingRenderFrame(block: TranslationBlock, page: MangaPage) {
  if (getActiveGeneratedLettering(block)) return resolveBlockRenderBbox(block, page);
  const text = parseRichText(block.translatedText || block.sourceText || "...", Boolean(block.bold), Boolean(block.italic)).plainText;
  return resolveEffectiveRenderBbox(block, page, text);
}
function retainedEvidenceWarnings(page: MangaPage, block: TranslationBlock): McpSourceRectWarning[] {
  const warnings: McpSourceRectWarning[] = ["source_text_not_rechecked"];
  if (page.inpaintedImagePath || page.inpaintMaskPath) warnings.push("erasure_and_masks_retained");
  if (block.generatedLettering) warnings.push("generated_lettering_retained");
  if (block.sourceFontFacePx !== undefined || block.fontSizeIntent === "source-match") warnings.push("source_font_metrics_retained");
