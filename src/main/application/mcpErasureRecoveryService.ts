...
type Action = { jobId: string; requestId: string; revision: PageRevision; direction: "undo" | "redo" };
type Receipt = Action & { status: "applied" | "already_applied"; pagesChanged: number; target: Target };
type Ports = {
  readJob: (id: string, owner: string) => Promise<Job>;
  inspect: (transactionId: string, target: Target) => Promise<View>;
  apply: (transactionId: string, target: Target, direction: "undo" | "redo", revision: PageRevision, guard: () => void) => Promise<{ revision: PageRevision }>;
...
export class McpErasureRecoveryService {
  ...
  async inspect(jobId: string, owner: string, authorize: () => void) {
    ...
    const view = target && eligible(job) && transactionId
      ? await this.ports.inspect(transactionId, target)
      : { state: "unavailable" as const,
          reason: !target ? "not_selected_erasure" : !eligible(job) ? "job_not_completed" : "history_unavailable" };
    this.check(authorize);
    return { jobId, target, ...view, sessionOnly: true as const,
  ...
  private async execute(action: Action, owner: string, authorize: () => void): Promise<Receipt> {
    ...
    if (!first.target) throw new McpEditError("invalid_edit", "Only a single-block erasure job can be recovered.");
    ...
    return this.ports.withPageEdit(target, guard, async (assertAuthorized) => {
      ...
      if (view.revision !== action.revision || view.state !== (action.direction === "undo" ? "applied" : "undone"))
        throw new McpEditError("revision_conflict", "Recovery is unavailable or the page changed. Inspect recovery again; subsequent edits are never overwritten.");
      const transactionId = this.references.get(action.jobId);
      if (!transactionId) throw new McpEditError("not_found", "Session image history is unavailable.");
      const saved = await this.ports.apply(transactionId, target, action.direction, action.revision, assertAuthorized);
