...
type Archive = Pick<
  Awaited<ReturnType<McpPageExportService["export"]>>,
...
type Ports = {
  ...
  ) => ReturnType<McpPageExportService["export"]>;
  archive: (
...
export class McpExportBatchService {
  ...
  async run(
    ...
    const pages: McpExportPageResult[] = assertMcpExportSelection(
      chapter,
      target,
    ).map((page) => ({ ...page, status: "unprocessed" }));
    ...
    for (const [index, page] of pages.entries()) {
      try {
        context.assertAuthorized();
        await assertShape();
        const artifact = await this.ports.exportPage(
          {
  ...
  private shapeGuard(target: McpExportPagesTarget, retainedAccess: () => void) {
    return async () => {
      ...
      if (
        ...
        mcpExportSnapshot(latest) !== target.snapshot
      )
  ...
  async zip(
    ...
    if (!pages.length || pages.some((page) => !page.url))
      throw new McpEditError(
        "not_found",
        "Source PNG files are unavailable after expiry or restart. Inspect the source job before a new export.",
      );
    const assertAccess = async () => {
      retainedAccess();
      assertMcpExportSelection(await this.ports.openChapter(data.chapterId), {
...
function exportFile(page: McpExportPageResult) {
  if (!page.url)
    throw new McpEditError("not_found", "Exported PNG is unavailable.");
