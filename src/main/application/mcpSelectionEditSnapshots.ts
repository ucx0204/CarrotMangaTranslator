...
function validateTransition(change: Change) {
  ...
  const before = change.beforeBlock,
    after = change.afterBlock;
...
function assertOrder(
  ...
  if (request.direction !== "undo" && appended.size)
    assertCompleteOrder(blocks, blockOrder);
}
function assertCompleteOrder(
  ...
  if (
    ...
    throw new McpEditError(
      "invalid_edit",
      "New discovery order must include each remaining block exactly once.",
    );
}
