...
export class McpContextResearchService {
  ...
  async run(
    ...
    context.assertAuthorized();
    const referenceSnapshot = await this.ports.verifyWork();
    ...
    if (!changes)
      return { ...base, status: "no_changes", ...researchUsage(researched) };
    const proposal = await this.ports.proposals.previewAppResearch(
      owner,
      changes.input,
      changes.evidence,
      researchWarnings(researched),
      context.assertAuthorized,
      { research: { target, referenceSnapshot, ...researchUsage(researched) } },
    );
    context.assertAuthorized();
    await this.ports.verifyWork();
    const result = McpContextResearchResultSchema.parse({
      ...proposal,
      ...researchUsage(researched),
    });
    return {
      ...base,
      status: "proposed",
...
function researchUsage(researched: WorkContextResearchProposal) {
  return {
...
function researchWarnings(researched: WorkContextResearchProposal) {
  const warnings = [
    ...researched.warnings,
