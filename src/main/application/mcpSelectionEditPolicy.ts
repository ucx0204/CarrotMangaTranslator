  ...
  return {
    ...
    plan: async (saved, input, access) => {
      ...
      const pages = input.pages.map((target) => {
        ...
        if (
          new Set(changes.map((change) => change.blockId)).size !==
          changes.length
        )
          throw new McpEditError(
            "invalid_edit",
    ...
    request: (page, input, direction, plan) => {
      ...
      if (!order)
        throw new McpEditError(
          "invalid_edit",
          "Planned reading order is missing.",
        );
      return {
        chapterId: input.chapterId,
        pageId: page.pageId,
        ...
        changes: structuredClone(
          page.changes.filter((change) => change.changed),
        ),
...
function planChange(
  ...
  prepared: Awaited<ReturnType<SelectionPlanningPorts["prepare"]>>,
): SelectionChange {
  const item =
    "itemId" in edit
      ? prepared.evidence?.items.find((entry) => entry.itemId === edit.itemId)
      : undefined;
