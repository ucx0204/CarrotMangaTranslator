...
import { assertMcpSelectionFreshness, type McpSelectionBinding } from "./mcpSelectionAnalysisService";
...
type Policy = BatchPolicy<McpSelectionBatchPreview, SelectionChange,
  SelectionSnapshotRequest, McpSelectionChangeView, SelectionPlan>;

export function createMcpSelectionEditPolicy(ports: SelectionPlanningPorts, now = Date.now): Policy {
  return {
...
function buildPlan(saved: McpContextSnapshot, input: McpSelectionBatchPreview, prepared: Prepared, now: number): SelectionPlan {
...
function selectedItem(edit: McpSelectionEdit, prepared: Prepared) {
  ...
  const evidence = prepared.evidence;
  if (!evidence) throw new McpEditError("not_found", "Owned analysis is unavailable.");
...
function planChange(saved: McpContextSnapshot, page: MangaPage, input: McpSelectionBatchPreview,
  edit: McpSelectionEdit, prepared: Prepared): SelectionChange {
  const projected = projectMcpSelectionEdit({ saved, page, edit,
    item: selectedItem(edit, prepared), requestId: input.requestId, defaults: prepared.defaults });
