...
import {
  projectMcpSelectionEdit,
  type SelectionProjection,
} from "./mcpSelectionEditProjection";
...
export type SelectionPlanningPorts = {
  prepare: (
    ...
    input: McpSelectionBatchPreview,
...
type Prepared = Awaited<ReturnType<SelectionPlanningPorts["prepare"]>>;
...
type Policy = BatchPolicy<
  McpSelectionBatchPreview,
  SelectionChange,
  SelectionSnapshotRequest,
  McpSelectionChangeView,
  SelectionPlan
>;

export function createMcpSelectionEditPolicy(
  ports: SelectionPlanningPorts,
  now = Date.now,
): Policy {
  return {
    ...
    plan: async (saved, input, access) => {
      ...
      access.guard();
      return buildPlan(saved, input, prepared, now());
    },
...
function buildPlan(
  ...
  if (input.command.kind === "analysis" && !evidence)
    throw new McpEditError(
      "not_found",
      "Completed owned analysis is required.",
    );
  if (evidence) {
    if (evidence.expiresAt <= now)
      throw new McpEditError(
        "not_found",
        "Selection evidence expired before preview.",
      );
  ...
  const pages = input.pages.map((target) => {
    ...
    const changes = target.edits.map((edit) =>
      planChange(saved, page, input, edit, prepared),
    );
    if (
      new Set(changes.map((change) => change.blockId)).size !== changes.length
    )
      throw new McpEditError(
        "invalid_edit",
        "Change each block or discovery at most once per page.",
      );
  ...
  pages.sort(
    (a, b) =>
      saved.chapter.pages.findIndex((p) => p.id === a.pageId) -
  ...
  return {
    ...
    binding: structuredClone(
      evidence?.binding ?? {
        ...
        membership: mcpBatchMembership(saved.chapter),
        pages: input.pages.map(({ pageId, revision }) => ({
          pageId,
          revision,
        })),
      },
...
const snapshotRequest: Policy["request"] = (page, input, direction, plan) => {
  const order = plan.orders.find((entry) => entry.pageId === page.pageId);
  if (!order)
    throw new McpEditError("invalid_edit", "Planned reading order is missing.");
  return {
    chapterId: input.chapterId,
    pageId: page.pageId,
    revision: page.expectedRevision,
    direction,
    changes: structuredClone(page.changes.filter((change) => change.changed)),
    expectedOrder: structuredClone(
      direction === "undo" ? order.after : order.before,
    ),
    blockOrder: structuredClone(
      direction === "undo" ? order.before : order.after,
    ),
    evidenceExpiresAt: plan.evidenceExpiresAt,
    binding: {
      ...plan.binding,
      pages: plan.binding.pages.map((dependency) => ({
        ...dependency,
        revision:
          plan.pages.find((target) => target.pageId === dependency.pageId)
...
function validateInput(
  ...
  validateBatchTargets(saved, {
    ...input,
    pages: input.pages.map((page) => ({ ...page, edits: [] })),
  });
  if (input.pages.reduce((count, page) => count + page.edits.length, 0) > 100)
    throw new McpEditError(
      "invalid_edit",
      "At most 100 selected changes are allowed.",
    );
  for (const page of input.pages) {
    if (
      page.edits.some(
        (edit) =>
          (edit.kind === "references") !==
          (input.command.kind === "references"),
      )
    )
      throw new McpEditError(
        "invalid_edit",
        "Analysis application and reference editing are separate commands.",
      );
  }
}
function selectedItem(edit: McpSelectionEdit, prepared: Prepared) {
  if (edit.kind === "references") return undefined;
  const evidence = prepared.evidence;
  if (!evidence)
    throw new McpEditError("not_found", "Owned analysis is unavailable.");
  const item = evidence.items.find((entry) => entry.itemId === edit.itemId);
  if (!item)
    throw new McpEditError(
      "not_found",
      "Selected item is absent from owned analysis.",
    );
  const expected = edit.kind === "translation" ? "translation" : "ocr";
  if (evidence.kind !== expected)
    throw new McpEditError(
      "invalid_edit",
      "Analysis kind does not match the requested edit.",
    );
  return item;
}
function planChange(
  ...
  const projected = projectMcpSelectionEdit({
    saved,
    page,
    edit,
    item: selectedItem(edit, prepared),
    requestId: input.requestId,
    defaults: prepared.defaults,
  });
  const changed =
    !projected.excludedReason &&
    hashStableValue(projected.before) !== hashStableValue(projected.after);
  return {
    pageId: page.id,
    ...
    after: textState(
      projected.excludedReason ? projected.before : projected.after,
    ),
    sourceRect: projected.sourceRect,
    overlapBlockIds: projected.overlapBlockIds,
    excludedReason: projected.excludedReason,
    changed,
    warnings: [
      "no_implicit_models_or_rendering",
      "verify_rendering_after_application",
      ...(edit.kind === "source"
        ? ["retained_source_measurements_require_review"]
        : []),
    ],
...
function planOrder(page: MangaPage, changes: SelectionChange[]): PageOrder {
  const appends = changes.filter(
    (change) => change.changed && change.requested.kind === "append",
  );
  ...
  for (const change of appends) {
    ...
    if (anchor === undefined) {
      after.push(change.blockId);
      continue;
    }
    if (anchor !== null && !page.blocks.some((block) => block.id === anchor))
      throw new McpEditError(
        "not_found",
        "Insertion anchor must be an existing block on this page.",
      );
    const resolved = anchors.get(anchor) ?? anchor;
    after.splice(
      resolved === null ? 0 : after.indexOf(resolved) + 1,
      0,
      change.blockId,
    );
...
function describePage(
  target: { pageId: string; revision: string },
  changes: SelectionChange[],
): BatchPage<SelectionChange> {
