...
import type { ImageRedactionPage, ImageRedactionStroke } from "../shared/imageRedaction";
import { imageFingerprint } from "./imageRedactionContext";

type SavedPages = Record<string, { fingerprint: string; strokes: ImageRedactionStroke[] }>;
...
export async function prepareImageRedactionPages(
  pages: readonly MangaPage[], saved: SavedPages, signal?: AbortSignal,
  ...
  for (let offset = 0; offset < pages.length; offset += 4) {
    ...
    const chunk = await Promise.all(pages.slice(offset, offset + 4).map(async (page) => {
      ...
      return {
        id: page.id, name: page.name, imagePath: page.imagePath,
        width: page.width, height: page.height, fingerprint,
        strokes: saved[page.imagePath]?.fingerprint === fingerprint ? saved[page.imagePath].strokes : [],
      };
    }));
    prepared.push(...chunk);
  }
