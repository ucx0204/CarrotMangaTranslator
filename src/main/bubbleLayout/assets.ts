...
export async function ensureComicBubbleDetectorAssets(options: {
  ...
  const [modelPath, wasmBinaryPath] = await Promise.all([
    ensureRemoteFile({
      ...
      expectedTotalBytes: COMIC_BUBBLE_DETECTOR_BYTES,
      maximumBytes: COMIC_BUBBLE_DETECTOR_BYTES,
    ...
    ensureRemoteFile({
      ...
      expectedTotalBytes: ONNXRUNTIME_WEB_WASM_BINARY_BYTES,
      maximumBytes: ONNXRUNTIME_WEB_WASM_BINARY_BYTES,
      progressPhase: "bubble_layout_preparing",
      signal: options.signal,
      onProgress: options.onProgress,
    }),
