...
export function RedactionWorkspaceHeader({
  form,
  onOpen,
  onHelp,
}: {
  ...
  return (
    <div className={styles.headerTools}>
      <Tabs
        ...
        value={view.mode}
        onChange={(mode) => {
          if (!form.busy && !form.drawing)
            commit((current) => changeRedactionView(current, { mode }));
        }}
      />
      <Select
        ...
        options={(
          ["all", "unreviewed", "deferred", "masked", "error"] as const
        ).map((value) => ({
          value,
      ...
      <Button size="sm" onClick={onHelp} disabled={form.busy || form.drawing}>
