...
export function RedactionPresetsDialog(props: Props): React.JSX.Element {
  ...
  return (
    <Modal
      title={t("manualRedaction.presets")}
      size="md"
          ...
          actions={
            <>
              <Button onClick={props.onClose}>
                {t("manualRedaction.close")}
              </Button>
              <Button
                variant="primary"
                disabled={!model.preset}
                onClick={() => {
                  if (model.preset) props.onApply(model.preset);
                }}
              >
    ...
    >
      ...
      <Select
        ariaLabel={t("manualRedaction.savedPresets")}
        value={model.selected}
        disabled={!model.presets.length}
        options={model.presets.map((item) => ({
          value: item.id,
          label: item.name,
        }))}
        onValueChange={(value) => {
...
function usePresetModel({ form, source }: Props) {
  ...
  const canSave = Boolean(
    name.trim() && source.strokes.length && presets.length < 30,
  );
  const save = () => {
    if (!canSave) return;
...
function PresetSaveControls({
  ...
  return (
    <>
      ...
      <ControlTooltip
        content={t("manualRedaction.presetHint")}
        placement="bottom"
      >
        <Button onClick={model.save} disabled={!model.canSave}>
          {t("manualRedaction.savePreset")}
        </Button>
      </ControlTooltip>
    </>
  );
  ...
  return (
    <div className={styles.previewPanel}>
      <span>
        ...
        {t("manualRedaction.strokeCount", { count: preset.strokes.length })}
      </span>
      <div
      ...
      <Button variant={deleting ? "danger" : "secondary"} onClick={remove}>
        {t(
          deleting
            ? "manualRedaction.confirmDeletePreset"
            : "manualRedaction.deletePreset",
        )}
      </Button>
    </div>
  );
