...
import { dismissToast, getToasts, subscribeToasts, type Toast } from "../../lib/toastStore";
import { CheckCircleIcon, CloseIcon, InfoIcon, WarnIcon } from "./icons";

function ToastGlyph({ variant }: { variant: Toast["variant"] }): React.JSX.Element {
  if (variant === "success") {
...
function ToastCard({ toast }: { toast: Toast }): React.JSX.Element {
  return (
    <div className={`toast ${toast.variant}`} role="status">
      ...
      <button type="button" className="toast-close" aria-label="알림 닫기" onClick={() => dismissToast(toast.id)}>
        <CloseIcon size={14} />
      </button>
    </div>
  );
