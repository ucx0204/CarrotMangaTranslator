...
- 현재 페이지 또는 전체 화의 번역문/OCR 원문을 `텍스트 모아보기`로 확인하고 복사하거나 TXT/CSV/TSV 검수표로 내보냅니다.
...
- 자주 쓰는 보기/번역/인페인팅/블록 편집 단축키를 설정에서 바꿀 수 있습니다.

## v0.8.1에서 달라진 점

v0.8.1은 v0.8.0 이후 텍스트 편집, 확대/축소 읽기, PNG 출력 일치도를 보강한 패치 릴리즈입니다.

- 번역 블록 텍스트에서 `**굵게**`, `*기울임*` 같은 간단한 마크다운 서식을 사용할 수 있습니다.
- 장평 조절을 추가해 글자를 좌우로 살짝 좁히거나 넓힐 수 있고, 기본 서식/서식 일괄 적용에도 반영됩니다.
- 현재 화 편집 내용에 실행 취소 단축키를 추가해 블록 이동, 크기 조절, 텍스트/서식 변경을 되돌리기 쉬워졌습니다.
- 읽기 영역 확대/축소 단축키를 추가하고, 텐키 `+`도 확대 단축키로 동작하도록 보강했습니다.
- 확대된 페이지에서 마우스 휠을 돌리면 먼저 이미지 안을 스크롤하고, 맨 위/아래에 닿은 뒤에야 이전/다음 페이지로 넘어갑니다.
- PNG 출력은 UI의 텍스트 줄바꿈, 장평, 줄 간격, 굵게/기울임, 외곽선을 더 정확히 따라가며, 긴 페이지 하단이 잘리는 문제를 수정했습니다.
- 설정의 설치/확인 화면에서 현재 버전을 확인하고 GitHub Releases를 열 수 있습니다. 새 설치 파일을 덮어 설치해도 모델, 보관함, OCR 런타임 데이터는 유지됩니다.

자세한 패치노트는 [docs/release-notes/v0.8.1.md](docs/release-notes/v0.8.1.md)를 확인하세요.
...
- 설치 파일 예시: `CarrotMangaTranslator-Setup-v0.8.1.exe`
...
설정 창에는 엔진 설정 외에도 `하드웨어 · OCR`, `기본 서식`, `단축키` 탭이 있습니다.

- `하드웨어 · OCR`: Paddle OCR 장치와 품질 모드를 고릅니다. `최소`/`절약`은 CPU 기반 PP-OCRv6 경로로 저사양 PC에서 더 가볍게 돌리고, `풀로드`는 PaddleOCR-VL GPU 경로를 사용합니다.
- `기본 서식`: 새로 만들어지는 번역 블록의 방향, 정렬, 폰트, 글자 크기, 줄 간격, 자간, 글자색, 외곽선을 미리 정합니다. 이미 있는 블록은 자동으로 바뀌지 않습니다.
- `단축키`: 보기 전환, 번역 시작, 텍스트 모아보기, 인페인팅, 블록 복제/삭제 같은 단축키를 원하는 조합으로 바꾸거나 비울 수 있습니다. 같은 조합을 다른 기능에 지정하면 기존 기능의 단축키는 자동으로 해제됩니다.
...
![기본 서식 설정](docs/images/43-settings-format-defaults.png)

`기본 서식`은 이후 AI 번역으로 새로 만들어지는 블록에 적용됩니다. 기존 블록을 한꺼번에 바꾸고 싶다면 블록을 선택한 뒤 오른쪽 블록 패널의 일괄 적용 기능을 사용합니다.
...
가져온 뒤에도 왼쪽 보관함의 화 목록과 페이지 목록에서 드래그 핸들을 잡아 순서를 바꿀 수 있습니다. 순서 저장에 실패하면 이전 순서로 되돌리고 상태 메시지에 원인을 표시합니다.
...
화 카드의 `번역` 버튼을 누르면 바로 작업이 시작되지 않고, 번역 범위와 대상을 고르는 모달이 먼저 열립니다. 이전처럼 `이어서 번역`과 `전체 다시 번역` 버튼이 화면에 따로 놓인 구조가 아니라, 하나의 `번역` 버튼 안에서 필요한 방식을 선택하는 흐름입니다.

![번역 시작 모달](docs/images/39-translation-start-modal.png)

- `번역 범위`: `이어서`는 이미 끝난 페이지를 최대한 유지하며 남은 분량을 이어서 처리하고, `전체 다시`는 선택한 대상을 처음부터 다시 번역합니다.
- `대상`: `현재 화`만 처리할지, 같은 작품의 `작품 전체` 화를 함께 처리할지 고릅니다.
...
처음 2차 번역을 켜면 작품 전체를 훑으며 용어/기억을 채우는 시간이 추가될 수 있습니다. 빠르게 대충 읽는 것이 목적이거나 API 비용이 부담된다면 끄고, 공유용 결과물처럼 번역 일관성과 문체를 더 챙기고 싶다면 켜는 쪽을 권장합니다.
...
- `표시`: `한국어+OCR`, `한국어만`, `OCR만` 중 필요한 텍스트를 고릅니다.
