...
v0.9.0은 v0.8.4 이후 번역 범위 선택, 재번역, 편집 작업 흐름, 텍스트 검수 흐름을 크게 다듬은 릴리즈입니다.

- 번역 시작 모달에서 작품 안의 화와 페이지를 직접 펼쳐 고를 수 있습니다. `전체 선택`, `미번역만`, `전체 해제`로 빠르게 범위를 잡고, 필요한 페이지 썸네일만 체크해 부분 번역할 수 있습니다.
- `기존 블록 유지` 번역 모드를 추가했습니다. 이미 손본 말풍선 위치, 크기, 방향, 폰트, 색상, 외곽선 같은 서식은 그대로 두고 각 블록 안의 원문/번역문만 다시 채웁니다.
- 현재 페이지만 다시 처리하는 `페이지 재번역` 흐름을 추가했습니다. 페이지 재번역에서도 `자동 생성`과 `기존 블록 유지`를 고를 수 있습니다.
- 작업 영역 왼쪽에 `선택`, `블록`, `손바닥` 도구 모음을 추가했습니다. `블록` 도구로 빈 영역을 드래그하면 새 텍스트 블록을 직접 만들 수 있고, `손바닥` 도구로 확대된 페이지를 끌어 이동할 수 있습니다.
- 블록 편집기를 오른쪽 패널에서 떼어 앱 안의 떠 있는 패널로 크게 보거나, 별도 OS 창으로 분리할 수 있습니다. 분리한 창의 위치와 크기는 다음 실행에도 최대한 유지됩니다.
- `텍스트 모아보기`에 검색, 페이지 머리말 제외, `.txt 불러오기`를 보강했습니다. 한국어만 TXT로 저장한 뒤 외부 편집기에서 고친 내용을 줄 순서 기준으로 다시 불러올 수 있습니다.
- 선택한 페이지만 실행 중으로 표시되도록 번역 진행 상태를 정리하고, 작업 중 들어오는 라이브 갱신이 사용자가 방금 고친 완료 페이지를 덮어쓰지 않도록 보강했습니다.
- 검수표 가져오기와 TXT 불러오기에서 없는 block id, 줄 수 불일치, OCR 원문 불일치 같은 문제를 경고로 알려주면서 적용 가능한 내용은 계속 반영하도록 안정성을 높였습니다.

자세한 패치노트는 [docs/release-notes/v0.9.0.md](docs/release-notes/v0.9.0.md)를 확인하세요.
...
- 설치 파일 예시: `당근망가번역기 Setup 0.9.0.exe`
