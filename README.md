...
<p align="center">
  <strong>만화 이미지를 넣고 → 번역하고 → 말풍선을 다듬고 → 완성 PNG로 저장하는 데스크톱 앱</strong>
</p>
...
당근망가번역기는 이미지 속 대사와 효과음을 찾고, AI 번역 블록을 만든 뒤, 사람이 문장과 배치를 다듬어 완성 PNG로 내보내는 만화 작업 도구입니다. 이미지 한 장부터 폴더, ZIP, CBZ까지 불러올 수 있습니다.

> **이 설명서의 데모 줄거리**
>
> 일본어는 할 줄 아는 주인공이 악역영애로 빙의했습니다. 그런데 이세계 공용어는 일본어가 아닙니다. 한마디도 못 알아들은 주인공은 일단 웃고 끄덕이기 시작하고, 그때마다 수상한 약속이 하나씩 늘어납니다. 설정은 여기까지입니다. 이제 앱을 배워봅시다.

- 다운로드: [GitHub Releases](https://github.com/ucx0204/CarrotMangaTranslator/releases)
- 현재 소스 버전: `v1.11.1`
- 패치노트: [v1.11.1](docs/release-notes/v1.11.1.md)
...
| **정밀 2차**       | 1차 결과를 다시 분석하고 재번역합니다.                  | 시간과 사용량보다 일관성이 중요할 때 |
...
메인 화면의 출력 메뉴에서 다음 중 하나를 고릅니다.

- **현재 페이지 PNG**: 지금 보고 있는 한 장만 저장합니다.
- **전체 화 PNG**: 현재 화의 모든 페이지를 저장합니다.

PNG에는 블록 위치, 방향, 폰트, 색, 외곽선과 회전이 반영됩니다. 출력 전에는 원본 미리보기와 블록 표시를 번갈아 보면서 빠진 말풍선이 없는지 확인하세요.
...
`+ TTF/OTF 폰트 등록`으로 폰트를 추가할 수 있습니다. 등록한 폰트는 화면 미리보기와 PNG 출력에 함께 사용됩니다.
...
`*.mgtshare`는 완성 PNG가 아니라 **다른 컴퓨터에서 다시 편집할 수 있는 작업 파일**입니다.
