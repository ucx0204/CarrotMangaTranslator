...
v0.8.0은 v0.7.1 이후 번역 품질, 검수 흐름, 블록 서식, 인페인팅 선택지를 크게 보강한 릴리즈입니다.

- `용어/기억`과 2차 번역이 더 적극적으로 연결되어 OCR 오독, 이상한 말풍선 분리, 이전 번역 실수를 다시 검토합니다.
- 영역 번역은 선택 crop만 보는 것이 아니라 원본 전체 페이지, 선택 영역 안 OCR 후보, 작품 용어/캐릭터/스토리 기억을 함께 참고합니다.
- 텍스트 모아보기에서 CSV/TSV 검수표를 내보내고 다시 가져와 번역문, 검수 상태, 메모를 일괄 반영할 수 있습니다.
- 설정에 새 블록 기본 서식과 단축키 탭이 추가됐고, 블록 편집에는 서식 일괄 적용이 추가됐습니다.
- 인페인팅 모델에 가벼운 Koharu AOT/LaMa 선택지가 추가되어 Flux가 부담스러운 PC에서도 원문 제거를 시도하기 쉬워졌습니다.
- Paddle OCR 저사양 모드, OCR 캐시/노이즈 처리, CPU 워커 제한, RTX 50/AMD 관련 런타임 처리가 보강됐습니다.

자세한 패치노트는 [docs/release-notes/v0.8.0.md](docs/release-notes/v0.8.0.md)를 확인하세요.
...
- 설치 파일 예시: `CarrotMangaTranslator-Setup-v0.8.0.exe`
