...
v0.9.2는 v0.9.1 이후 CPU OCR 품질 조합, GPU OCR CPU 폴백, 선택 영역 번역 저장 흐름을 다듬은 패치 릴리즈입니다.

- CPU OCR에서는 너무 느린 `풀로드` PaddleOCR-VL 품질을 쓰지 않도록, 설정 저장값과 런타임 옵션을 자동으로 `절약` 품질로 보정합니다.
- GPU OCR 실패 후 CPU로 폴백할 때 VL 레이아웃 파이프라인 대신 PP-OCRv6 텍스트라인 OCR 경로로 내려가, CPU 폴백이 사실상 멈춘 것처럼 느려지는 일을 줄였습니다.
- 선택 영역 번역 결과를 최신 페이지 상태에 새 블록으로 append하도록 바꿔, 같은 페이지를 편집하던 중에도 저장 충돌로 영역 번역 결과가 사라질 가능성을 줄였습니다.
- dirty 페이지를 라이브 갱신으로 보존할 때도 영역 번역이 만든 새 블록만 안전하게 덧붙입니다.
- 영역 번역 모드에서는 사용자가 직접 고른 영역의 짧은 텍스트나 UI 목록형 결과가 전체 페이지용 잡음 필터에 의해 버려지지 않도록 조정했습니다.
- README FAQ와 설정 설명에 CPU OCR과 풀로드 품질 조합 제한을 추가했습니다.

자세한 패치노트는 [docs/release-notes/v0.9.2.md](docs/release-notes/v0.9.2.md)를 확인하세요.
...
- 설치 파일 예시: `당근망가번역기 Setup 0.9.2.exe`
