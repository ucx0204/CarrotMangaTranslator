...
- `Codex 모델`: 현재 표시 카탈로그의 GPT-5.6-Sol/Terra/Luna, GPT-5.5, GPT-5.4 계열, GPT-5.3-Codex-Spark 중에서 고릅니다. 계정별 배포 차이로 목록에 없는 모델은 맨 아래 `Custom (직접 입력)…`에 모델 ID를 넣을 수 있습니다. `생각` 단계도 선택한 모델이 지원하는 값만 표시하며, GPT-5.6-Sol/Terra는 `ultra`, GPT-5.6-Luna는 `max`까지 선택할 수 있습니다.
