...
`Gemma 4`는 내 PC에서 로컬 모델을 직접 실행합니다. 인터넷 없이 로컬에서 처리하고 싶거나 OpenAI 계정을 쓰지 않으려는 경우에 적합하지만, NVIDIA RTX GPU와 충분한 VRAM이 필요합니다.

![Gemma 4 설정](docs/images/05-settings-gemma.png)

- 모델 소스는 기본 Hugging Face repo 또는 직접 받은 GGUF 파일을 선택할 수 있습니다.
- 모델 프리셋은 대략 `Q3` 16GB, `Q4` 24GB, `Q6` 32GB VRAM 기준으로 생각하면 됩니다.
- `GPU Layers`는 GPU에 올릴 레이어 수입니다. VRAM이 부족하면 값을 낮춰 안정성을 높입니다.
...
- 기본 모델은 `gpt-5.5`입니다.
- `생각` 단계는 모델이 답을 만들기 전에 더 깊게 검토할 정도를 조절합니다. 기본값은 `보통`입니다.
