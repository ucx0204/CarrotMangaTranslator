...
- 현재 소스 버전: `v1.12.0`
- 패치노트: [v1.12.0](docs/release-notes/v1.12.0.md)
...
한국어 번역에서 `설정 → 번역 → 폰트 자동 맞춤`을 켜면 원문의 굵기와 분위기를 보고 21개 한국어 후보 중 블록에 어울리는 폰트를 고릅니다. v1.12.0의 v2 런타임은 일반 대사와 강조 계열을 분리해 평가하고, 본문에 개성체 `Single Day`가 잘못 들어가지 않도록 별도 안전 조건을 적용합니다. 사용자가 직접 고정한 폰트와 외곽선·색은 덮어쓰지 않으며, 자동 선택은 `자동 폰트 선택 되돌리기`로 되돌릴 수 있습니다.
