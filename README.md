...
데이터를 로컬에서 처리해야 한다면 PC 사양에 맞는 Gemma 프리셋을 사용합니다. `모델 계열`은 `속도(추천)`가 기본이며, 전용 VRAM이 16GB 이상이면 24GB 이상인 경우도 `26B`를 기본으로 선택합니다. `31B`는 원하는 사용자가 직접 선택할 수 있습니다. 처리 속도나 메모리 제약이 크다면 Codex 또는 API 엔진으로 전환할 수 있습니다.
...
버튼을 누르는 대신 파일 탐색기에서 여러 이미지, 이미지 폴더 하나, ZIP/CBZ/RAR/CBR 하나 또는 PDF 하나를 앱 어디에나 끌어 놓아도 됩니다. 폴더·압축파일·PDF는 다른 항목과 섞지 말고 하나씩 놓아 주세요.
...
지원하는 형식은 PNG, JPG, JPEG, WEBP, ZIP, CBZ, RAR, CBR, PDF입니다. 폴더와 압축파일 안의 이미지는 파일명 기준으로 자연 정렬되며, PDF는 페이지 순서대로 PNG로 변환됩니다.
