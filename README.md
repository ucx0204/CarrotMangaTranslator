...
기본적으로 일본어 만화를 한국어로 번역하며, 설정에서 다른 원문·번역 언어쌍도 선택할 수 있는 **Windows 데스크톱 만화 번역 도구**입니다. 번역 블록 편집, 원문 지우기, PNG 출력까지 한 앱에서 처리합니다.
...
3. `Codex 모델` 드롭다운과 모델별 `생각` 단계를 고릅니다. 목록에 없는 모델은 맨 아래 `Custom (직접 입력)…`을 사용하고, 잘 모르면 기본값을 둡니다.
...
- `Codex 모델`: 현재 표시 카탈로그의 GPT-5.6-Sol/Terra/Luna, GPT-5.5, GPT-5.4 계열, GPT-5.3-Codex-Spark 중에서 고릅니다. 계정별 배포 차이로 목록에 없는 모델은 맨 아래 `Custom (직접 입력)…`에 모델 ID를 넣을 수 있습니다. 목록 모델은 지원하는 `생각` 단계만 표시하며, Custom 모델은 모든 단계를 표시합니다. GPT-5.6-Sol/Terra는 `ultra`, GPT-5.6-Luna는 `max`까지 선택할 수 있습니다.
...
번역 작업은 보통 OCR 선분석이 먼저 돌고, 그 결과를 바탕으로 AI 번역 요청이 이어집니다. 오른쪽 작업 카드에서 현재 몇 번째 페이지가 OCR 중인지 볼 수 있습니다. 한 번 OCR이 끝난 페이지는 캐시를 재사용하고, 원문 언어가 일본어일 때 OCR에서 일본어 근거가 거의 없는 페이지는 불필요한 AI 호출을 줄입니다.
...
`캐릭터` 탭에서는 표시 이름, 원문 이름, 번역 이름, 말투, 커스텀 말투, 메모를 관리합니다. 존댓말/반말 같은 기본 말투를 고르거나 직접 말투 지침을 적어두면 같은 인물이 여러 페이지에서 더 일관되게 번역됩니다. 수정한 내용은 `저장`을 눌러 보관함에 반영합니다.
...
텍스트는 페이지별로 묶이고, 같은 페이지 안에서는 원문 언어의 읽기 방향에 맞춰 정렬됩니다. 일본어와 RTL 언어는 같은 줄에서 오른쪽에서 왼쪽으로, 그 밖의 언어는 왼쪽에서 오른쪽으로 읽으며 위에서 아래 순서를 유지합니다. 대사 확인, 맞춤법 검토, 외부 번역 검수, OCR 원문 백업이 필요할 때 쓰면 됩니다.
...
원문 언어가 일본어이면 Paddle OCR에서 일본어 텍스트 근거가 없을 때 모델 호출을 생략합니다. 다른 언어쌍은 OCR 오탐으로 텍스트를 놓치는 일을 줄이기 위해 이 생략 최적화를 사용하지 않습니다. 일본어 원문인데도 블록이 생긴다면 해당 페이지가 예전 버전에서 분석된 결과일 수 있으니 `전체 다시 번역`으로 새로 분석해 보세요.
