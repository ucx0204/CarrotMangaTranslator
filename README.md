...
v0.9.1은 v0.9.0 이후 AMD OCR GPU 안정성과 실패 복구를 보강한 패치 릴리즈입니다.

- AMD GPU OCR이 실패하면 남은 페이지를 CPU OCR로 자동 재시도하고, 같은 세션에서는 CPU로 계속 진행해 번역 작업이 중간에 멈출 가능성을 줄였습니다.
- Windows ROCm PyTorch가 지원하지 않는 AMD GPU에서는 OCR GPU 기본값을 켜지 않도록 제한했습니다. RX 7700~7900, RX 9000, Radeon PRO V710 계열처럼 지원 범위에 있는 GPU만 ROCm OCR 기본값을 받습니다.
- GPU OCR 시작 전에 Flux/Koharu 인페인팅 엔진 캐시를 자동 해제해 VRAM 부족 가능성을 줄였습니다.
- PaddleOCR-VL 배치 처리에서 페이지별 OOM을 한 번 정리 후 재시도하고, 연속 실패가 이어지면 CPU 폴백으로 넘어가도록 보강했습니다.
- 여러 HIP/CUDA 장치가 보일 때 가장 VRAM이 큰 GPU를 우선 선택해, 내장 GPU가 먼저 잡히는 문제를 줄였습니다.
- AMD OCR 실패 메시지와 README FAQ에 VRAM 부족, iGPU 동시 인식, Windows TDR, 지원 GPU 범위 안내를 추가했습니다.

자세한 패치노트는 [docs/release-notes/v0.9.1.md](docs/release-notes/v0.9.1.md)를 확인하세요.
...
- 설치 파일 예시: `당근망가번역기 Setup 0.9.1.exe`
...
- `.txt 불러오기`: `한국어만` 형식으로 저장한 TXT를 다시 읽어, 줄 순서 기준으로 번역문만 업데이트합니다.
...
TXT 불러오기는 가벼운 문장 교정용입니다. `한국어만`으로 저장한 파일을 외부 편집기에서 고친 뒤 다시 불러오면, OCR 원문과 블록 위치는 그대로 두고 번역문만 바뀝니다. 페이지별 줄 수가 현재 블록 수와 맞지 않거나, 머리말이 현재 페이지와 맞지 않으면 해당 페이지는 건너뛰고 경고를 표시합니다.
