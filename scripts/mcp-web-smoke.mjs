...
async function main() {
  ...
  const connection = JSON.parse(
    await readFile(join(root, ".tmp/mcp-web-connection.json"), "utf8"),
  );
  ...
  const password = (
    await readFile(join(root, ".tmp/mcp-web-password"), "utf8")
  ).trim();
  if (!/^[A-Za-z0-9_-]{43,128}$/.test(password))
    throw new Error("Invalid connection password file.");
...
main().catch(() => {
  ...
  console.error(
    "FAIL web OAuth diagnostic. Confirm mcp-web.cjs is READY and these files belong to that running instance. See docs/mcp-web-testing.md and the last PASS stage. Do not share the password or token files.",
  );
  process.exitCode = 1;
});
