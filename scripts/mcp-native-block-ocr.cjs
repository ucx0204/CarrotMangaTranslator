  ...
  return {
    ...
    runtime: {
      ...
      collect: async (options) => {
        ...
        const image = nativeImage.createFromBuffer(await readFile(options.imagePath));
        assert.deepEqual(image.getSize(), { width: 130, height: 115 });
        const offset = (45 * 130 + 33) * 4;
        assert.deepEqual([...image.toBitmap().subarray(offset, offset + 3)], [0, 0, 0], "OCR must use original text, not the erased raster");
        assert.equal(options.skipOcrBboxHints, false);
        assert.doesNotMatch(options.outputDir, /ocr-hints/);
        return {
          hints: [{ x1: 20, y1: 20, x2: 100, y2: 90, ocrText: "原文再読 ABC 🥕", direction: "vertical" }],
          diagnostics: [],
        };
      },
      release: async () => { state.releases++; return true; },
    },
  };
}
/** @param {Content[]} value */
function metadata(value) {
  ...
  assert.doesNotMatch(JSON.stringify(value), /resource_link|dataUrl|mcp-artifacts/);
...
async function checkNativeBlockOcr(root, app, invoke, chapterId, pageId) {
  ...
  const { McpOperationService } = require(join(root, "out/main/application/mcpOperationService.js"));
  const { createMcpOperationTools } = require(join(root, "out/main/mcp/mcpOperationTools.js"));
  const { createMcpBlockOcrExecutor } = require(join(root, "out/main/mcp/mcpBlockOcrSession.js"));
  ...
  const tools = createMcpOperationTools(operations, { blockOcr: createMcpBlockOcrExecutor(app, observationRuntime.runtime) });
  /** @type {Invoke} */
  ...
  try {
    ...
    const request = { ...target, blockId: page.blocks[0].id, revision: createPageRevision(page), requestId: randomUUID() };
    ...
    assert.equal(job.result.blockOcr.previousSourceText, page.blocks[0].sourceText);
    assert.equal(job.result.blockOcr.recognizedText, "原文再読 ABC 🥕");
    assert.equal(job.result.revision, request.revision);
    assert.deepEqual(await library.openChapter(chapterId), before, "Observation must not save chapter metadata or blocks");
    assert.equal(metadata(await call("carrot_run_block_ocr", request)).jobId, first.jobId);
    ...
    const edit = { ...target, revision: job.result.revision, edits: [{ blockId: request.blockId, fields: { sourceText: job.result.blockOcr.recognizedText } }] };
    ...
    assert.deepEqual(saved.blocks, [{ ...page.blocks[0], sourceText: job.result.blockOcr.recognizedText }, ...page.blocks.slice(1)]);
    assert.deepEqual(await raster(invoke, target), rendered);
    await invoke("carrot_update_page_blocks", { ...edit, revision: applied.revision, edits: [{ blockId: request.blockId, fields: { sourceText: job.result.blockOcr.previousSourceText } }] });
    assert.deepEqual((await library.openChapter(chapterId)).pages[0].blocks, page.blocks);
    assert.deepEqual(await readFile(page.imagePath), original);
    assert.deepEqual(await readFile(page.inpaintedImagePath), inpainted);
    assert.deepEqual(await raster(invoke, target), rendered);
    assert.equal(observationRuntime.state.calls, 1, "Apply and restoration must not rerun OCR");
    assert.equal(app.jobs.gate.activities.length, 0);
    console.log("PASS native block OCR: original crop after erasure, unchanged observation snapshot, one inference/release, duplicate receipt, source-only apply/restore and identical rendering");
