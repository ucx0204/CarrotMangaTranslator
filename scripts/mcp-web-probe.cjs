...
async function runMcpWebProbe(
  ...
  requireCondition(
    ...
    "Unauthenticated MCP request must return an OAuth challenge.",
  );
  const metadata = await send("/.well-known/oauth-protected-resource/mcp").then(
    readJson,
  );
  requireCondition(
    metadata.resource === resource &&
      metadata.authorization_servers?.[0] === expectedOrigin,
    "Protected-resource metadata does not match this endpoint.",
  );
  const auth = await send("/.well-known/oauth-authorization-server").then(
    readJson,
  );
  for (const [name, path] of Object.entries({
    ...
    requireCondition(
      auth[name] === `${expectedOrigin}${path}`,
      "Unexpected OAuth endpoint. No credentials were sent.",
    );
  console.log(
    "PASS public OAuth discovery and unauthenticated access rejection",
  );
  const client = await send("/oauth/register", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      client_name: "Carrot connection diagnostic",
  ...
  const transaction = (await consent.text()).match(
    /name="transaction" value="([A-Za-z0-9_-]{43})"/,
  )?.[1];
  requireCondition(
    !!transaction && cookie.startsWith("__Host-carrot-link="),
    "Consent session is incomplete.",
  );
  const approved = await send("/oauth/approve", {
    method: "POST",
    headers: { Origin: expectedOrigin, Cookie: cookie },
  ...
  requireCondition(
    approved.status === 303,
    "Connection password was not accepted. Use the current web-launcher password.",
  );
  const callback = new URL(approved.headers.get("location") ?? "invalid:");
  requireCondition(
    `${callback.origin}${callback.pathname}` === CALLBACK &&
      callback.searchParams.get("state") === state,
    "OAuth callback or state mismatch.",
  );
  const tokens = await send("/oauth/token", {
    method: "POST",
    body: new URLSearchParams({
...
const CALLBACK =
  "https://chatgpt.com/connector/oauth/carrot-connection-diagnostic";
...
async function exerciseGrant(send, clientId, resource, initial) {
  ...
  try {
    ...
    requireCondition(
      ...
      "Refresh tokens were not rotated.",
    );
  ...
  try {
    const revoked = await send("/oauth/revoke", {
      method: "POST",
      body: new URLSearchParams({
        client_id: clientId,
    ...
    requireCondition(
      revoked.status === 200,
      "Diagnostic grant revocation failed.",
    );
  } catch (error) {
    if (failure)
      throw new AggregateError(
        [failure, error],
        "Diagnostic and grant cleanup both failed.",
      );
    throw error;
  }
...
async function exerciseTools(send, token) {
  ...
  const rpc = async (method, params = {}) => {
    const body = await send("/mcp", {
      method: "POST",
      headers,
      body: JSON.stringify({ jsonrpc: "2.0", id: 1, method, params }),
    }).then(readJson);
    requireCondition(
      !body.error && body.result && !body.result.isError,
      "An authenticated MCP request failed.",
    );
    return body.result;
  };
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "carrot-web-diagnostic", version: "1" },
  });
  const notification = await send("/mcp", {
    method: "POST",
    headers,
    body: JSON.stringify({
      jsonrpc: "2.0",
  ...
  requireCondition(
    Array.isArray(tools.tools) && tools.tools.length >= 4,
    "Missing MCP tools.",
  );
  ...
  requireCondition(
    info.oauth === true && info.mode === "read-only",
    "Server is not advertising the web test profile.",
  );
  ...
  console.log(
    `PASS authenticated MCP handshake, ${tools.tools.length} tools, capabilities and library read`,
  );
}
...
async function readJson(response) {
  requireCondition(
    response.ok,
    `OAuth/MCP request returned HTTP ${response.status}.`,
  );
