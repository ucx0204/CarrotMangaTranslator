#!/usr/bin/env python3
"""Deblind r2 queue rows 1..200 as global rows 801..1000.

The public reviewer saw only opaque A..G slots.  This sealer opens exactly the
first 200 private r2 bindings after review, excludes every incomplete/invalid
crop and every protected validation work, then seals both the 801..1000 shard
and an optional 001..1000 cumulative training-only artifact.  Existing sealed
001..800 labels are used only as an identity/duplicate guard and merge source;
their private bindings are never reopened.
"""
...
try:
    from scripts import (
        seal_manga_font_v2_high_value_supervised_labels_range_v2 as prior,
    )
...
base = prior.base
BUILD_MODE_RANGE = "r2_local_range_deblind_global_remap"
BUILD_MODE_MERGE = "sealed_shard_merge"
LOCAL_SPAN = [1, 200]
DIRECT_SPAN = [801, 1000]
CUMULATIVE_SPAN = [1, 1000]
GLOBAL_OFFSET = 800
EXPECTED_REVIEW_ROWS = 200
EXPECTED_RECROP_ROWS = 21
...
def _load_review_contract(
    ...
    if len(decisions) != EXPECTED_REVIEW_ROWS or len(recrop) != EXPECTED_RECROP_ROWS:
        raise base.HighValueSupervisedLabelError("r2 review/recrop row count drifted")
    ...
    if queue_rows != list(range(LOCAL_SPAN[0], LOCAL_SPAN[1] + 1)):
        raise base.HighValueSupervisedLabelError(
            "r2 blind decisions are not rows 1..200"
        )
...
def _guard_prior_duplicates(
    *,
    ...
    if (
        list(counts.get("expected_queue_row_span", ())) != [1, 800]
        or len(old_labels) != 697
        or int(counts.get("training_label_rows", -1)) != 697
    ):
        raise base.HighValueSupervisedLabelError("001..800 cumulative boundary drifted")
    old_samples = {str(row.get("sample_id")) for row in old_labels}
    ...
    if old_samples & new_samples or old_reviews & new_reviews:
        raise base.HighValueSupervisedLabelError(
            "r2 review repeats an existing 001..800 sample/review identity"
        )
    return {
        "duplicate_guard": {
...
def _remap_direct(
    ...
    *,
    ...
    label_ids = {str(row.get("sample_id")) for row in remapped}
    if label_ids & recrop_ids:
        raise base.HighValueSupervisedLabelError(
            "recrop/ruby/split row was promoted into a training label"
        )
    ...
    if (
        ...
        + int(
            base.mapping(counts.get("exclusions"), "exclusions").get(
                "adapter_validation_work", 0
            )
        )
...
def _merge_counts(
    ...
    if blind_rows != 1000 or blind_rows - len(labels) != sum(exclusions.values()):
        raise base.HighValueSupervisedLabelError(
    ...
    return {
        ...
        "r2_recrop_ruby_split_excluded_rows": EXPECTED_RECROP_ROWS,
        "r2_recrop_ruby_split_negative_promotions": 0,
...
def validate_output(
    ...
    if require_current_source and manifest.get(
        ...
        raise base.HighValueSupervisedLabelError("range-v4 sealer source drifted")
    ...
    negative_promotions = int(
        counts.get(
            "recrop_ruby_split_negative_promotions",
            counts.get("r2_recrop_ruby_split_negative_promotions", -1),
        )
    )
    if negative_promotions != 0:
        raise base.HighValueSupervisedLabelError(
            "recrop/ruby/split row gained negative supervision"
        )
    if span == DIRECT_SPAN and (
        ...
        raise base.HighValueSupervisedLabelError("direct r2 exclusion contract drifted")
    if span == CUMULATIVE_SPAN and int(counts.get("blind_rows_consumed", -1)) != 1000:
        raise base.HighValueSupervisedLabelError("cumulative row accounting drifted")
    return {
        **result,
        "expected_queue_row_span": span,
        "recrop_ruby_split_negative_promotions": negative_promotions,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    build.add_argument(
        ...
        type=Path,
        default=Path(
            "artifacts/manga-font-v2-high-value-blind-review-agent-801-1000-"
            "public-only-r1"
        ),
    )
    build.add_argument(
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(f"range-v4 high-value sealer error: {error}") from error
