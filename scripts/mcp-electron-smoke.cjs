...
async function importFixture(dataRoot) {
  ...
  return library.createImport({
    preview: { mode: "single", sourceKind: "images", suggestedWorkTitle: "MCP smoke fixture", chapters: [{
      draftId: "sample", title: "Sample", sourceKind: "images", pages: [{ name: "sample.png", sourcePath, sourceKind: "file" }],
    }] },
...
async function checkRedaction(dataRoot, url, token, chapterId) {
  ...
  const blocked = await call(url, token, "carrot_get_page_preview", { chapterId, pageId: chapter.pages[0].id });
...
async function checkRuntime(dataRoot, tunnel) {
  ...
  const runtime = createMcpRuntime({
    env: {
      CARROT_MCP_ENABLED: "1", CARROT_MCP_TOKEN: token, CARROT_MCP_PORT: "38475", CARROT_MCP_ALLOW_IMAGES: "1",
      CARROT_MCP_PUBLIC_ORIGIN: issuer, CARROT_MCP_OAUTH_ENABLED: "1", CARROT_MCP_OAUTH_PASSWORD: password,
    },
...
async function main() {
  ...
  try {
    ...
    for (const directory of ["main", "shared", "app-runtime"]) {
      await cp(join(root, "out", directory), join(dataRoot, "out", directory), { recursive: true });
    }
    if (process.env.CARROT_MCP_SMOKE_TUNNEL === "1") tunnel = await startQuickTunnel(38475);
