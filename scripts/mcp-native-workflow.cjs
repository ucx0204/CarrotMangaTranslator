...
async function checkNativeWorkflow(root, app, editing, chapterId) {
  ...
  const originals = await Promise.all(
    chapter.pages.map((/** @type {{imagePath:string}} */ page) =>
  ...
  try {
    const plan = await client.call("prepare_workflow", {
      ...
      reason: "Isolated native workflow reconstruction",
      chapters: [
        {
          chapterId,
    ...
    const outputIds = complete.steps
      .filter(
        (/** @type {{stage:string}} */ step) => step.stage === "export-png",
      )
      .map((/** @type {{outputId:string}} */ step) => step.outputId);
    ...
    await client.call("resume_workflow", request);
    assert.deepEqual(
      (await client.call("get_workflow", { id: plan.id })).steps,
      complete.steps,
    );
    ...
    assert.deepEqual(
      (await library.openChapter(chapterId)).pages.map(capturePageRecovery),
      before,
    );
...
async function acknowledgeSavedPage(client, id, chapterId, pageId) {
  ...
  const page = waiting.pages.find(
    (/** @type {Target} */ target) =>
      target.chapterId === chapterId && target.pageId === pageId,
  );
  assert.ok(page);
  const accepted = await client.call("accept_workflow_external", {
    id,
