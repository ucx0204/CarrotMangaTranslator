...
async function retainedClient(root, app, editing) {
  ...
  const call = async (name, args) => {
    ...
    // Failed job metadata is a valid get_job result, not a failed tool call.
    const validated = mcpToolResult(tool, response);
    assert.equal(validated.isError, false, response[0].text);
    assert.ok(
      validated.structuredContent,
      "A registered output contract is required",
    );
    assert.doesNotMatch(response[0].text, /imagePath|transactionId|dataUrl/);
    return validated.structuredContent;
  };
...
module.exports = { checkNativeRetention, retainedClient, waitOutput };
