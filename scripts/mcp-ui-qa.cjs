...
function audit() {
  ...
  if (scenario === "diagnostics") {
    const section = [...document.querySelectorAll(".settings-section")].find(item => item.textContent?.includes("연결 진단 결과"));
    if (!section || !section.textContent?.includes("PASS · OAuth 보호 리소스") ||
        !section.textContent.includes("FAIL · OAuth 인증 서버") || !section.textContent.includes("FAIL · 무인증 MCP POST 차단"))
      throw new Error("Missing independent diagnostic result rows");
    section.scrollIntoView({ block: "end" });
    const bounds = section.getBoundingClientRect(), viewport = panel.getBoundingClientRect();
