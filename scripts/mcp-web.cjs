...
async function assertPortFree(port) {
  await new Promise((resolvePromise, reject) => {
    ...
    server.once("error", () =>
      reject(
        new Error(
          "MCP port is already in use. Close the previous test app first.",
        ),
      ),
    );
  ...
  try {
    ...
    try {
      ...
      throw new AggregateError(
        [error, cleanup],
        "Connection file setup and cleanup failed.",
        { cause: cleanup },
      );
    }
    throw error;
...
function configureWebConnection(origin, port) {
  ...
  writeConnectionFile(
    "mcp-web-connection.json",
    JSON.stringify(
      { url: `${origin}/mcp`, origin, port, pid: process.pid },
      null,
      2,
    ),
  );
  ...
  console.log(
    "[mcp-web] Authentication: OAuth / dynamic registration. Leave client ID and client secret empty.",
  );
  console.log(
    "[mcp-web] Password is in .tmp/mcp-web-password. Paste it only into the consent page on this exact tunnel host.",
  );
}
...
async function main() {
  ...
  if (!/^\d+$/.test(portText) || port < 1 || port > 65535)
    throw new Error("Invalid CARROT_MCP_PORT.");
  await assertPortFree(port);
  console.log(
    "[mcp-web] Starting Cloudflare. Only the authenticated MCP listener is exposed; not the development UI.",
  );
  ...
  tunnel.once("exit", () => {
    if (appStarted) {
      console.error(
        "[mcp-web] Tunnel stopped; shutting down the development app.",
      );
  ...
  console.log(
    "[mcp-web] READY: public OAuth discovery responds. Run node scripts/mcp-web-smoke.mjs, then add the URL in ChatGPT Plugins.",
  );
}
