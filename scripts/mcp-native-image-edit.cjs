...
async function preview(library, revisions, invoke, chapterId, command) {
  ...
  return call(invoke, "carrot_preview_image_edit", {
    chapterId,
    pageId: page.id,
    revision: revisions.createPageRevision(page),
    contextRevision: revisions.mcpContextRevision(saved),
    requestId: randomUUID(),
    reason: "Isolated native image-edit parity",
    command,
  });
}
...
function assertPixelBoundary(before, after, mask) {
  ...
  for (let offset = 0; offset < before.length; offset += 4) {
    const a = before.subarray(offset, offset + 4),
      b = after.subarray(offset, offset + 4);
...
async function checkNativeImageEdit(root, invoke, chapterId) {
  ...
  const original = await readFile(before.imagePath),
    originalPixels = await bitmap(before.imagePath);
  const geometry = {
    kind: "rectangle",
    start: { x: 40, y: 40 },
    end: { x: 90, y: 80 },
  ...
  const protectedAreas = [
    { kind: "ellipse", start: { x: 55, y: 50 }, end: { x: 70, y: 70 } },
  ];
  ...
  const content = await invoke("carrot_get_image_edit_mask", {
  ...
  assertPixelBoundary(
    originalPixels,
    await bitmap(painted.inpaintedImagePath),
    mask.toBitmap(),
  );
  const sample = await call(invoke, "carrot_sample_page_color", {
    chapterId,
    pageId: painted.id,
    revision: revisions.createPageRevision(painted),
    image: "cleaned",
    x: 45,
    y: 45,
  });
  ...
  assert.deepEqual(
    await bitmap(
      (await library.openChapter(chapterId)).pages[0].inpaintedImagePath,
    ),
    originalPixels,
  );
  await act(invoke, restoration.batchId, "undo");
  for (const direction of ["undo", "redo", "undo"])
    await act(invoke, plan.batchId, direction);
  const recovered = (await library.openChapter(chapterId)).pages[0];
  assert.equal(
    revisions.createPageRevision(recovered),
    revisions.createPageRevision(before),
  );
