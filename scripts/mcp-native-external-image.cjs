...
function externalClient(invoke) {
  ...
  const act = async (batchId, action) => {
    await call(`${action}_external_image`, { batchId, requestId: randomUUID() });
  ...
  const upload = async (binding, bytes, purpose) => {
    ...
    const receipt = await call("begin_image_upload", {
      ...binding, ...size, purpose, mimeType: "image/png", requestId: randomUUID(),
      bytes: bytes.length, sha256: createHash("sha256").update(bytes).digest("hex"),
    });
    for (let offset = 0; offset < bytes.length; offset += receipt.chunkBytes)
      await call("write_image_upload", { uploadId: receipt.uploadId, offset,
        data: bytes.subarray(offset, offset + receipt.chunkBytes).toString("base64") });
    const ready = await call("finish_image_upload", { uploadId: receipt.uploadId });
...
async function checkNativeExternalImage(root, invoke, chapterId) {
  ...
  const binding = async () => {
    ...
    return { chapterId, pageId: saved.chapter.pages[0].id,
      revision: createPageRevision(saved.chapter.pages[0]), contextRevision: mcpContextRevision(saved) };
  };
  ...
  const imageUploadId = await client.upload(await binding(), patchPng(false), "image");
  const plan = await client.call("preview_external_image", {
    ...(await binding()), requestId: randomUUID(), reason: "Isolated native external lettering",
  ...
  assert.ok(layered.blocks[0].generatedLettering?.dataUrl.startsWith("data:image/png;base64,"));
  ...
  for (const action of ["undo", "redo", "undo"]) await client.act(plan.batchId, action);
  assert.deepEqual((await read()).blocks, before.blocks);
  assert.deepEqual(await readFile(before.imagePath), original);
  console.log("PASS native external PNG chunks -> protected background pixels -> image history -> lettering layer -> exact recovery without models");
}
...
async function checkBackground(client, binding, original) {
  ...
  const protectedMaskUploadId = await client.upload(binding, patchPng(true), "mask");
  const plan = await client.call("preview_external_image", {
    ...binding, requestId: randomUUID(), reason: "Isolated native external background",
    command: { kind: "patch-background", imageUploadId, protectedMaskUploadId,
  ...
  const inspected = await client.call("get_external_image", { batchId: plan.batchId });
  const page = inspected.pages[0];
  const base = { ...binding, revision: page.expectedRevision, image: "cleaned" };
  const changed = await client.call("sample_page_color", { ...base, x: 11, y: 10 });
  assert.equal(changed.color, "#134679");
  const preserved = await client.call("sample_page_color", { ...base, x: 10, y: 10 });
  ...
  assert.equal(preserved.color, `#${[bgra[2], bgra[1], bgra[0]].map((v) => v.toString(16).padStart(2, "0")).join("")}`);
  await client.call("discard_image_upload", { uploadId: imageUploadId });
  await client.call("discard_image_upload", { uploadId: protectedMaskUploadId });
  for (const action of ["undo", "redo", "undo"]) await client.act(plan.batchId, action);
}
/** @param {boolean} mask */
