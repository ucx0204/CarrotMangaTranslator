...
async function checkNativeSourceRect(root, invoke, chapterId, pageId) {
  ...
  const request = {
    ...target, blockId: before.blocks[0].id,
    revision: createPageRevision(before),
    sourceRect: { x: 42.25, y: 52.5, w: 134.5, h: 119.25 },
  };
  const result = metadata(await invoke("carrot_update_block_source_rect", request));
  ...
  await assert.rejects(() => invoke("carrot_update_block_source_rect", request));
  const noOp = metadata(await invoke("carrot_update_block_source_rect", { ...request, revision: result.revision }));
  assert.equal(noOp.changed, false);
  assert.equal(noOp.status, "already_applied");
  await assert.rejects(() => invoke("carrot_update_block_source_rect", {
    ...request, revision: result.revision,
    sourceRect: { x: before.width - 1, y: 0, w: 2, h: 10 },
  }));
  const restored = metadata(await invoke("carrot_update_block_source_rect", {
    ...request, revision: result.revision, sourceRect: result.previousSourceRect,
  }));
  ...
  assert.deepEqual(await raster(invoke, target), rendered);
  assert.deepEqual(await readFile(before.imagePath), original);
  console.log("PASS native source rectangle edit/readback/restore preserves text, rendering, original bytes and untouched blocks; stale and off-page edits rejected");
}
