...
async function checkNativeErasureRecovery(root, app, editing, target, runtime) {
  ...
  const { McpOperationService } = load("main/application/mcpOperationService.js");
  const { createMcpErasureRecoverySession } = load("main/mcp/mcpErasureRecoverySession.js");
  ...
  const recovery = createMcpErasureRecoverySession(app, operations, editing.notifySaved);
  assert.ok(recovery, "The production history must expose recovery");
  let allowed = true;
  let acquisitions = 0;
  let historyId = "";
  const context = { principalId: "native-recovery-fixture", assertAuthorized: () => assert.ok(allowed, "revoked"), assertScopes: (/** @type {string[]} */ scopes) => {
    assert.ok(allowed, "revoked");
    assert.ok(scopes.every((scope) => ["carrot.read", "carrot.process"].includes(scope)));
  }};
  /** @param {string} name @param {object} args */
  const invoke = async (name, args) => {
    const tool = recovery.tools.find((/** @type {{name: string}} */ tool) => tool.name === name);
    assert.ok(tool, name);
    const content = await tool.invoke(args, context);
    assert.equal(content.length, 1);
    assert.equal(content[0].type, "text");
    assert.doesNotMatch(JSON.stringify(content), /resource_link|mcp-artifacts|"url"|"uri"|transactionId/);
    assert.equal(JSON.stringify(content).includes(root), false);
    return JSON.parse(content[0].text);
  };
  const readPage = async () => (await library.openChapter(target.chapterId)).pages.find((/** @type {{id: string}} */ p) => p.id === target.pageId);
  const before = await readPage();
  const original = await readFile(before.imagePath);
  const previousImage = await readFile(before.inpaintedImagePath ?? before.imagePath);
  try {
    const started = await operations.start({ owner: context.principalId, kind: "erase", requestId: target.requestId, parameters: target, assertAuthorized: context.assertAuthorized,
      execute: async (/** @type {import("../src/main/application/mcpOperationService").McpOperationContext} */ operation) => eraseMcpPage(app, editing, target, operation, {
        ...runtime,
        acquireEngine: async (/** @type {Parameters<typeof runtime.acquireEngine>[0]} */ options) => { acquisitions++; return runtime.acquireEngine(options); },
    ...
    while (operations.status(started.jobId, context.principalId).status === "running") {
    ...
    const inspect = () => invoke("carrot_get_erasure_recovery", { jobId: started.jobId });
    const ready = await inspect();
    assert.equal(ready.canUndo, true);
    const undo = { jobId: started.jobId, revision: ready.revision, requestId: randomUUID() };
    const undone = await invoke("carrot_undo_erasure", undo);
    assert.equal(undone.pagesChanged, 1);
    const undonePage = await readPage();
    assert.deepEqual(await readFile(undonePage.inpaintedImagePath ?? undonePage.imagePath), previousImage);
    ...
    const redo = { jobId: started.jobId, revision: undone.revision, requestId: randomUUID() };
    const redone = await invoke("carrot_redo_erasure", redo);
    assert.equal(redone.pagesChanged, 1);
    assert.deepEqual(await readFile((await readPage()).inpaintedImagePath), erasedImage);
    ...
    assert.equal((await inspect()).canUndo, true, "Old retries must never toggle state");
    ...
    await library.savePageBlocks({ chapterId: target.chapterId, pageId: target.pageId, blocks, blockOrder: saved.blockOrder, expectedRevision: createPageRevision(saved) });
    assert.equal((await inspect()).state, "conflict");
    await assert.rejects(invoke("carrot_undo_erasure", { ...undo, requestId: randomUUID() }));
    assert.equal((await readPage()).blocks[0].translatedText, blocks[0].translatedText);
    const manual = await readPage();
    await library.savePageBlocks({ chapterId: target.chapterId, pageId: target.pageId, blocks: saved.blocks, blockOrder: saved.blockOrder, expectedRevision: createPageRevision(manual) });
    allowed = false;
    ...
    console.log("PASS native selected erasure -> availability -> undo -> redo; exact pixels, unchanged blocks, no extra model, retry and manual-edit protection");
    return result.result;
  } finally {
    recovery.stop(); operations.stop();
    await recovery.close(); await operations.close();
  }
}
