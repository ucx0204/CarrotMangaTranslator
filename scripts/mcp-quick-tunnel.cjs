...
function findQuickTunnelOrigin(text) {
  const match = text.match(/https:\/\/[a-z0-9][a-z0-9-]{0,120}\.trycloudflare\.com(?=[\s|]|$)/);
  return match ? match[0] : null;
}

/** @param {number} port @param {string} [binary]
 * @returns {Promise<{origin: string, child: import("node:child_process").ChildProcess}>} */
function startQuickTunnel(port, binary = process.env.CARROT_CLOUDFLARED_PATH || "cloudflared") {
  ...
  return new Promise((resolve, reject) => {
    ...
    let buffer = "";
    let ready = false;
    const cleanupOnExit = () => { child.kill(); };
    const stopDuringStartup = () => { if (!ready) child.kill(); };
    process.once("exit", cleanupOnExit);
    process.on("SIGINT", stopDuringStartup);
    process.on("SIGTERM", stopDuringStartup);
    const timer = setTimeout(() => {
      child.kill();
      reject(new Error("Cloudflare did not provide a Quick Tunnel URL. Check connectivity and local cloudflared configuration."));
    }, 45_000);
    /** @param {Buffer} chunk */
    const onData = (chunk) => {
      if (ready) return;
    ...
    child.once("error", () => {
      clearTimeout(timer);
      reject(new Error("Could not start cloudflared. Install it or set CARROT_CLOUDFLARED_PATH to the executable."));
    });
    child.once("close", (code) => {
      ...
      if (!ready) reject(new Error(`Cloudflare exited before connection (code ${code}). Check cloudflared configuration and network access.`));
    });
  });
}
...
async function waitQuickTunnelReady(origin, child) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null || child.signalCode !== null) throw new Error("Cloudflare tunnel stopped.");
    try {
...
module.exports = { findQuickTunnelOrigin, startQuickTunnel, waitQuickTunnelReady };
