...
function main() {
  ...
  console.log(
    `[mcp] Local token file: ${tokenPath} (secret; do not share or commit)`,
  );
  console.log(
    `[mcp] Image transfer: ${args.includes("--images") ? "enabled; use only approved test images" : "disabled"}`,
  );
  console.log(
    "[mcp] After the app opens, run node scripts/mcp-smoke.mjs in another terminal.",
  );
