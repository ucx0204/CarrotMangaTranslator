...
- Enrollment stays available while MCP is online, with no timed app-side enrollment button. Each browser request still expires after five minutes; the pending queue is bounded and server stop rejects new registration/approval. Browser-cookie and S256 PKCE binding, comparison code, local approve/deny and same-origin completion remain required. The display code alone is not a credential. No pairing password file is needed in the normal user flow.
- App settings controls for on/off, image/edit/processing/auto-start, stable URL copy, diagnostics, pending approvals and connection revocation. All four first-use preferences are checked; existing stored opt-outs remain unchanged. Preferences never silently expand an old grant or approve a new client.
