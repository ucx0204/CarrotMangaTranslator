...
최종 정리: 2026-09-03 KST
작업 브랜치: `codex/font-size-ai-lab-20260902-r1`
작업 워크트리: `C:\Users\sam40\Downloads\망가번역기-font-size-ai-lab-20260902-r1`
현재 내부 실험 버전: `fsai-lab-v0.10.0`. 철회된 `v0.9.0`은 재사용하지 않았다. 과거 실패와
사용자 직접 판정을 모은 91페이지 고정 세트에서 15회 실험하고, 실제 HayaiOCR 91/91페이지,
일반 텍스트 818개, 사용자 hard constraint 49/49를 통과했다. 2026-09-03 사용자 화면 확인과
승격 승인을 받았으며, 추가 개선 폭이 작다는 판단에 따라 캠페인 010을 재개하지 않고 여기서
중단한다.
...
10. 서로 닿거나 겹쳐 보이는 말풍선도 발화 단위는 분리한다. 잘못 커진 text mask가 이웃
    말풍선을 침범한 경우 큰 논리 bbox로 합치지 말고, 중복 소유된 mask 꼬리를 제거하거나
    상호 배타적 경계를 복구한다. OCR 문장이 자연스럽게 이어진다는 사실은 결합 근거가 아니다.
...
- **2026-09-03 사용자 재감사 정정:** 위 세 건을 같은 종류로 묶은 판정 자체가 잘못됐다.
  P005 D004/D005(`T006+T020`)는 한 말풍선의 조각이 아니라 서로 닿은 위·아래 두
  말풍선이다. `T006`의 본문 core는 약 y=373~620인데, 아래 말풍선의 `T020`과 100%
  중복되는 11px짜리 고립 mask 꼬리가 y=720~773에 붙어 bbox가 y=701까지 늘어났다.
  해결 목표는 `T006`의 타 후보 소유 고립 꼬리를 제거해 위 bbox를 줄이고 `T020`을 별도
  발화로 보존하는 것이다. P005의 v0.6.0 재결합은 실패 사례로 철회하며, 아래 당시 OCR
  exact 수치는 오결합을 정당화하지 않는다.
...
- 당시 실제 결과는 세 대상 OCR exact `3/3`, 나머지 논리 영역 bbox·OCR·추정 크기
  `158/158` 동일이었으나, P005는 정답 단위가 틀렸으므로 `3/3`을 품질 근거로 다시 쓰지
  않는다. 캠페인 001 P032와 P009/P011도 고정 회귀 세트에서 각각 독립 재감사한다.
...
- 당시에는 3/5회 안에 개선으로 오판해 `fsai-lab-v0.6.0`으로 올렸지만, P005 오결합이
  뒤늦게 확인됐다. v0.8.0은 역사적 기준선으로만 유지하며 이 결함을 고치기 전 새 버전으로
  승격하지 않는다.
- 권위 결과는
...
- **2026-09-03 사용자 재감사 확인:** P017의 수리 전 페이지 횡단 꼬리와 수리 후 로컬
  bbox 비교는 올바른 해결로 재확인됐다. 이 사례는 “문장/말풍선을 합치지 않고 오염된
  mask 꼬리만 제거한다”는 positive anchor다. 캠페인 006 P005의 타 후보 소유 꼬리 수리도
  이 원칙을 따라야 하며, 이후 후보는 P017의 말풍선 수와 로컬 bbox를 그대로 보존해야 한다.
...
- 이 불변식은 캠페인 009에만 한정되지 않는다. 캠페인 006 P005처럼 두 text mask bbox가
  잘못 겹치는 경우도 먼저 **타 후보가 소유한 고립 mask 꼬리**를 제거해 각 말풍선 bbox를
  수리해야 한다. 겹침을 이유로 union하거나 recognition segment 두 개로 묶지 않는다.
...
## 고정 회귀 실험 1 · 사용자 직접 정답

2026-09-03의 첫 91페이지 전체 보고서 감사에서 받은 판정을 자동 지표보다 높은 user gold로
고정한다. 아래 ID는 고정 세트의 suite page와 정상 v0.8 HayaiOCR 기준선 candidate다.

| 판정               | 페이지·블록                                                                                                                                                                                                                                                                                    | 반드시 보존하거나 고칠 내용                                                                                                                           |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| 분리 유지          | `P013 D008/D010`, `P015 D006/D007`, `P032 D004/D006`, `P033 D001/D003`, `P038 D003/D004`, `P056 D006/D008`, `P058 D008/D011`, `P062 D009/D010`, `P065 D004/D006`, `P065 D007/D009`, `P070 D014/D015`, `P071 D008/D009`, `P072 D002/D003`, `P074 D005/D006`, `P077 D001/D002`, `P080 D002/D003` | v0.8에서는 이미 별도다. 접촉, 같은 bubble, 가까운 거리, 이어지는 문장을 이유로 결합하지 않는다.                                                       |
| 신규 분할          | `P033 D009`, `P035 D002/D004`, `P041 D006`, `P042 D001/D003/D004`, `P048 D007`, `P059 D011`, `P061 D010`, `P069 D001`, `P076 D004`                                                                                                                                                             | 한 bbox 안에서 큰 빈 축 간격으로 갈라진 실제 텍스트 군집을 각각 독립 블록으로 만든다.                                                                 |
| v0.8 판정 철회     | `P066 D006`, `P070 D011`, `P078 D005`                                                                                                                                                                                                                                                          | 과거 캠페인 008이 성공으로 취급한 세 재결합을 전부 원 text fragment로 되돌린다. 사용자 직접 판정으로 기존 승격 근거는 무효다.                         |
| 경계 수리          | `P019 D006`, `P030 D001`, `P034 D001/D002`, `P038 D008/D011`, `P039 D001/D002`, `P040 D001`, `P046 D002/D003`, `P059 D006/D008`                                                                                                                                                                | 이웃과 합치지 않고, 아래로 늘어진 꼬리·글자를 가르는 경계·상대 영역 침범만 국소적으로 고친다.                                                         |
| 중립/낮은 우선순위 | `P028 D002`, `P038` 미검출 손글씨 디저트 문구, `P047 D004`                                                                                                                                                                                                                                     | 합침/분리 어느 쪽도 허용하거나 있으면 좋은 개선이다. 승격·실패를 단독으로 결정하지 않는다.                                                            |
| 합쳐야 하는 정답   | `P081 D001/D002`                                                                                                                                                                                                                                                                               | `戦いが長引けばこっちが持ちませんよっ!` 한 발화의 잘못 갈린 조각이다. 겹친 경계를 수리하며 하나로 복원한다. 현재 유일한 직접 positive merge anchor다. |
| 마지막 분리 묶음   | `P081 D005/D006`, `P083 D002/D003`, `P084 D002/D003`, `P085 D001/D003`, `P090 D002/D003`, `P091 D004/D005`                                                                                                                                                                                     | v0.8 분리를 보존한다. `P091 D001`은 두 cluster로 새로 분할한다.                                                                                       |
| 분리 모범          | `P091 D006/D008`                                                                                                                                                                                                                                                                               | 현재의 독립 bbox와 발화 수를 그대로 유지한다.                                                                                                         |

사용자 스크린샷의 첫 묶음 번호별 원문 판정은 suite manifest의 `USER GOLD ... #1~#10`,
두 번째~다섯 번째 묶음은 `B2#1~#10`, `B3#1~#10`, `B4#1~#10`, `B5#1~#9` reason에 보존한다. 실험 후보는 이
표의 강제 분리·경계 수리를 모두 점수화하고, 하나라도 이전보다 합쳐지면 hard veto한다.

## 고정 회귀 실험 1~15 · v0.10.0 승격

- 고정 세트는 과거 실패·애매 사례 91페이지이며 suite digest는
  `767fa1a22d11d0abcc39578adc934089de9043d46cafb781a2ffdceaed5c614a`다. 효과음 194개는
  검출 결과에는 남기되 판정·보고서에서는 제외했다. 사용자 직접 제약은 분리 48개와 유일한
  결합 정답 1개, 별도 육안 경계 표적 8개다.
- 실험 1의 v0.8 기준선은 일반 텍스트 `799`, 크기 추정 `754`, 보류 `45`였다. 실험 2에서
  일반 fragment 재결합을 끄자 `817`개와 `46/49`가 됐고, 실험 3의 dialogue overlap-union
  veto로 `819`개와 `48/49`가 됐다. 거리·같은 bubble·문장 연속성은 결합 근거로 쓰지 않는다.
- 실험 4는 composite/child mask가 실제로 중복되는 경우의 문턱만 `0.90→0.85`로 좁혀
  P081의 유일한 positive merge를 복원했다. 일반 텍스트 `818`, hard constraint `49/49`이며
  실험 5 실제 HayaiOCR에서도 같은 구조와 크기 추정 `773`개를 확인했다.
- 실험 6은 다른 후보가 90% 이상 소유한 작은 고립 mask 꼬리만 잘라 P019/P046/P059 경계를
  수리했다. 실험 7의 전역 quantile `0.005→0.01`은 적용 범위가 너무 넓어 기각했다. 실험 8은
  원 detector bbox가 지지하지 않는 고립 꼬리만 잘라 P030을 수리했고, 실험 9는 그 경로의
  bbox 면적 이득 문턱만 `1.25→1.15`로 낮춰 P040을 수리했다. 실험 9의 추가 변화는
  P017/P058뿐이며 기존 승인 영역을 보존했다.
- 실험 10 실제 HayaiOCR은 일반 텍스트 `818`, 크기 추정 `774`, 보류 `44`, 제약 `49/49`였다.
  실험 11은 하나의 composite mask가 서로 떨어진 두 child mask의 합집합일 때 각 duplicate
  group 소유 부분만 남겨 P038/P039의 잘린 경계를 복원했다. 유일한 결합 정답 P081은 child
  mask가 겹치므로 이 경로가 건드리지 않는다.
- 실험 12는 원 detector box가 본문을 85% 이상 지지하고 이웃 detector box가 떨어진 꼬리를
  90% 이상 소유할 때만 최대 20% 꼬리를 자르는 보조 gate를 추가했다. P034의 두 발화를
  올바르게 분리·복원했고 P009/P062의 추가 변화도 원문을 보존했다. 실험 13 실제 HayaiOCR은
  일반 텍스트 `818`, 크기 추정 `775`, 보류 `43`, 제약 `49/49`였다.
- P034는 OCR 문자열을 유지하면서 `私じゃなくてよかった...!`의 추정 크기/신뢰도가
  `20.2282px/0.8252→23.8756px/0.9027`로 개선됐고, `油断してた...!!`는 보류에서
  `17.7101px/0.7792`로 복구됐다. P038은 잘렸던 왼쪽 문장이 `早く運べ!!→すぐ運べ!!`로
  완전해졌고, P039는 깨진 두 문자열이 각각 `一生私の言いなり`,
  `奴隷扱いなんてどうかしら`로 복구됐다.
- 실험 14는 disjoint-child 최소 간격을 grid `3→4`로 높인 반증 실험이다. 구조 제약은
  `49/49`였지만 P038 한 페이지만 퇴행해 오른쪽 bbox가 17px 침범하고 왼쪽 bbox가 17.75px
  잘렸으므로 기각했다. 구조 개수 검사만으로 경계 품질을 대신하지 않는다.
- 실험 15는 최종 코드로 실제 HayaiOCR 91페이지를 다시 실행했다. 일반 텍스트 `818`, 크기
  추정 `775`, 보류 `43`, 효과음 `194`, 제약 `49/49`이며 실험 13과 91페이지 후보 배열이
  완전히 같았다. 정규화 결과 SHA-256은
  `3250ddf6fe8b3b3a0386f8feed7cc8bbf9503d5ee21ad4bfb3363fb2f6e5e0fd`다.
- 최종 self-contained 보고서는
  `artifacts/font-size-ai-lab/failure-balloon-regression-001/checkpoint-exp-15.html`이다.
  91개 전체 페이지와 818개 일반 bbox crop, 총 909개 이미지를 내장하며 외부 이미지와 효과음
  렌더는 0개다. SHA-256은
  `1957064252418509202bc83739a27a10ee3e963e145539d6606975aec2a2baa4`다. 1280×900과
  760×900 브라우저 검증에서 가로 overflow 0, 누락/깨진 로드 이미지 0이었고 검색·상세
  지연 로딩·원본 크기 모달을 확인했다.
- 2026-09-03 사용자는 결과가 완벽하지는 않지만 상당히 훌륭하다고 직접 확인하고 승격을
  승인했다. 추가 개선 여지가 작으므로 15/30에서 의도적으로 종료한다. 새 무작위 화, 추가
  threshold 조정, 캠페인 010 재개는 사용자가 다시 요청할 때만 한다.
- 새 내부 minor 버전은 `fsai-lab-v0.10.0`이다. 철회된 `v0.9.0` 태그/의미를 되살리지 않는다.
  제품 정책은 일반 fragment 자동 재결합 비활성화, dialogue overlap union 금지, 중복 mask
  근거가 있는 선택적 composite 복원, detector/peer 소유 증거가 있는 꼬리 절삭, disjoint
  child composite의 상호 배타 경계 복원으로 고정한다.
...
| `Tada no Murabito…/10.1`     |     3/5 | P005 접촉 말풍선 오결합 확인; 당시 3개 일괄 승격 판정 철회      | `v0.6.0` 철회 사례 포함 | `artifacts/font-size-ai-lab/campaign-006/chapter-report.html`                      |
...
| 과거 실패 91페이지 고정 세트 |   15/30 | 분리 48·선택 결합 1·경계 8 검증, 실제 HayaiOCR 재현·사용자 승인 | `v0.10.0`               | `artifacts/font-size-ai-lab/failure-balloon-regression-001/checkpoint-exp-15.html` |
