...
> 최신 상태: 사용자 승인 C23의 로컬 앱 통합 완료. `font-chapter-c23-handoff.md`를 본다.
> 이후 비교의 B는 변경 직전 승인 앱이며, 아래 v2.5.0 설명은 당시 비교의 역사적 기록이다.
...
> 전체 화 군집 S18은 승격하지 않았으며, 최종 비교의 경계·가독성 반례도 함께 남겼다.

> 2026-09-06 기본 한글 폰트 교체와 실제 proxy 후보 확장은
> [폰트 풀 교체 인계서](font-palette-refresh-20260906.md)를 먼저 본다.
> R33 출력 ID는 보존하고 설치/렌더 후보를 분리한다. 화 단위 S6 연구의 제품 통합과는 별개다.

> 2026-09-06 강한 고딕의 일반체/명조 오선택을 줄이는 추가 제품 경로는
> [C10 고딕 강조 보정 인계서](font-expression-heavy-sans-handoff.md)를 먼저 본다.
> R33/proxy 외부 자산은 그대로이며, 작은 원문 유형 분류기에서 검증된 고딕 대응만 추가한다.
> C9의 명조→경찰감성체 교체는 사용자 판정과 새 화 시각 검증에서 과해 기각했다.
