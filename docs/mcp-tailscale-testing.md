...
2. 처음 사용하는 데이터 폴더는 **이미지 전송·번역문 수정·새 블록/로컬 처리·앱 시작 시 자동 실행**이 모두 체크되어 있습니다. 필요 없는 항목은 끌 수 있습니다. 이미 저장한 설정은 그대로 유지하므로 업데이트로 사용자의 꺼둔 선택을 덮어쓰지 않습니다. 옵션이 체크되어 있어도 새 AI 연결에는 앱의 숫자 확인 승인이 필요합니다.
