...
> 후속 사용자 지시로 C23 승격을 진행한다. 이 문서는 C18 당시 실행의 역사적 근거다.
> C23 통합과 최신 비교 기준은 `font-chapter-c23-handoff.md`를 따른다.
...
후속 비교 기준 변경: 사용자의 지시에 따라 system-reset-005의 새 보고서는 **B=변경 직전
승인 앱(C18), C=새 후보**로 비교한다. 아래 과거 v2.5.0 보고서의 설명과 원본은 역사적
실행 기록으로 보존한다. 기각 후보를 다음 B로 올리지 않는다. C19~C23은 모두 미승격이며
앱은 C18.1을 유지한다. C23의 새 화 부분 개선과 W06 회귀, 새 비교표는
[후속 연구 메모](font-c19-research-memory.md)에 기록했다.
...
2026-09-07 후속 이력 감사 정정: 아래 confirmation-011(S09)과 당시 다음 화로 적은
S10에는 LAB 레지스트리의 C15 완료 기록이 있었다. 최초 인계의 미사용 표기는 잘못이다.
통합 parity와 사용자의 시각 승인은 유효하지만 두 화를 새 일반화 증거로 세지 않는다.
두 레지스트리와 과거 selection 기록을 합쳐 이후 화를 선택했다. 정정 근거와 실제 미사용
confirmation-012/013 기록은 [후속 연구 메모](font-c19-research-memory.md)에 있다.
...
- confirmation-011: Rukeichi Koushaku Tsuma no Mahou Kaikaku / Chapter 4.2, 사전 봉인한 재사용 화.
...
[통합 검증 원본 화질 A/B/C 10쪽](http://127.0.0.1:8769/system-reset-004/chapter-011-app-c18-review.html)
...
사용자는 이 화 결과도 승인하고 이전 20작품 194페이지에 같은 상태를 적용하도록 지시했다.
...
20작품 194페이지의 재적용을 완료했고, A/B/C 582개 원본 크기 이미지와 작품별 독립 HTML을
검증했다. C18 적용 1,222건의 최종 서체·굵기 불일치는 0이다. 직접 확인한 대표 20페이지의
잔여 반례와 정확한 비교 범위는 [194페이지 완료 기록](font-c18-20-work-regression-results.md)에 있다.
당시 다음 화 선택은 `real-source-style-s16/S10/chapter-selection.json`에 보존했으나,
후속 이력 감사에서 이미 사용한 화로 확인해 새 화 목록에서 제외했다.
