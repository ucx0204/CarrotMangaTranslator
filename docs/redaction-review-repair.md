...
- F09: 원본 크롭 계약·소스 검증·분리된 캐시와 실제 캔버스 연결을 반영했다 (`ffe8358`, `6f7c2ed`, `99e354f`). 100% 이상에서 현재 표시 영역의 원본 픽셀을 확인한다. 오래된 요청과 다른 크기의 이미지는 준비 완료로 인정하지 않는다.
...
- F18: 기존 명령·host만 만들어 놓고 실제 앱 조립이 빠졌던 부분을 `15527b3`, `0804cbd`에서 마무리했다. 명령 팔레트의 작품·화·현재 선택 페이지가 사전 편집 화면을 연다. 열기 전에 현재 화 저장을 기다리고 다른 작업과의 충돌을 차단한다. 준비 모드는 job이나 전송 승인을 만들지 않는다.
- F19: 옛 feature 전용 검사와 사용이 끝난 소스 exporter를 제거했다 (`ad9d4f8`). 이전 재개 문서는 이 문서를 가리키며 과거 원문은 Git 이력에 보존한다. 일반 Check는 그대로 실행하고 실패 로그·실제 커버리지 결과를 보존한다. 같은 PR의 이전 검사만 새 커밋으로 대체한다.
...
`14d9c47`의 Check `34592234249`는 타입·린트·아키텍처를 통과한 뒤 미연결 파일 4개의 deadcode 검사에서 실패했다. 미사용 예외를 추가하지 않고 실제 사전 편집 진입점과 원본 미리보기 화면을 연결했다.

사전 편집 연결의 기존 명령/selector/render-boundary 검사 17개, 원본 크롭/캐시/합성/기본 UI 검사 10개, 전체 renderer/shared TypeScript와 해당 lint·아키텍처·오류·mock 경계 검사가 통과했다. 새 검사는 원본 크기·위치·취소를 작은 이미지/전송 경계로 확인하며 기존 DOM 시나리오 3개를 늘리지 않았다.

최신 브랜치의 Windows/macOS 일반 Check가 최종 기준이다. 구현 완료와 전체 검사 성공을 혼동하지 않는다. 실패는 실제 로그를 읽어 고치고, 새 파일의 커버리지 목록은 실제 Windows 측정으로 갱신한다. 기존 파일의 기준을 낮추거나 검사를 끄지 않는다. 최종 실행의 소스 SHA·run ID·검증 범위는 PR #95의 검증 댓글과 Checks에 기록한다. 이후 재개 시 그 결과와 이후 커밋을 함께 확인한다.
