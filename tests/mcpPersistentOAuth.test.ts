...
it("restores confidential client authentication and rotating grants across independent instances", () => {
  const f = fixture();
...
it("rejects corrupt and inconsistent persisted grants before replacing live permissions", () => {
  ...
  // Each grant is now stored once; duplicate or dangling references are invalid.
  data.grants.push({ ...data.grants[0], revoked: true });
  assert.throws(() => f.provider.restore(data), /Duplicate/);
