...
export async function selectionHttpFixture(enableEditing = false) {
  ...
  const origin = "https://selection.test", secret = "s".repeat(43);
  const provider = new McpOAuthProvider(origin, secret, Date.now, {
    allowProcessing: true, allowEditing: enableEditing,
  });
  const session = new McpOAuthSession(provider, { save: async () => {} });
  const grant = createMcpTestGrant(origin, secret);
  const scopes = enableEditing ? "carrot.read carrot.edit carrot.process" : "carrot.read carrot.process";
  const token = grant(provider, scopes), read = grant(provider, "carrot.read"),
    other = grant(provider, scopes), processOnly = grant(provider, "carrot.read carrot.process");
  ...
  const server = await startMcpHttpServer({
    ...
    oauthHttp: new McpOAuthHttp(origin, secret, {
      session, pairing: new McpPairingBroker(provider, secret),
    }),
    tools: f.tools.map((tool) => ({
      ...tool,
      invoke: (args, context) => f.withExecutionSettings(f.settings, () => tool.invoke(args, context)),
    })),
  ...
  return {
    ...f, call, principal, provider, read, other, processOnly,
    close: async () => {
      f.session.stop();
      f.operations.stop();
