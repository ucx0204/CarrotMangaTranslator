...
function authorize(fixture: ReturnType<typeof setup>) {
  ...
  const location = fixture.provider.approve(
    {
...
it("requires explicit OAuth opt-in, HTTPS and separate credentials", () => {
  ...
  assert.throws(() =>
    readMcpConfiguration({ ...env, CARROT_MCP_OAUTH_ENABLED: "1" }),
  );
  assert.throws(() =>
    readMcpConfiguration({ ...env, CARROT_MCP_OAUTH_ENABLED: "bad" }),
  );
  const web = {
    ...env,
  ...
  assert.throws(() =>
    readMcpConfiguration({
      ...web,
      CARROT_MCP_OAUTH_PASSWORD: env.CARROT_MCP_TOKEN,
    }),
  );
});
...
for (const change of [
  ...
  it(`rejects authorization mismatch ${JSON.stringify(change)}`, () => {
    const fixture = setup();
    assert.throws(() =>
      fixture.provider.begin({ ...fixture.input, ...change }),
    );
  });
}
...
it("requires a matching browser cookie and connection password; consent is single-use", () => {
  ...
  assert.throws(() =>
    fixture.provider.approve(
      { ...approval, pairing_secret: "wrong" },
      consent.cookie,
    ),
  );
  ...
  const location = fixture.provider.approve(
    { transaction: denied.transaction, decision: "deny" },
    denied.cookie,
  );
...
it("binds codes to PKCE, client, redirect and audience without leaking access", () => {
  ...
  assert.equal(
    fixture.provider.accepts(`Bearer ${token.refresh_token}`),
    false,
  );
...
it("rotates refresh tokens and revokes their family on replay", () => {
  ...
  assert.throws(() =>
    fixture.provider.token({ ...refresh, refresh_token: next.refresh_token }),
  );
});
...
it("expires access, renews within the grant lifetime, and never extends the grant", () => {
  ...
  now += 3_600_001;
  assert.equal(
    fixture.provider.accepts(`Bearer ${initial.access_token}`),
    false,
  );
  const refresh = {
    grant_type: "refresh_token",
...
for (const method of ["client_secret_post", "client_secret_basic"] as const) {
  it(`authenticates DCR confidential clients using ${method}`, () => {
    const fixture = setup();
    const client = fixture.provider.register({
      redirect_uris: [CALLBACK],
    ...
    assert.equal(
      fixture.provider.accepts(`Bearer ${token.access_token}`),
      true,
    );
  });
}
...
it("escapes metadata in consent HTML and never embeds connection secrets", () => {
  const html = mcpOAuthConsentPage({
    transaction: "tx",
...
it("runs discovery, consent, tokens and MCP over actual HTTP with auth challenges", async () => {
  ...
  try {
    assert.deepEqual(await (await fetch(`${local}/`)).json(), {
      service: "Carrot MCP",
      mode: "read-only",
    ...
    const noOrigin = await fetch(`${local}/oauth/approve`, {
      method: "POST",
      body,
    ...
    const tokens = await fetch(`${local}/oauth/token`, {
      method: "POST",
