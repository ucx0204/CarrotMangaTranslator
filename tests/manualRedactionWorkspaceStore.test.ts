...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), "manual-redaction-workspace-"));
  roots.push(root);
  const path = join(root, "page.png");
  await writeFile(path, "unchanged image bytes");
  const id = randomUUID();
  sessions.push(id);
  const page = {
    id: "page",
...
it("persists drafts and resumes review without touching the original or approving a job", async () => {
  ...
  await expect(
    readFile(join(f.root, "image-redactions.json")),
...
it("requires current saved decisions and an identical final snapshot", async () => {
  const f = await fixture();
  await expect(
    assertRedactionWorkspaceConfirmation(confirmation(f.workspace, 0)),
  ).rejects.toThrow();
  const revision = await saveRedactionWorkspace(request(f.workspace));
  await expect(
    assertRedactionWorkspaceConfirmation(confirmation(f.workspace, revision)),
  ).resolves.toBeUndefined();
  const changed = confirmation(f.workspace, revision);
  changed.pages[0].strokes = [
    { shape: "rectangle", size: 1, points: [{ x: 1, y: 1 }] },
  ];
...
it("rejects foreign pages, duplicate changes and stale concurrent saves without losing the stored draft", async () => {
  const f = await fixture();
  const save = request(f.workspace);
  await expect(
    saveRedactionWorkspace({
      ...save,
      changes: [...save.changes, ...save.changes],
    }),
  ).rejects.toThrow();
  await expect(
    saveRedactionWorkspace({
      ...save,
      view: { ...save.view, selectedIds: ["foreign"] },
    }),
  ).rejects.toThrow();
  ...
  expect(() =>
    getRedactionWorkspacePage(f.workspace.sessionId, "foreign"),
  ).toThrow();
});
it("does not reuse decisions after a source replacement and preserves failed-save input", async () => {
  const f = await fixture();
  await saveRedactionWorkspace(request(f.workspace));
...
it("surfaces corrupt storage rather than silently discarding drafts", async () => {
  ...
  expect(
    await readFile(join(f.root, "manual-redaction-workspaces.json"), "utf8"),
  ).toBe("broken");
});
