...
it.each([true, false])(
  "composes context tools with editing/processing=%s and closes its proposals",
  async (enabled) => {
    const f = await recoveryLibrary();
    const { createMcpContextSession } =
      await import("../src/main/mcp/mcpContextSession");
    ...
    try {
      ...
      const preview = session.tools.find(
        (tool) => tool.name === "carrot_preview_context_edit",
      );
      const inspect = session.tools.find(
        (tool) => tool.name === "carrot_get_context_proposal",
      );
      ...
      const proposal = mcpToolResult(
        preview,
        await preview.invoke(request, context),
      );
      expect(proposal.isError).toBe(false);
      const receipt = JSON.parse(
        proposal.content[0].type === "text" ? proposal.content[0].text : "null",
      );
      const reviewed = mcpToolResult(
        inspect,
        await inspect.invoke({ proposalId: receipt.proposalId }, context),
      );
      expect(reviewed.isError).toBe(false);
      expect(
        mcpContextRevision(await f.library.readWorkContextForEdit("chapter")),
      ).toBe(request.revision);
      session.stop();
      await expect(
        preview.invoke({ ...request, requestId: randomUUID() }, context),
      ).rejects.toThrow();
