...
async function referenceFixture() {
  ...
  const metadata = { enabled: true, origin: "manual" as const,
    createdAt: "2026-09-18T00:00:00.000Z", updatedAt: "2026-09-18T00:00:00.000Z" };
  guide.characters.push({ ...metadata, id: "character-one", displayName: "First speaker",
    sourceNames: ["First"], targetName: "화자", speechStyle: "neutral" });
  guide.glossary.push({ ...metadata, id: "term-one", source: "Term", target: "용어", category: "term" });
...
it("links enabled native IDs, exposes saved references, then restores exact absence without any model", async () => {
  ...
  try {
    ...
    const input = await f.references({ speakerId: "character-one", glossaryEntryIds: ["term-one"] });
    ...
    expect(projectMcpBlocks(after.pages[0], 0, 1)[0]).toMatchObject({
      speakerId: "character-one", glossaryEntryIds: ["term-one"],
    });
...
it("rejects unknown disabled or duplicate references and requires at least one explicit field", async () => {
  ...
  try {
    ...
    for (const values of [{ speakerId: "not-in-this-work" },
    ...
    await f.library.saveWorkStyleGuide(guide);
    await expect(f.preview(await f.references({ speakerId: "character-one" }))).rejects.toThrow(/enabled/i);
...
it("allows undo after context deletion but refuses forward reference use against changed context", async () => {
  ...
  try {
    const before = await f.snapshot();
    const plan = await f.preview(await f.references({ speakerId: "character-one" }));
...
it("keeps bounded explicit input contracts and does not accept caller-supplied raw blocks or replacement text", async () => {
  ...
  try {
    ...
    expect(McpSelectionBatchPreviewSchema.safeParse({ ...input, blocks: [] }).success).toBe(false);
    const bad = structuredClone(input);
    bad.pages[0].edits = [{ ...bad.pages[0].edits[0], translatedText: "unbound" } as never];
    expect(McpSelectionBatchPreviewSchema.safeParse(bad).success).toBe(false);
    const tooMany = { ...input, pages: Array.from({ length: 21 }, () => input.pages[0]) };
    expect(McpSelectionBatchPreviewSchema.safeParse(tooMany).success).toBe(false);
  } finally { await f.close(); }
});
