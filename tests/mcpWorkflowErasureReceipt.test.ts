...
it("requires a conclusive matching erasure receipt after journal reconstruction and never executes to verify it", async () => {
  ...
  try {
    ...
    const tool = createMcpOperationTools(manager, { erase: execute }).find(
      (tool) => tool.name === "carrot_run_page_erasure",
    );
    if (!tool) throw new Error("Missing native erasure tool");
    const receipt = await tool.invoke(
      {
    ...
    expect(
      await workflowErasureCompleted(manager, record, step, () => {}),
    ).toBe(true);
    await manager.close();
    ...
    ).rejects.toThrow("different workflow attempt");
    const wrong = structuredClone(record);
    wrong.pages[0].pageId = "different-page";
    await expect(
      workflowErasureCompleted(manager, wrong, step, () => {}),
    ).rejects.toThrow();
  ...
  try {
    ...
    for (const outcome of [
      ...
      const tool = createMcpOperationTools(manager, {
        erase: async () => ({ ...outcome, revision: page.revision }),
      }).find((tool) => tool.name === "carrot_run_page_erasure");
      if (!tool) throw new Error("Missing native erasure tool");
      step.attemptId = randomUUID();
      const receipt = await tool.invoke(
        {
