...
it("serves versioned sound-effect metadata to read-only OAuth and rejects malformed or unregistered mutations", async () => {
  ...
  try {
    ...
    const origin = "https://sound-effect.test", secret = "s".repeat(43);
    ...
    const server = await startMcpHttpServer({
      ...
      tools: [f.reader], enforceScopes: true,
    ...
    const call = async (name: string, args: object) => (await fetch(server.url, {
    ...
    const result = await call("carrot_get_sound_effects", { chapterId: "chapter", pageId: "page" });
    expect(result.result.isError).toBe(false);
    expect(result.result.structuredContent).toMatchObject({ total: 2, generation: { runtimeChecked: false } });
    ...
    const bad = await call("carrot_get_sound_effects", { chapterId: "chapter", pageId: "page", path: "C:/private.png" });
