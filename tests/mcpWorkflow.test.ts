...
import { createPageRevision, createSoundEffectReviewPageRevision } from "../src/shared/pageRevision";
...
it("runs all selected translations before PNG export in saved page order and retains exact results", async () => {
  ...
  try {
    ...
    expect(plan.steps.map((step) => [step.stage, step.pageId])).toEqual([
      ["translate", "page"], ["translate", "second"], ["export-png", "page"], ["export-png", "second"],
    ]);
    ...
    for (const [index, page] of after.pages.entries()) {
      ...
      expect(page.blocks.map((block) => block.sourceText)).toEqual(before.pages[index].blocks.map((block) => block.sourceText));
      expect(page.blocks.every((block) => block.translatedText === `translated ${block.sourceText}`)).toBe(true);
      expect(await readFile(page.imagePath)).toEqual(await readFile(before.pages[index].imagePath));
    }
...
it("persists external waiting and accepts only the exact saved page before explicit continuation", async () => {
  const f = await workflowFixture();
  try {
    const plan = await f.prepare([{ kind: "await-external", purpose: "translation" }, { kind: "export-png" }]);
    await f.run(plan.id);
    ...
    for (const pageId of ["page", "second"]) {
      ...
      const before = (await f.library.openChapter("chapter")).pages.find((page) => page.id === pageId);
      if (!before) throw new Error("Missing fixture page");
      await f.invoke("carrot_update_page_blocks", {
        chapterId: "chapter", pageId, revision: createPageRevision(before),
        edits: [{ blockId: before.blocks[0].id, fields: { translatedText: "External saved result" } }],
      });
      const page = (await f.library.openChapter("chapter")).pages.find((page) => page.id === pageId);
      if (!page) throw new Error("Missing updated page");
      const input = { id: plan.id, version: state.version, requestId: randomUUID(), page: {
        chapterId: "chapter", pageId, revision: createPageRevision(page), reviewRevision: createSoundEffectReviewPageRevision(page),
      } };
      await expect(f.invoke("carrot_accept_workflow_external", { ...input, page: { ...input.page, revision: createPageRevision(before) } })).rejects.toThrow();
      expect(await f.invoke("carrot_accept_workflow_external", input)).toMatchObject({ status: "paused" });
...
it("pauses only after the current native output settles and resumes remaining pages without rerendering completed ones", async () => {
  ...
  try {
    ...
    f.render.mockImplementationOnce(async (page) => { await gate; return render(page); });
    const plan = await f.prepare([{ kind: "export-png" }]);
    await f.run(plan.id);
    await vi.waitFor(() => expect(f.render).toHaveBeenCalledTimes(1));
    expect(await f.invoke("carrot_pause_workflow", { id: plan.id })).toMatchObject({ status: "running", pauseRequested: true });
    await expect(f.invoke("carrot_discard_workflow", { id: plan.id, confirm: true })).rejects.toThrow();
    release();
    const paused = await f.done(plan.id);
    expect(paused).toMatchObject({ status: "paused", completedSteps: 1 });
    await f.restart();
    await f.run(plan.id);
    expect(await f.done(plan.id)).toMatchObject({ status: "completed", completedSteps: 2 });
    expect(f.render).toHaveBeenCalledTimes(2);
...
it("refuses changed source and later page edits without starting a model", async () => {
  ...
  try {
    ...
    const original = await readFile(path), changed = Buffer.from(original);
    changed[changed.length - 1] ^= 1;
    await writeFile(path, changed);
    await f.run(plan.id);
    expect(await f.done(plan.id)).toMatchObject({ status: "failed", lastError: "revision_conflict", pageAttemptsUsed: 0 });
    expect(f.request).not.toHaveBeenCalled();
    await writeFile(path, original);
    f.settings.api.model = "changed-model";
    await f.run(plan.id);
    expect(await f.done(plan.id)).toMatchObject({ status: "failed", lastError: "revision_conflict" });
    expect(f.request).not.toHaveBeenCalled();
  } finally { await f.close(); }
});
