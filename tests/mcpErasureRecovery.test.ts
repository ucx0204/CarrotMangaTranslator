...
function fixture() {
  ...
  const job = {
    ...
    status: "completed",
    target,
    result: { pagesChanged: 1, blocksErased: 1, cleanupFailed: false },
  };
  const ports: Ports = {
    readJob: vi.fn(async (id, who) => {
      if (id !== jobId || who !== owner) throw new Error("not found");
      return job;
    }),
    inspect: vi.fn(async () => ({
      state,
      reason: "ready" as const,
      revision: state === "applied" ? after : before,
    })),
    apply: vi.fn(async (_id, _target, direction, _revision, guard) => {
      guard();
      state = direction === "undo" ? "undone" : "applied";
      return { revision: state === "applied" ? after : before };
    }),
    withPageEdit: vi.fn(async (_target, guard, execute) => {
      guard();
...
describe("owned selected-erasure recovery", () => {
  it("inspects without mutation and replays undo/redo with separate request IDs", async () => {
    const f = fixture();
    expect(
      await f.service.inspect(f.jobId, f.owner, f.authorize),
    ...
    expect(
      await f.service.inspect(f.jobId, f.owner, f.authorize),
    ).toMatchObject({ canUndo: false, canRedo: true });
  ...
  it("coalesces simultaneous exact retries and never repeats an old undo after redo", async () => {
    ...
    const results = await Promise.all([
      f.service.apply(f.action, f.owner, f.authorize),
      f.service.apply(f.action, f.owner, f.authorize),
    ]);
    ...
    expect(await f.service.apply(f.action, f.owner, f.authorize)).toMatchObject(
      { status: "already_applied", pagesChanged: 0, revision: before },
    );
    expect(
      await f.service.inspect(f.jobId, f.owner, f.authorize),
    ).toMatchObject({ state: "applied" });
    await expect(
      f.service.apply({ ...f.action, direction: "redo" }, f.owner, f.authorize),
  ...
  it("checks ownership on inspect, actions and cached receipts", async () => {
    ...
    await expect(
      f.service.inspect(f.jobId, "other", f.authorize),
    ).rejects.toThrow("not found");
    await expect(
      f.service.apply(f.action, "other", f.authorize),
    ).rejects.toThrow("not found");
    expect(f.ports.apply).toHaveBeenCalledTimes(1);
    await f.service.close();
  });
  it.each(["running", "failed", "cancelled", "partial", "interrupted"])(
    "does not recover a %s job",
    async (status) => {
      const f = fixture();
      f.job.status = status;
      expect(
  ...
  it("does not recover page-wide, other-kind, no-op or cleanup-failed work", async () => {
    ...
    expect(
      await f.service.inspect(f.jobId, f.owner, f.authorize),
    ).toMatchObject({ reason: "not_selected_erasure" });
    f.job.target.blockId = "block";
    f.job.result.cleanupFailed = true;
    expect(
      await f.service.inspect(f.jobId, f.owner, f.authorize),
    ).toMatchObject({ reason: "job_not_completed" });
    f.job.result.cleanupFailed = false;
    f.job.result.pagesChanged = 0;
    expect(
      await f.service.inspect(f.jobId, f.owner, f.authorize),
    ).toMatchObject({ reason: "job_not_completed" });
    f.job.kind = "ocr";
  ...
  it("requires a session history association, including after restart", async () => {
    ...
    expect(
      await restarted.inspect(f.jobId, f.owner, f.authorize),
    ).toMatchObject({ reason: "history_unavailable" });
    await restarted.close();
    await f.service.close();
  });
  it("rechecks authority and state after the page handoff", async () => {
    const f = fixture();
    vi.mocked(f.ports.withPageEdit).mockImplementation(
      async (_target, guard, execute) => {
  ...
  it("retains a failed action and stops deferred actions without releasing native history", async () => {
    const f = fixture();
    vi.mocked(f.ports.notifySaved).mockImplementation(() => {
    ...
    expect(f.ports.apply).toHaveBeenCalledTimes(1);
    await f.service.close();
    f.service.remember("ignored", "ignored");
    await expect(
      f.service.inspect(f.jobId, f.owner, f.authorize),
...
it("exposes strict, scoped, metadata-only tools with validated output contracts", async () => {
  ...
  const context = {
    principalId: f.owner,
    assertAuthorized: f.authorize,
    assertScopes: vi.fn(),
  };
  for (const tool of tools) {
    await expect(
      tool.invoke({ jobId: f.jobId, transactionId: "injected" }, context),
    ).rejects.toThrow();
    await expect(
      tool.invoke(
        { jobId: f.jobId, revision: after, requestId: randomUUID() },
        undefined,
      ),
    ).rejects.toThrow();
  }
  ...
  expect(
    mcpErasureRecoveryOutputs.carrot_get_erasure_recovery.safeParse(
      JSON.parse(result[0].text),
    ).success,
  ).toBe(true);
  expect(result[0].text).not.toMatch(
    /private-native|resource_link|image\/|url/,
  );
  const denied = {
    ...context,
    assertScopes: () => {
