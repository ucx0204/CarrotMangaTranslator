...
describe("manual redaction locale contract", () => {
  it.each(SUPPORTED_UI_LOCALES)("has every key and interpolation variable in %s", (locale) => {
    ...
    for (const key of Object.keys(reference) as Array<keyof typeof reference>) {
      ...
      expect(placeholders(messages[key]), `${locale}.${key}`).toEqual(placeholders(reference[key]));
    }
  });

  it.each(SUPPORTED_UI_LOCALES)("renders its own messages without fallback in %s", async (locale) => {
    ...
    expect(i18n.t("manualRedaction.title")).toBe(APP_I18N_RESOURCES[locale].components.manualRedaction.title);
    const progress = i18n.t("manualRedaction.progress", { count: 7, total: 100 });
