...
it.each(["source", "translation"] as const)(
  "applies only reviewed %s text and exactly restores native blocks without another model call",
  async (kind) => {
    ...
    try {
      ...
      for (const [index, page] of after.pages.entries()) {
        ...
        expect(page.blocks[0][field]).not.toBe(
          before.pages[index].blocks[0][field],
        );
        expect({
          ...page.blocks[0],
          [field]: before.pages[index].blocks[0][field],
        }).toEqual(before.pages[index].blocks[0]);
        expect(page.blocks.slice(1)).toEqual(
          before.pages[index].blocks.slice(1),
        );
      ...
      expect((await f.snapshot()).pages.map((page) => page.blocks)).toEqual(
        after.pages.map((page) => page.blocks),
      );
...
it("appends reviewed OCR discoveries at the front and undo removes only owned blocks with absent order restored", async () => {
  ...
  try {
    await f.mutateStored((chapter) => {
      for (const page of chapter.pages) delete page.blockOrder;
    });
    const before = await f.snapshot();
    ...
    expect(restored.pages.every((page) => page.blockOrder === undefined)).toBe(
      true,
    );
    expect((await f.action(plan.batchId, "redo")).result.status).toBe(
      "completed",
  ...
  try {
    ...
    for (const [index, page] of after.pages.entries()) {
      ...
      expect({
        ...
        glossaryEntryIds: undefined,
      }).toEqual({
        ...before.pages[index].blocks[0],
        glossaryEntryIds: undefined,
      });
    }
    expect((await f.action(plan.batchId, "undo")).result.status).toBe(
    ...
    const omitted = await f.preview(
      await f.references({ glossaryEntryIds: [] }),
    );
    await f.action(omitted.batchId, "apply");
    expect((await f.snapshot()).pages[0].blocks[0].speakerId).toBe(
      "old-speaker",
    );
  } finally {
    await f.close();
...
it("does not let a repeated historical action reapply after undo or replace different preview input", async () => {
  ...
  try {
    ...
    const plan = await f.preview(input);
    await expect(f.preview({ ...input, reason: "different" })).rejects.toThrow(
      /requestId/i,
    );
    ...
    await expect(f.action(plan.batchId, "redo", actionId)).rejects.toThrow(
      /requestId/i,
    );
  } finally {
    await f.close();
