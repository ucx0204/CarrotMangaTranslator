...
export async function imageEditingFixture() {
  ...
  const engine: InpaintingEngine = {
    model: "lama-manga",
    runtimePath: "fixture-native",
    backend: "cpu",
    runRootDir: f.env.root,
    inpaint,
    dispose: async () => {},
  };
  const release = vi.fn(async () => {});
  const acquireEngine = vi.fn<McpImageEditRuntime["acquireEngine"]>(
    async () => ({ engine, release }),
  );
  const getSettings = vi.fn<McpImageEditRuntime["getSettings"]>(
    async () => settings,
  );
  const editing = {
  ...
  const session = createMcpImageEditSession(app, editing, true, true, {
    getSettings,
    acquireEngine,
  });
  ...
  const input = async (command: McpImageEditCommand) => {
    ...
    return McpImageEditPreviewSchema.parse({
      ...
      reason: "Explicit isolated image edit",
      command,
    });
  };
  const preview = async (command: McpImageEditCommand) => {
    ...
    return {
      request,
      ...mcpImageEditOutputs.carrot_preview_image_edit.parse(
        reply.structuredContent,
      ),
    };
  };
  const inspect = async (batchId: string) =>
    mcpImageEditOutputs.carrot_get_image_edit.parse(
    ...
    await vi.waitFor(
      async () => {
        if ((await inspect(batchId)).status === "running")
          throw new Error("Image edit still running");
      },
      { timeout: 10000 },
    );
  ...
  const action = async (
    ...
    const receipt = await invoke(`carrot_${direction}_image_edit`, {
      batchId,
      requestId,
    });
    return { receipt, result: await done(batchId) };
  };
  const pixels = async (path: string | undefined) => {
    if (!path) throw new Error("Expected an existing image artifact.");
...
export function brushCommand(): McpImageEditCommand {
  return {
    ...
    strokes: [{ points: [{ x: 25, y: 35 }], radiusPx: 8 }],
    protectedAreas: [
