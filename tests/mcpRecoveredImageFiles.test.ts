...
} from "../src/main/libraryStore/inpaintedArtifacts";
...
async function fixture() {
  ...
  const image = join(directory, "inpaintedImagePath.png"),
    mask = join(directory, "inpaintMaskPath.png");
...
it("accepts only exact recovery filenames and preserves every current page reference regardless of its field", async () => {
  ...
  try {
    expect(isRecoveredImageArtifact(f.chapter, "inpainted", f.image)).toBe(
      true,
    );
    ...
    for (const path of [
      f.directory,
      f.mask,
      join(f.directory, "extra.png"),
      join(f.chapter, ".mcp-recovered-not-a-uuid", "inpaintedImagePath.png"),
      join(f.root, "outside.png"),
    ])
      expect(isRecoveredImageArtifact(f.chapter, "inpainted", path)).toBe(
        false,
      );
...
it("refuses a recovery-directory symlink without deleting its external target", async () => {
  ...
  try {
    ...
    await expect(
      removeUnreferencedInpaintedArtifacts(
        f.chapter,
        [join(linked, "inpaintedImagePath.png")],
        [],
      ),
    ).rejects.toThrow("symlink");
    expect(await readFile(target, "utf8")).toBe(
      "outside the owned recovery directory",
    );
  } finally {
...
it("reports directory cleanup failures and can retry after the image itself was already removed", async () => {
  ...
  try {
    await rm(f.mask);
    const denied = Object.assign(new Error("directory cleanup denied"), {
      code: "EACCES",
    });
    vi.mocked(rmdir).mockRejectedValueOnce(denied);
    await expect(
      removeUnreferencedInpaintedArtifacts(f.chapter, [f.image], []),
    ).rejects.toBe(denied);
