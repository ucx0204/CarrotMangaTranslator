...
it("requires explicit existing-layer replacement and preserves unrelated block fields", async () => {
  ...
  try {
    const page = (await f.snapshot()).pages[0],
      first = await f.upload(externalPng());
    const command = {
    ...
    await expect(
      f.preview({ ...command, imageUploadId: second.uploadId }),
    ).rejects.toThrow("replaceExisting");
    const replacement = await f.preview({
      ...command,
      imageUploadId: second.uploadId,
      replaceExisting: true,
    });
    expect((await f.action(replacement.batchId, "apply")).result.status).toBe(
      "completed",
    );
    expect((await f.action(replacement.batchId, "undo")).result.status).toBe(
      "completed",
    );
...
it("keeps empty masks unapplied and rejects partially transparent background pixels", async () => {
  ...
  try {
    ...
    ).rejects.toThrow("must be opaque");
...
it("cancels a pending native layer commit without a save and rejects stale cancellation IDs", async () => {
  ...
  try {
    ...
    f.editing.assertWritable.mockImplementationOnce(async () => {
      entered();
      await pending;
    });
    ...
    await started;
    await expect(
      f.invoke("carrot_cancel_external_image", {
        batchId: plan.batchId,
        requestId: randomUUID(),
      }),
    ).rejects.toThrow("currently inspected");
    await f.invoke("carrot_cancel_external_image", {
      batchId: plan.batchId,
      requestId,
    });
    release();
...
it("blocks redacted candidate images and refuses access after session shutdown", async () => {
  ...
  try {
    ...
    await expect(
      f.invoke("carrot_get_external_image_preview", { batchId: plan.batchId }),
