...
it("rejects output publication if retention expires during the native renderer boundary", async () => {
  ...
  try {
    ...
    const png = await readFile((await f.library.openChapter("chapter")).pages[0].imagePath);
    ...
    expect((await f.storage.index()).entries.filter((entry) => entry.kind === "output")).toEqual([]);
...
it("recognizes a committed translation despite notification failure and never repeats it on resume", async () => {
  ...
  try {
    ...
    expect(stopped.steps[0]).toMatchObject({ status: "completed", outcome: "reconciled_native_receipt" });
    expect(stopped.steps[0].changeId).toBeTruthy();
    const saved = await f.library.openChapter("chapter");
    expect(saved.pages[0].blocks[0].translatedText).not.toBe(before.pages[0].blocks[0].translatedText);
    ...
    expect(f.request.mock.calls.length - alreadyCalled).toBe(before.pages[1].blocks.length);
    expect((await f.library.openChapter("chapter")).pages[0].blocks).toEqual(saved.pages[0].blocks);
  } finally {
    await f.close();
  }
});
