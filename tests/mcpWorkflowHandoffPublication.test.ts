...
it("excludes run and settled controls during acceptance while allowing the donor to revoke pending consent", async () => {
  ...
  const hook = vi.spyOn(f.codec, "seal").mockImplementation(async (value) => {
    if (typeof value === "object" && value !== null && "owner" in value && value.owner === f.recipient.principalId) {
      entered(); await gate;
    }
    return seal(value);
  });
  try {
    ...
    const requests: [string, object][] = [
      ["carrot_run_workflow", { id: plan.id, version: plan.version, requestId: randomUUID() }],
      ["carrot_pause_workflow", { id: plan.id }],
      ["carrot_cancel_workflow", { id: plan.id }],
      ["carrot_discard_workflow", { id: plan.id, confirm: true }],
      ["carrot_accept_workflow_external", { id: plan.id, version: plan.version, requestId: randomUUID(), page: plan.pages[0] }],
    ];
    ...
    await f.invoke("carrot_revoke_workflow_handoff", {
      id: plan.id, offerId: offered.receipt.offerId,
    }, f.donor);
    ...
    release(); await rejected; hook.mockRestore(); await f.close();
  }
});
...
it("rechecks original file evidence after encryption and rolls ownership back when it changes", async () => {
  ...
  let changed = false;
  const hook = vi.spyOn(f.codec, "seal").mockImplementation(async (value) => {
    if (!changed && typeof value === "object" && value !== null && "owner" in value && value.owner === f.recipient.principalId) {
      changed = true;
      await writeFile(page.imagePath, Buffer.concat([original, Buffer.from("changed source")]));
    }
    return seal(value);
  });
  try {
    ...
    await f.storage.owned(f.owner, plan.id, "workflow");
    expect(await f.storage.record(plan.id)).toMatchObject({ owner: f.owner, version: plan.version });
    expect(f.render).not.toHaveBeenCalled();
  } finally {
    hook.mockRestore(); await writeFile(page.imagePath, original); await f.close();
  }
});
...
it("refuses handoff while a native child is active and keeps the original owner", async () => {
  ...
  f.request.mockImplementationOnce(async (args) => { await gate; return request(args); });
  try {
    const plan = await f.prepare();
    await f.run(plan.id);
    await vi.waitFor(() => expect(f.request).toHaveBeenCalled(), { timeout: 10000 });
  ...
  it(`recovers both owners and the exact accepted receipt after native ${point}`, async () => {
    ...
    try {
      ...
      reset = tx.setLibraryTransactionCrashInjectorForTests((position) => {
        if (position === point) { crashed = true; throw new tx.SimulatedLibraryTransactionCrash(position); }
      });
      await f.accept(offered.acceptance).catch((error: unknown) => {
        if (!(error instanceof tx.SimulatedLibraryTransactionCrash)) throw error;
      });
      ...
      if (point === "after-commit-point") {
        ...
        expect((await f.accept(offered.acceptance)).status).toBe("already_transferred");
