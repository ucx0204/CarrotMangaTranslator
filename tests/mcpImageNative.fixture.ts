...
class Image {
  ...
  getSize() {
    return { width: this.png?.width ?? 0, height: this.png?.height ?? 0 };
  }
  toPNG() {
  ...
  resize(size: { width: number; height: number }) {
    ...
    for (let y = 0; y < size.height; y++)
      for (let x = 0; x < size.width; x++) {
        const sx = Math.min(
          this.png.width - 1,
          Math.floor((x * this.png.width) / size.width),
        );
        const sy = Math.min(
          this.png.height - 1,
          Math.floor((y * this.png.height) / size.height),
        );
        this.png.data.copy(
          next.data,
          (y * size.width + x) * 4,
          (sy * this.png.width + sx) * 4,
          (sy * this.png.width + sx) * 4 + 4,
        );
      }
...
export const imageNativeBoundary = {
  createFromPath: (path: string) => {
    try {
      return new Image(PNG.sync.read(readFileSync(path)));
    } catch (_error) {
      return new Image(null);
    } // Native API represents decode failure as an empty image.
  },
  createFromBuffer: (bytes: Buffer) => {
    try {
      return new Image(PNG.sync.read(bytes));
    } catch (_error) {
      return new Image(null);
    }
  },
