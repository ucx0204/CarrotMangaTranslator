...
describe("single-block source rectangle mutation", () => {
  it("saves only selected geometry through the existing transaction and returns metadata", async () => {
    ...
    expect(JSON.stringify(result)).not.toMatch(
      /private|PRIVATE|dataUrl|sourceText|resource_link/,
    );
  });
  ...
  it("requires a fresh revision even for retries and unchanged bounds", async () => {
    ...
    await expect(f.service.updateSourceRect(f.request)).rejects.toMatchObject({
      code: "revision_conflict",
    });
    const noOp = await f.service.updateSourceRect({
      ...f.request,
    ...
    const restored = await f.service.updateSourceRect({
      ...f.request,
      revision: saved.revision,
      sourceRect: saved.previousSourceRect,
    });
  ...
  it.each(["dirty", "running", "missing", "revision", "revoked"])(
    ...
    async (cause) => {
      ...
      if (cause === "revision")
        f.chapter.pages[0].blocks[1].translatedText = "later edit";
      const before = structuredClone(f.chapter);
      await expect(
        f.service.updateSourceRect(f.request, () => {
          if (cause === "revoked")
  ...
  it("rechecks concurrent saves and revocation before committing", async () => {
    for (const conflict of [true, false]) {
      ...
      await expect(
        ...
        code: conflict ? "revision_conflict" : "access_denied",
      });
  ...
  it("rejects extra fields and nonrectangles before reading the library", async () => {
    ...
    for (const args of [
      ...
      {
        ...f.request,
        sourceRect: { ...f.request.sourceRect, path: "private" },
      },
