...
it.each(["own cancellation", "foreign AbortError", "cleanup failure"])(
  "settles an owned image export after physical cleanup and reports only unexpected %s",
  async (mode) => {
    const f = exportFixture();
    const entered = exportBarrier();
    const cleaning = exportBarrier();
    const release = exportBarrier();
    const parent = new AbortController();
    const parentReason = new Error("Composite cancellation");
    ...
    try {
      ...
      f.renderImage.mockImplementation(async (page, signal) => {
        if (page.id === "p2") {
          ...
          thrown =
            mode === "own cancellation"
              ? signal.reason
              : mode === "foreign AbortError"
                ? new DOMException("Different abort", "AbortError")
                : new AggregateError(
                    [signal.reason, cleanupFailure],
                    "Renderer cancellation cleanup failed",
                  );
          throw thrown;
        }
        return Buffer.from(`png fixture ${page.id}`);
      ...
      const target = await f.target(
        undefined,
        McpPageExportOptionsSchema.parse({ format: "png" }),
      );
      const receipt = await operations.start({
        ...
        parameters: target,
        assertAuthorized: f.assertRetained,
        execute: (context) => f.service.run(target, context, f.assertRetained),
      });
      const completion = operations.waitForCompletion(
        receipt.jobId,
        "owner",
        parent.signal,
      );
      await Promise.race([
        entered.promise,
        completion.then(() => {
          throw new Error("Export settled before its cancellation boundary");
        }),
      ]);
      parent.abort(parentReason);
      await cleaning.promise;
      expect(operationReason).toBeInstanceOf(DOMException);
      expect(operationReason).not.toBe(parentReason);
      expect(operations.status(receipt.jobId, "owner")).toMatchObject({
        status: "running",
      ...
      expect(settled).toMatchObject({
        ...
        status: "cancelled",
        cancellationRequested: true,
        result: { exportPages: { completed: 1, total: 3 } },
      });
      expect(settled.error).toBeUndefined();
      expect(
        settled.result?.exportPages?.pages.map((page) => page.status),
      ).toEqual(["exported", "cancelled", "unprocessed"]);
      expect(f.reportError.mock.calls).toEqual(
        mode === "own cancellation" ? [] : [[thrown]],
      );
      expect(f.reportError.mock.calls[0]?.[0]).toBe(
        mode === "own cancellation" ? undefined : thrown,
      );
      ...
      try {
        await restored.ready();
        expect(restored.status(receipt.jobId, "owner")).toMatchObject({
          status: "cancelled",
      ...
      parent.abort(parentReason);
      release.resolve();
      const cleanup = await Promise.allSettled([operations.close()]);
      failures.push(
    ...
    if (failures.length)
      throw new AggregateError(
        failures,
        "Export cancellation regression failed",
      );
  },
);

it("reports a falsy export failure when no cancellation was requested", async () => {
  const f = exportFixture();
  try {
    f.render.mockRejectedValue(undefined);
    const result = await f.run();
    expect(f.controller.signal.aborted).toBe(false);
    expect(result.status).toBe("failed");
    expect(f.reportError.mock.calls).toEqual([[undefined]]);
  } finally {
    await f.close();
  }
});
