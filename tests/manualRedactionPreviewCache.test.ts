...
const request = (id: string): RedactionPreviewRequest => ({ sessionId: "11111111-1111-4111-8111-111111111111", pageId: id, maxEdge: 320 });
it("limits concurrent loading, deduplicates and prioritizes the current page", async () => {
  ...
  const cache = new RedactionPreviewCache(async (input) => {
    ...
    return new Promise<string>((resolve) => releases.set(input.pageId, resolve));
  });
  const loading = ["0", "1", "2", "3", "4"].map((id) => cache.read(request(id)));
  expect(started).toEqual(["0", "1", "2"]);
  expect(cache.read(request("4"), true)).toBe(loading[4]);
  releases.get("0")?.("url-0"); await loading[0];
  expect(started).toEqual(["0", "1", "2", "4"]);
  releases.get("1")?.("url-1"); await loading[1];
