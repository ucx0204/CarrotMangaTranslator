...
it("uses owned observations, native handoffs and real storage for apply/undo/redo", async () => {
  ...
  try {
    ...
    expect(inspected.changes[0].after).toMatchObject({ fontFamily: "jua", fontSizeIntent: "source-match" });
    expect(JSON.stringify(inspected)).not.toMatch(/imagePath|sourceImageSha256|beforeBlock|afterBlock/);
    ...
    expect((await f.action(plan.batchId, "apply", action)).historical).toBe(true);
    const after = await f.library.openChapter("chapter");
    expect(after.pages[0].blocks[0].fontSizePx).toBe(original.pages[0].blocks[0].fontSizePx);
    ...
    expect((await f.library.openChapter("chapter")).pages.map((page) => page.blocks)).toEqual(original.pages.map((page) => page.blocks));
    await f.action(plan.batchId, "redo");
    expect((await f.done(plan.batchId)).status).toBe("completed");
    expect((await f.library.openChapter("chapter")).pages.map((page) => page.blocks)).toEqual(after.pages.map((page) => page.blocks));
    for (const page of original.pages) expect(await readFile(page.imagePath)).toEqual(f.bytes);
...
it.each(["preview", "apply", "redo"])("rejects modified original bytes at %s without an additional save", async (stage) => {
  const f = await typographyBatchAppFixture("size");
  try {
    const request = await f.analyze();
    if (stage === "preview") {
      await changedOriginal(f.chapter.pages[0].imagePath);
      await expect(f.invoke("carrot_preview_typography_batch", request)).rejects.toMatchObject({ code: "revision_conflict" });
    } else {
      const plan = await f.invoke("carrot_preview_typography_batch", request);
      if (stage === "redo") {
        await f.action(plan.batchId, "apply"); await f.done(plan.batchId);
        await f.action(plan.batchId, "undo"); await f.done(plan.batchId);
      }
...
it("rejects another owner's observation and refuses raw block/evidence payloads", async () => {
  ...
  try {
    const request = await f.analyze();
    const before = await readFile(f.chapterPath);
    await expect(f.invoke("carrot_preview_typography_batch", request, { ...f.auth, principalId: "different-owner" })).rejects.toMatchObject({ code: "not_found" });
    await expect(f.invoke("carrot_preview_typography_batch", { ...request, observation: {} })).rejects.toThrow();
    await expect(f.invoke("carrot_preview_typography_batch", { ...request, blocks: [] })).rejects.toThrow();
    await expect(f.invoke("carrot_preview_typography_batch", { ...request, analysisJobId: randomUUID() })).rejects.toMatchObject({ code: "not_found" });
    expect(await readFile(f.chapterPath)).toEqual(before);
  } finally { await f.close(); }
});
...
it("detects changed work-font profiles after preview while allowing exact undo", async () => {
  ...
  const { makeProfile } = await import("./helpers/automaticFontMatchingV2Fixtures");
  try {
    const request = await f.analyze();
    const before = await readFile(f.chapterPath);
    const plan = await f.invoke("carrot_preview_typography_batch", request);
...
it("retains all analyzed dependency leases through the actual save notification", async () => {
  const f = await typographyBatchAppFixture("size");
  const { assertLibraryActivityAccess, withLibraryActivityOwner } = await import("../src/main/library/lock");
  try {
    const request = await f.analyze();
    request.pages = [request.pages[0]];
    const plan = await f.invoke("carrot_preview_typography_batch", request);
    f.editing.notifySaved.mockImplementation(() => {
      expect(() => withLibraryActivityOwner("unrelated-editor", () => assertLibraryActivityAccess([
