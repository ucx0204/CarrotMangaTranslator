...
export async function contextHttpFixture() {
  ...
  const { createMcpContextEditingTools } = await import("../src/main/mcp/mcpContextTools");
  const { McpOperationService } = await import("../src/main/application/mcpOperationService");
  const { createMcpContextResearchExecutor } = await import("../src/main/mcp/mcpContextResearchAdapter");
  const { createMcpContextResearchTool } = await import("../src/main/mcp/mcpContextResearchTool");
  ...
  const persistence = { load: async () => structuredClone(journal), save: async (value: unknown) => { journal = structuredClone(value); } };
  const errors: unknown[] = [];
  const operations = new McpOperationService((error) => errors.push(error), Date.now, persistence);
  ...
  const research = vi.fn<typeof researchWorkContext>(async (request) => ({
    engine: request.engine, baseFingerprint: createWorkContextResearchFingerprint(request.guideSnapshot),
    operations: [{
      id: "operation-1", entity: "glossary", action: "add", reason: "Synthetic referenced result", confidence: "high", selectedByDefault: true,
      evidence: { pageCount: 1, mentionCount: 1 }, sources: [{ title: "Synthetic evidence", url: "https://example.com/reference" }],
      after: { id: "research-entry", source: "Research hero", target: "Reviewed hero", category: "term", enabled: true, origin: "ai", createdAt: "2026-09-17T00:00:00.000Z", updatedAt: "2026-09-17T00:00:00.000Z" },
    }], warnings: [], stats: { queryCount: 1, sourceCount: 1, tavilyCreditsUsed: 1, estimatedTokenDelta: 10, elapsedMs: 1 },
  }));
  const origin = "https://context-review.test.ts.net", secret = "s".repeat(43);
  ...
  const full = grant(provider, "carrot.read carrot.edit carrot.process"), read = grant(provider, "carrot.read"), other = grant(provider, "carrot.read carrot.edit carrot.process");
  const server = await startMcpHttpServer({
    config: { port: 0, token: "t".repeat(43), publicOrigin: origin },
    tools: [...createMcpContextEditingTools(proposals, true), ...createMcpOperationTools(operations, {}), createMcpContextResearchTool(operations, createMcpContextResearchExecutor(app, proposals, research))],
    enforceScopes: true,
  ...
  const settle = async (jobId: string) => {
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      if (!response.result || response.result.isError) throw new Error(JSON.stringify(response));
  ...
  return { ...f, proposals, operations, jobs, research, errors, call, rpc, settle, read, other, full, provider,
