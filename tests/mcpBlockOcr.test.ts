...
import {
  McpBlockOcrService,
  selectMcpBlockOcr,
} from "../src/main/application/mcpBlockOcrService";
...
function fixture() {
  ...
  const service = new McpBlockOcrService({
...
describe("read-only block OCR observations", () => {
  it("reads a single saved source rectangle, preserving the whole chapter until explicit application", async () => {
    ...
    const applied = await f.editor.updateBlocks({
      ...
      edits: [
        {
          blockId: "a",
          fields: { sourceText: result.blockOcr.recognizedText },
        },
      ],
    });
    ...
    await f.editor.updateBlocks({
      ...
      edits: [
        {
          blockId: "a",
          fields: { sourceText: result.blockOcr.previousSourceText },
        },
      ],
    });
    expect(f.chapter).toEqual(before);
  ...
  it("uses a containing pixel crop without rewriting fractional source coordinates", () => {
    ...
    expect(selectMcpBlockOcr(f.page, "a").cropRect).toEqual({
      x: 100,
      y: 800,
      w: 200,
      h: 800,
  ...
  it("rejects missing/duplicate IDs and stale snapshots before inference", async () => {
    const f = fixture();
    await expect(
      f.service.run({ ...f.target, blockId: "missing" }, f.context),
  ...
  it("returns empty and multi-region observations without clearing or splitting saved blocks", async () => {
    ...
    f.recognize.mockResolvedValueOnce({
      ...f.evidence,
      regions: [
        f.evidence.regions[0],
        { ...f.evidence.regions[0], sequence: 1 },
      ],
    });
    expect(
      (await f.service.run(f.target, f.context)).blockOcr.warnings,
    ).toContain("multiple_regions");
    expect(f.chapter).toEqual(before);
    expect(f.savePageBlocks).not.toHaveBeenCalled();
  });
  ...
  it("rejects excessive text and regions without truncation", async () => {
    ...
    f.recognize.mockResolvedValueOnce({
      ...f.evidence,
      recognizedText: "x".repeat(20_001),
    });
    await expect(f.service.run(f.target, f.context)).rejects.toMatchObject({
      code: "invalid_edit",
    });
    f.recognize.mockResolvedValueOnce({
      ...f.evidence,
      regions: Array.from({ length: 101 }, () => f.evidence.regions[0]),
    });
    await expect(f.service.run(f.target, f.context)).rejects.toMatchObject({
      code: "invalid_edit",
    });
