...
function grant(provider: McpOAuthProvider, scope: string) {
  ...
  const client = provider.register({ redirect_uris: [redirect_uri], token_endpoint_auth_method: "none" });
  const pending = provider.begin({
    client_id: client.client_id, response_type: "code", redirect_uri,
    resource: `${origin}/mcp`, scope, state: "fixture",
    code_challenge: oauthDigest(secret), code_challenge_method: "S256",
  });
  const response = new URL(provider.approve({ transaction: pending.transaction, decision: "approve", pairing_secret: secret }, pending.cookie));
  return provider.token({
    grant_type: "authorization_code", client_id: client.client_id,
    redirect_uri, resource: `${origin}/mcp`,
    code: response.searchParams.get("code"), code_verifier: secret,
  }).access_token;
}

async function fixture() {
  const f = await recoveryLibrary();
  const { McpBlockOcrService } = await import("../src/main/application/mcpBlockOcrService");
  const { McpOperationService } = await import("../src/main/application/mcpOperationService");
  const { McpPageEditService } = await import("../src/main/application/mcpPageEditService");
  const { createMcpPageEditScope } = await import("../src/main/mcp/mcpPageEditScope");
  const { createMcpPageEditTools } = await import("../src/main/mcp/mcpPageEditTools");
  ...
  const evidence = {
    engine: "hayai", sourceLanguage: "ja", sourceCropSha256: "a".repeat(64),
    recognizedText: "再読した原文 🥕",
    regions: [{ sequence: 0, sourceText: "再読した原文 🥕", sourceRect: { x: 10, y: 20, w: 90, h: 120 }, sourceDirection: "vertical" as const, textRole: "ordinary" as const }],
  };
  const recognize = vi.fn<ConstructorParameters<typeof McpBlockOcrService>[0]["recognize"]>(async () => structuredClone(evidence));
  const observer = new McpBlockOcrService({ openChapter: f.library.openChapter, recognize });
  let journal: unknown = null;
  const persistence = {
    load: async () => structuredClone(journal),
    save: async (value: unknown) => { journal = structuredClone(value); },
  };
  const operations = new McpOperationService(() => {}, Date.now, persistence);
  const tools = createMcpOperationTools(operations, {
    blockOcr: (target, context) => runMcpAppJob(app, context, "gemma-analysis",
      (job) => observer.run(McpBlockOcrTargetSchema.parse(target), job),
      { resources: [{ kind: "model-runtime", scope: "*", access: "write" }], page: { ...target, readChapter: f.library.openChapter } },
    ),
  });
  const editor = new McpPageEditService({
    openChapter: f.library.openChapter, savePageBlocks: f.library.savePageBlocks,
    assertWritable: async () => {}, notifySaved: () => {},
    withPageEdit: createMcpPageEditScope(app, f.library.openChapter),
  });
  const provider = new McpOAuthProvider(origin, secret, Date.now, { allowProcessing: true, allowEdits: true });
  ...
  const server = await startMcpHttpServer({
    ...
    tools: [...tools, ...createMcpPageEditTools(editor, true, true)], enforceScopes: true,
    oauthHttp: new McpOAuthHttp(origin, secret, { session, pairing: new McpPairingBroker(provider, secret) }),
    reportError: () => {},
  });
  ...
  return {
    ...f, evidence, recognize, operations, jobs, call, rpc, read, other, provider, full,
    target: { ...f.target, revision: createPageRevision(f.after), requestId: randomUUID() },
    stored: () => journal,
    restart: async () => {
    ...
    close: async () => { await operations.close(); await server.close(); stop(); await f.close(); },
  };
}
async function settled(f: Awaited<ReturnType<typeof fixture>>, jobId: string) {
  for (let i = 0; i < 100; i++) {
    ...
    if (reply.result.structuredContent.status !== "running") return reply.result.structuredContent;
...
it("observes without saving, applies only the chosen source text, and expires text evidence after restart", async () => {
  ...
  try {
    ...
    expect(job).toMatchObject({ kind: "blockOcr", status: "completed", result: { pagesChanged: 0, blockOcr: { recognizedText: f.evidence.recognizedText, previousSourceText: "source" } } });
    expect(await f.snapshot()).toEqual(before);
    for (const name of ["carrot_get_job", "carrot_list_jobs"]) {
      const result = await f.call(name, name === "carrot_list_jobs" ? {} : { jobId });
      expect(result.result.content).toHaveLength(1);
      expect(JSON.stringify(result)).not.toMatch(/resource_link|dataUrl|mcp-artifacts|image\/png/);
      expect(JSON.stringify(result)).not.toContain(f.environment.root);
    }
    expect((await f.call("carrot_run_block_ocr", f.target)).result.structuredContent.jobId).toBe(jobId);
    ...
    const applied = await f.call("carrot_update_page_blocks", {
      chapterId: f.target.chapterId, pageId: f.target.pageId, revision: job.result.revision,
      edits: [{ blockId: f.target.blockId, fields: { sourceText: payload.recognizedText } }],
    });
    expect(applied.result.isError).toBe(false);
    const page = (await f.library.openChapter("chapter")).pages[0];
    expect(page.blocks).toEqual([{ ...f.after.blocks[0], sourceText: payload.recognizedText }, f.after.blocks[1]]);
    expect(page.inpaintedImagePath).toBe(f.after.inpaintedImagePath);
    const restoredText = await f.call("carrot_update_page_blocks", {
      chapterId: f.target.chapterId, pageId: f.target.pageId, revision: applied.result.structuredContent.revision,
      edits: [{ blockId: f.target.blockId, fields: { sourceText: payload.previousSourceText } }],
    });
    expect(restoredText.result.isError).toBe(false);
    expect((await f.library.openChapter("chapter")).pages[0].blocks).toEqual(f.after.blocks);
    ...
    expect(restored.status(jobId, owner).result).toMatchObject({ observationExpired: true });
...
it("enforces processing approval, exact target inputs and job ownership", async () => {
  ...
  try {
    ...
    expect(listed.result.tools.some((tool: { name: string }) => tool.name === "carrot_run_block_ocr")).toBe(false);
    expect((await f.call("carrot_run_block_ocr", f.target, f.read)).error.code).toBe(-32602);
    const { blockId: _block, ...missingBlock } = f.target;
    for (const args of [missingBlock, { ...f.target, path: "C:/private" }, { ...f.target, blockId: "../escape" }])
      expect((await f.call("carrot_run_block_ocr", args)).error.code).toBe(-32602);
    expect(f.recognize).not.toHaveBeenCalled();
    const started = await f.call("carrot_run_block_ocr", f.target);
    const jobId = started.result.structuredContent.jobId;
    await settled(f, jobId);
    expect((await f.call("carrot_get_job", { jobId }, f.other)).result.structuredContent.error).toBe("not_found");
    expect((await f.call("carrot_cancel_job", { jobId }, f.other)).result.structuredContent.error).toBe("not_found");
    expect((await f.call("carrot_get_job_file", { jobId })).error.code).toBe(-32602);
    expect((await f.call("carrot_run_block_ocr", { ...f.target, blockId: "b" })).result.structuredContent.error).toBe("invalid_edit");
    expect(await f.snapshot()).toEqual(before);
  } finally { await f.close(); }
});
...
it("retains the selected block on explicit failure retry and never reruns a completed request", async () => {
  ...
  try {
    ...
    const retried = await f.call("carrot_retry_job", { jobId: first, revision: f.target.revision, requestId: randomUUID() });
    ...
    const forbidden = await f.call("carrot_retry_job", { jobId: next, revision: f.target.revision, requestId: randomUUID() });
