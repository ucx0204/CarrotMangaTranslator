...
it("rejects malformed, duplicate and unbounded research evidence without changing the library", async () => {
  ...
  try {
    ...
    const mutations = [
      (value: typeof result) => {
        value.baseFingerprint = "wrong";
      },
      (value: typeof result) => {
        value.operations.push(structuredClone(value.operations[0]));
      },
      (value: typeof result) => {
        value.operations = Array.from({ length: 101 }, () =>
          structuredClone(value.operations[0]),
        );
      },
      (value: typeof result) => {
        Reflect.set(value.operations[0].after, "source", "");
      },
      (value: typeof result) => {
        Reflect.set(value.operations[0], "action", "delete");
      },
      (value: typeof result) => {
        value.operations[0].action = "update";
      },
      (value: typeof result) => {
        value.operations[0].sources = [];
      },
      (value: typeof result) => {
        value.operations[0].reason = "x".repeat(2001);
      },
    ];
...
it.each(["empty", "warnings"] as const)(
  "finishes %s app research without publishing or applying unwanted context",
  async (kind) => {
    const f = await contextHttpFixture();
    try {
