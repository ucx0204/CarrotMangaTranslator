...
function fixture(): RedactionWorkspace {
  return {
    ...
    revision: 0,
    pages: [{
      id: "page-1", name: "001.png", imagePath: "fixture.png",
      fingerprint: "a".repeat(64), width: 1200, height: 1800,
      strokes: [], decision: "unreviewed",
    }],
    view: { ...DEFAULT_REDACTION_VIEW, currentId: "page-1", selectedIds: ["page-1"] },
    preferences: { ...DEFAULT_REDACTION_PREFERENCES },
    presets: [],
  };
}

function HeaderHarness({ preparation, onHelp }: { preparation: boolean; onHelp: () => void }) {
  const [workspace] = React.useState(fixture);
  const form = useRedactionWorkspace(workspace);
  return <RedactionWorkspaceHeader form={form} preparation={preparation} onOpen={vi.fn()} onHelp={onHelp} />;
}

async function showHeader(locale: UiLocale, preparation: boolean) {
  const i18n = createInstance();
  await i18n.init({
    lng: locale, fallbackLng: false, defaultNS: "components",
    resources: { [locale]: APP_I18N_RESOURCES[locale] },
    interpolation: { escapeValue: false },
  });
  const onHelp = vi.fn();
  const result = render(<I18nextProvider i18n={i18n}>
    <HeaderHarness preparation={preparation} onHelp={onHelp} />
  </I18nextProvider>);
  return { ...result, onHelp, text: APP_I18N_RESOURCES[locale].components.manualRedaction };
}

describe("compact manual-redaction help", () => {
  for (const preparation of [false, true]) {
    it.each(SUPPORTED_UI_LOCALES)(`links mode-specific guidance to the info control in %s (preparation=${preparation})`, async (locale) => {
      const { text, container, onHelp } = await showHeader(locale, preparation);
      ...
      expect(tooltip?.textContent).toBe(preparation ? text.preparationHint : text.outboundHint);
      expect(info.getAttribute("title")).toBeNull();
      expect(container.querySelectorAll("p")).toHaveLength(0);
      fireEvent.focus(info);
      fireEvent.click(screen.getByRole("button", { name: text.shortcuts }));
