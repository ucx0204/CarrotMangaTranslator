...
it.each(["jpeg", "webp"] as const)(
  "retains %s under the owner and reissues exact bytes without a renderer",
  async (format) => {
    ...
    try {
      ...
      // These bytes represent the external renderer boundary, not a codec test.
      const bytes = Buffer.from(`${format} encoded test boundary`);
      ...
      const tool = wrap({
        name: "carrot_export_pages_images",
        description: "Fixture bytes from the external renderer only",
        inputSchema: { type: "object" },
        readOnly: true,
        requiredScopes: ["carrot.read", "carrot.images"],
        invoke: async () => {
          output = await artifacts.putImage(bytes, format, async () => {}, {
            chapterId: "chapter",
            pageId: page.id,
      ...
      if (!output?.retainedOutputId)
        throw new Error("Missing retained raster output");
      ...
      expect(issued.mimeType).toBe(`image/${format}`);
      expect(
        await f
          .operations()
