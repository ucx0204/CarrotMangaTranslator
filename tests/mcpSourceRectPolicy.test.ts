...
const request = {
  ...
  sourceRect: { x: 200, y: 320, w: 300, h: 160 },
};
function page() {
  return editingChapter().pages[0];
}
describe("source-only rectangle policy", () => {
  it("changes only the selected source box and preserves text, layout, masks and images", () => {
    ...
    expect(result.result).toMatchObject({
      ...
      previousSourceRect: { x: 10, y: 20, w: 90, h: 120 },
      renderFramePinned: false,
    });
    ...
    expect(JSON.stringify(result.result)).not.toMatch(
      /PRIVATE|private|sourceText|dataUrl/,
    );
  });
  it("does not touch an equivalent existing pixel box", () => {
    const source = page();
    const result = applyMcpSourceRect(source, {
      ...request,
      sourceRect: source.blocks[0].bbox,
    });
  ...
  it("pins the old expanded text frame when legacy blocks lack a render box", () => {
    ...
    block.translatedText =
      "A much longer translated sentence with <b>markup</b>";
    ...
    expect(resolveEffectiveRenderBbox(result.blocks[0], source, plain)).toEqual(
      before,
    );
  ...
  it.each([
    ...
    { x: 0, y: 1599, w: 2, h: 2 },
    { x: NaN, y: 0, w: 2, h: 2 },
    { x: 0, y: 0, w: Infinity, h: 2 },
    { x: 0, y: 0, w: 0.1, h: 2 },
  ])(
    "rejects invalid/unrepresentable bounds without clipping: %j",
    (sourceRect) => {
      const source = page(),
        before = structuredClone(source);
      expect(() =>
        applyMcpSourceRect(source, { ...request, sourceRect }),
      ).toThrow();
      expect(source).toEqual(before);
    },
  );
  it("accepts page-edge and fractional-pixel coordinates without rounding", () => {
    ...
    for (const sourceRect of [
      { x: 0, y: 0, w: 1000, h: 1600 },
      { x: 10.25, y: 20.5, w: 35.5, h: 40.25 },
    ]) {
      const result = applyMcpSourceRect(source, { ...request, sourceRect });
      for (const key of ["x", "y", "w", "h"] as const)
        expect(result.result.sourceRect[key]).toBeCloseTo(sourceRect[key], 8);
    }
  });
  it("refuses missing/ambiguous block IDs and corrupt page dimensions", () => {
    const source = page();
    expect(() =>
      applyMcpSourceRect(source, { ...request, blockId: "missing" }),
    ).toThrow("not found");
  ...
  it("validates strict single-block requests before application", () => {
    ...
    for (const extra of [
      { force: true },
      { blockIds: ["a", "b"] },
      { renderRect: request.sourceRect },
      { revision: "bad" },
    ])
      expect(
        McpSourceRectPatchSchema.safeParse({ ...request, ...extra }).success,
      ).toBe(false);
    expect(
      McpSourceRectPatchSchema.safeParse({
        ...request,
        sourceRect: { ...request.sourceRect, path: "private" },
      }).success,
    ).toBe(false);
  });
});
