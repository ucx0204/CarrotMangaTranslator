...
it.each(["translate", "note"] as const)(
  "finishes queued context application without retaining research leases (%s)",
  async (sfxMode) => {
    ...
    try {
      ...
      const preview = await f.call("carrot_preview_context_edit", {
        ...
        changes: [{ changeId: "rule", entity: "rules", values: { sfxMode } }],
      });
      ...
      const applying = f.call(
        ...
        {
          proposalId: preview.result.structuredContent.proposalId,
          requestId: randomUUID(),
