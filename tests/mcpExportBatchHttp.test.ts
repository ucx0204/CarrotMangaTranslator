...
import { mcpToolOutputSchema, mcpOutputSchemas } from "../src/main/mcp/mcpOutputSchemas";

async function exportPages(f: Awaited<ReturnType<typeof exportHttpFixture>>) {
  const response = await f.call("carrot_preflight_pages_export", { chapterId: "chapter" });
  ...
  const target = {
    ...
    pages: plan.pages.map((page: { pageId: string; revision: string }) => ({
      pageId: page.pageId, revision: page.revision,
    })),
...
it("serves explicitly selected PNG and ZIP files while every job receipt remains metadata-only", async () => {
  ...
  try {
    ...
    for (const name of ["carrot_preflight_pages_export", "carrot_export_pages_png", "carrot_create_export_zip"])
    ...
    expect((await f.call("carrot_export_pages_png", target)).result.structuredContent.jobId).toBe(jobId);
    expect((await f.call("carrot_get_job_file", { jobId })).result.isError).toBe(true);
    const png = (await f.call("carrot_get_job_file", { jobId, pageId: "p1" })).result;
    ...
    const attached = (await f.call("carrot_get_job_file", { jobId: zipId, includeAttachment: true })).result;
    expect(attached.structuredContent).toEqual(file);
    expect(attached.content[1]).toMatchObject({
      type: "resource_link", mimeType: "application/zip", name: "carrot-pages.zip", size: file.bytes,
    });
    ...
    expect(Object.keys(entries)).toEqual(["0001.png", "0002.png", "0003.png", "manifest.json"]);
    ...
    expect(entries["manifest.json"].toString()).not.toMatch(/mcp-artifacts|sourceText|translatedText|"url"/);
    for (const [name, args] of [
      ...
      expect(JSON.stringify(receipt)).not.toMatch(/mcp-artifacts|resource_link|"url"/);
    }
    expect(mcpOutputSchemas.carrot_get_job_file.safeParse({ ...file, mimeType: "image/png" }).success).toBe(false);
...
it("requires explicit partial ZIP approval and never includes failed or unprocessed pages", async () => {
  ...
  try {
    ...
    for (const pageId of ["p2", "p3", "absent"])
      expect((await f.call("carrot_get_job_file", { jobId, pageId })).result.isError).toBe(true);
    expect((await f.call("carrot_get_job_file", { jobId, pageId: "p1" })).result.isError).toBe(false);
    const denied = await f.call("carrot_create_export_zip", { sourceJobId: jobId, requestId: randomUUID() });
    expect((await f.finish(denied.result.structuredContent.jobId)).status).toBe("failed");
    const started = await f.call("carrot_create_export_zip", {
      sourceJobId: jobId, requestId: randomUUID(), allowPartial: true,
    });
    const zipId = started.result.structuredContent.jobId;
    expect((await f.finish(zipId)).status).toBe("completed");
    const file = (await f.call("carrot_get_job_file", { jobId: zipId })).result.structuredContent;
    expect(file.partialOutput).toBe(true);
    expect(file.pageCount).toBe(1);
    const entries = readExportZip(Buffer.from(await (await f.send(new URL(file.url).pathname)).arrayBuffer()));
...
it("enforces scopes, ownership, strict arguments, matching suffixes and HTTP policy", async () => {
  ...
  try {
    ...
    expect((await f.call("carrot_preflight_pages_export", { chapterId: "chapter" }, read)).result.isError).toBe(false);
    expect((await f.call("carrot_export_pages_png", {}, read)).error.code).toBe(-32602);
    const { jobId } = await exportPages(f);
    for (const caller of [read, other]) {
      const response = await f.call("carrot_get_job_file", { jobId, pageId: "p1" }, caller);
    ...
    for (const args of [
      ...
      expect((await f.call("carrot_get_job_file", args)).error.code).toBe(-32602);
    const png = (await f.call("carrot_get_job_file", { jobId, pageId: "p1" })).result.structuredContent;
    const pngPath = new URL(png.url).pathname;
    expect((await f.send(pngPath.replace("page.png", "pages.zip"))).status).toBe(404);
    const started = await f.call("carrot_create_export_zip", { sourceJobId: jobId, requestId: randomUUID() });
    const zipId = started.result.structuredContent.jobId;
    await f.finish(zipId);
    const file = (await f.call("carrot_get_job_file", { jobId: zipId })).result.structuredContent;
    expect((await f.call("carrot_get_job_file", { jobId: zipId, pageId: "p1" })).result.isError).toBe(true);
    const path = new URL(file.url).pathname;
    for (const invalid of [path.replace("pages.zip", "page.png"), path + "?extra=1", "/mcp-artifacts/invalid/pages.zip"])
      expect((await f.send(invalid)).status).toBe(404);
    expect((await f.send(path, { method: "POST" })).status).toBe(405);
    expect((await f.send(path, { headers: { Origin: "https://other.test" } })).status).toBe(403);
...
it.each(["expiry", "revision", "order", "redaction-equivalent denial", "stop"])(
  ...
  async (change) => {
    ...
    try {
      ...
      const zipId = started.result.structuredContent.jobId;
      await f.finish(zipId);
      const link = (await f.call("carrot_get_job_file", { jobId: zipId })).result.structuredContent;
      if (change === "expiry") f.expire();
      if (change === "revision") f.chapter.pages[0].blocks[0].translatedText = "changed";
      ...
      for (const includeAttachment of [false, true])
        expect((await f.call("carrot_get_job_file", { jobId: zipId, includeAttachment })).result.isError).toBe(true);
