...
} from "../src/shared/mcpExternalImages";
...
export async function externalImageFixture() {
  ...
  const upload = async (png: PNG, purpose: "image" | "mask" = "image") => {
    ...
    const receipt = mcpImageUploadOutputs.carrot_begin_image_upload.parse(
      (await invoke("carrot_begin_image_upload", input)).structuredContent,
    );
    for (
      let offset = 0;
      offset < bytes.length;
      ...
      await invoke("carrot_write_image_upload", {
    ...
    const ready = mcpImageUploadOutputs.carrot_finish_image_upload.parse(
      (
        await invoke("carrot_finish_image_upload", {
          uploadId: receipt.uploadId,
        })
      ).structuredContent,
    );
  ...
  const preview = async (command: McpExternalImagePreview["command"]) => {
    const request = McpExternalImagePreviewSchema.parse({
      ...(await binding()),
      requestId: randomUUID(),
      reason: "Isolated external asset",
      command,
    });
    const reply = await invoke("carrot_preview_external_image", request);
    return {
      request,
      ...mcpExternalImageOutputs.carrot_preview_external_image.parse(
        reply.structuredContent,
      ),
    };
  };
  const inspect = async (batchId: string) =>
    mcpExternalImageOutputs.carrot_get_external_image.parse(
      (await invoke("carrot_get_external_image", { batchId }))
        .structuredContent,
    );
  const action = async (
    batchId: string,
    ...
    const receipt = await invoke(`carrot_${direction}_external_image`, {
      batchId,
      requestId,
    });
    await vi.waitFor(
      async () => {
        if ((await inspect(batchId)).status === "running")
          throw new Error("External action is running");
      },
      { timeout: 10000 },
    );
