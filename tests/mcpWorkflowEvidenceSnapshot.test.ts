...
async function duringSourceRead(
  ...
  vi.mocked(readFile).mockImplementation(async (...args) => {
    ...
    if (!changed && String(args[0]) === source) {
      changed = true;
      const chapter = JSON.parse(await actual.readFile(f.chapterPath, "utf8")) as ChapterFile;
      mutate(chapter);
...
it("rechecks selected page revisions after hashing instead of returning a mixed-time snapshot", async () => {
  ...
  const { verifyWorkflowPages } = await import("../src/main/mcp/mcpWorkflowEvidence");
  try {
    const plan = await f.prepare([{ kind: "export-png" }]);
    const record = McpWorkflowRecordSchema.parse(await f.storage.record(plan.id));
    await duringSourceRead(f, (chapter) => {
      chapter.pages[0].blocks[0].translatedText = "concurrent native page edit";
    }, async () => {
      await expect(verifyWorkflowPages(record, () => {})).rejects.toThrow("changed while source evidence");
    });
...
it("does not publish a prepared plan when chapter order changes while hashing its sources", async () => {
  const f = await workflowFixture();
  try {
    await duringSourceRead(f, (chapter) => { chapter.pages.reverse(); }, async () => {
      await expect(f.prepare([{ kind: "export-png" }])).rejects.toThrow("changed while source evidence");
    });
...
it("does not broaden selected-page conflict checks to an unrelated page", async () => {
  ...
  const { verifyWorkflowPages } = await import("../src/main/mcp/mcpWorkflowEvidence");
  try {
    const chapter = JSON.parse(await readFile(f.chapterPath, "utf8")) as ChapterFile;
    chapter.pages.push({ ...structuredClone(chapter.pages[0]), id: randomUUID() });
    await writeFile(f.chapterPath, JSON.stringify(chapter));
    const prepared = await f.prepare([{ kind: "export-png" }], {
      chapters: [{ chapterId: "chapter", pages: chapter.pages.slice(0, 2).map((page) => ({ pageId: page.id, revision: createPageRevision(page) })) }],
    });
    const record = McpWorkflowRecordSchema.parse(await f.storage.record(prepared.id));
    await duringSourceRead(f, (value) => {
