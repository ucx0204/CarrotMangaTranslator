...
import {
  mcpWorkflowOutputs,
  type McpWorkflowStage,
} from "../src/shared/mcpWorkflow";
...
export async function workflowFixture() {
  ...
  const render = vi.fn(async (page: { imagePath: string }) =>
    readFile(page.imagePath),
  );
  const preferences = {
    allowEditing: true,
  ...
  const open = async () => {
    ...
    const artifacts = new McpArtifactStore(
      "https://workflow.test",
      Date.now,
      createRetainedOutputPublisher(storage),
    );
  ...
  const invoke = async (name: string, args: object, caller = f.auth()) =>
    f.withExecutionSettings(f.settings, async () => {
  ...
  const get = async (id: string) =>
    mcpWorkflowOutputs.carrot_get_workflow.parse(
      await invoke("carrot_get_workflow", { id }),
    );
  const prepare = async (
    stages: McpWorkflowStage[] = [
      {
        kind: "translate",
    ...
    return mcpWorkflowOutputs.carrot_prepare_workflow.parse(
      await invoke("carrot_prepare_workflow", {
        requestId: randomUUID(),
        reason: "Isolated ordered workflow",
        chapters: [
          {
            chapterId: chapter.id,
            pages: [...chapter.pages].reverse().map((page) => ({
              pageId: page.id,
  ...
  const run = async (
    ...
    const receipt = mcpWorkflowOutputs.carrot_run_workflow.parse(
      await invoke("carrot_run_workflow", input),
    );
    ...
    await vi.waitFor(
      async () => {
        if ((await get(id)).status === "running")
          throw new Error("Workflow still running");
      },
      { timeout: 20000 },
    );
    return get(id);
