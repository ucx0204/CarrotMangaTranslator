...
it.each(["replace-background", "patch-background"] as const)(
  ...
  async (kind) => {
    ...
    try {
      ...
      const id = randomUUID();
      expect((await f.action(plan.batchId, "apply", id)).result.status).toBe(
        "completed",
      );
      ...
      expect(
        (await f.action(plan.batchId, "apply", id)).receipt.structuredContent,
      ...
      expect(
        (await f.pixels((await f.snapshot()).pages[0].inpaintedImagePath)).data,
      ).toEqual(pixels);
...
it("excludes fully protected and unchanged backgrounds without a save or native history", async () => {
  ...
  try {
    ...
    const unchanged = await f.upload(
      await f.pixels(page.inpaintedImagePath ?? page.imagePath),
    );
    expect(
      (
        await f.preview({
          kind: "replace-background",
          imageUploadId: unchanged.uploadId,
        })
      ).canApply,
    ).toBe(false);
    ...
    const plan = await f.preview({
      kind: "patch-background",
      imageUploadId: image.uploadId,
      protectedMaskUploadId: protection.uploadId,
...
it("refuses changed originals and subsequent user edits without overwriting page content", async () => {
  ...
  try {
    ...
    expect((await f.action(stale.batchId, "apply")).result.status).toBe(
      "failed",
    );
    expect(await readFile(f.chapterPath)).toEqual(changed);
  } finally {
