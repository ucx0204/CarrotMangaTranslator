...
import { DEFAULT_REDACTION_PREFERENCES, DEFAULT_REDACTION_VIEW, type SaveRedactionWorkspace } from "../src/shared/imageRedactionWorkspace";
import { createRedactionSession, editRedactionDocuments } from "../src/renderer/src/components/imageRedaction/redactionSession";
...
function fixture() {
  return createRedactionSession({
    sessionId: "11111111-1111-4111-8111-111111111111", revision: 4,
    pages: [{ id: "a", name: "a", imagePath: "a.png", width: 10, height: 10,
      fingerprint: "a".repeat(64), strokes: [], decision: "unreviewed" }],
    view: { ...DEFAULT_REDACTION_VIEW, currentId: "a" }, preferences: DEFAULT_REDACTION_PREFERENCES, presets: [],
  });
}
it("coalesces saves and flushes newer edits with the acknowledged revision", async () => {
  ...
  const writer = new RedactionDraftWriter(current, {
    ...
    persist: async (request) => {
      ...
      if (requests.length === 1) return new Promise<number>((resolve) => { release = resolve; });
      return 6;
    }, notify: vi.fn(),
  });
  current = editRedactionDocuments(current, [{ ...current.documents.a, decision: "reviewed" }]);
  const first = writer.flush();
  expect(writer.flush()).toBe(first);
  current = editRedactionDocuments(current, [{ ...current.documents.a, decision: "deferred" }]);
  release(5);
...
it("retains failed input and revision for an explicit retry rather than acknowledging it", async () => {
  ...
  const notify = vi.fn();
  const writer = new RedactionDraftWriter(current, { read: () => current, persist, notify });
  current = editRedactionDocuments(current, [{ ...current.documents.a, decision: "reviewed" }]);
  ...
  expect(persist.mock.calls.map(([request]) => request.expectedRevision)).toEqual([4, 4]);
