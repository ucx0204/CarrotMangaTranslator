...
it("refuses another connection's analysis, a foreign item, duplicate targets and mixed command kinds", async () => {
  ...
  try {
    ...
    await expect(
      f.invoke("carrot_preview_selection_batch", input, "other-owner"),
    ).rejects.toThrow();
    ...
    await expect(
      f.preview({ ...input, command: { kind: "references" } }),
    ).rejects.toThrow(/separate commands/i);
    await expect(
      f.preview({
        ...input,
        command: { kind: "analysis", analysisId: randomUUID() },
      }),
    ).rejects.toThrow();
...
it.each(["page", "image", "context"] as const)(
  "rejects changed unselected analysis dependency: %s",
  async (change) => {
    const f = await selectionEditingFixture();
    try {
      ...
      if (change === "page")
        await f.mutateStored((chapter) => {
          chapter.pages[1].blocks[1].sourceText = "later edit";
        });
      if (change === "image")
        await writeFile(
          f.chapter.pages[1].imagePath,
...
it("records a committed page before a failing UI notification and can undo that partial save", async () => {
  ...
  try {
    ...
    expect(result.pages.map((page) => page.result)).toEqual([
      "saved",
      "not_started",
    ]);
    expect((await f.snapshot()).pages[1].blocks).toEqual(
      before.pages[1].blocks,
    );
...
it("cancels a pending native edit without a save and rejects an older cancellation ID", async () => {
  ...
  try {
    ...
    await expect(
      f.invoke("carrot_cancel_selection_batch", {
        batchId: plan.batchId,
        requestId: randomUUID(),
      }),
    ).rejects.toThrow(/currently inspected/i);
    await f.invoke("carrot_cancel_selection_batch", {
      batchId: plan.batchId,
      requestId,
    });
    gate.release();
...
it("does not undo over a later user edit or leak another owner's plan", async () => {
  ...
  try {
    ...
    await f.mutateStored((chapter) => {
      chapter.pages[0].blocks[0].translatedText = "user edit";
    });
    const before = await readFile(f.chapterPath);
