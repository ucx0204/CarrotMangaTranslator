...
export async function typographyBatchAppFixture(
  ...
  const { createMcpTypographyBatchSession } =
    await import("../src/main/mcp/mcpTypographyBatchSession");
  ...
  const { mcpContextRevision } =
    await import("../src/shared/mcpContextEditing");
  ...
  const session = createMcpTypographyBatchSession(
    f.app,
    operations,
    editing,
    true,
  ...
  const analyze = async (principal = owner) => {
    ...
    const target = {
      ...
      catalogSnapshot: preflight.catalogSnapshot,
      pages: preflight.pages.map(({ pageId, revision }) => ({
        pageId,
        revision,
      })),
    ...
    const receipt = await operations.start({
      ...
      assertAuthorized: guard,
      execute: (context) => analysis.run(target, context),
    });
    await vi.waitFor(
      () => {
    ...
    return {
      ...
      requestId: randomUUID(),
      preserveManualFontSize: true,
      reason: "Match only selected source typography",
  ...
  const invoke = async (
    ...
    if (content.length !== 1 || content[0].type !== "text")
      throw new Error("Expected text-only result");
    return JSON.parse(content[0].text);
  };
  const inspect = (batchId: string) =>
    invoke("carrot_get_typography_batch", { batchId });
