...
it("keeps the native block-mask authority identical for pixel and normalized source boxes", async () => {
  ...
  try {
    const { buildMcpImageEditMasks } =
      await import("../src/main/mcp/mcpImageEditMasks");
    const { buildPatternPageMask } =
      await import("../src/main/inpainting/patternPageMask");
    ...
    const bitmap = imageNativeBoundary
      .createFromPath(page.imagePath)
      .toBitmap();
    const command = {
      kind: "erase-blocks" as const,
      blockIds: ["a", "b"],
      expectedEngine: "lama-manga" as const,
      allowAssetDownloads: true,
      protectedAreas: [],
    };
    const actual = buildMcpImageEditMasks(page, command, bitmap);
    expect(actual.mask).toEqual(
      buildMcpImageEditMasks(normalized, command, bitmap).mask,
    );
    expect(actual.mask).toEqual(
...
it("matches native retouch pixels exactly when no protection is requested", async () => {
  ...
  try {
    ...
    const native = await applyInpaintingRetouch(before, {
      mode: "paint",
      geometry,
      color: "#abcdef",
    });
    ...
    expect((await f.pixels(actual.inpaintedImagePath)).data).toEqual(
      (await f.pixels(native.inpaintedImagePath)).data,
    );
...
it("rejects malformed trusted prepared masks and incompatible inputs before engine invocation", async () => {
  ...
  try {
    ...
    const { applyInpaintingRetouch } = await import("../src/main/inpainting");
    const page = (await f.snapshot()).pages[0];
    const lease = await f.acquireEngine({
      appPaths: f.app.appPaths,
      model: "lama-manga",
    });
    const options = { strokes: [], inpaintingEngine: lease.engine };
    for (const preparedMask of [
      new Uint8Array(1),
      new Uint8Array(10000).fill(2),
    ])
      await expect(
        inpaintDrawnPatternPage(page, { ...options, preparedMask }),
      ).rejects.toThrow(/Prepared mask/);
    await expect(
      inpaintDrawnPatternPage(page, {
        ...options,
        preparedMask: new Uint8Array(10000),
        strokes: [{ radiusPx: 4, points: [{ x: 2, y: 2 }] }],
      }),
    ).rejects.toThrow(/not both/);
    await expect(
      inpaintDrawnPatternPage(
        { ...page, width: 101 },
        { ...options, preparedMask: new Uint8Array(10000) },
      ),
    ).rejects.toThrow(/Prepared mask/);
    await expect(
      applyInpaintingRetouch(page, {
        mode: "paint",
        color: "#abcdef",
        protectedMask: new Uint8Array(1),
        geometry: {
          kind: "rectangle",
          start: { x: 1, y: 1 },
          end: { x: 5, y: 5 },
    ...
    ).rejects.toThrow(/Protected retouch/);
    expect(f.inpaint).not.toHaveBeenCalled();
    await lease.release();
  } finally {
    await f.close();
