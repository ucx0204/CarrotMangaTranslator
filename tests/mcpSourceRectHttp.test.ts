...
function grant(provider: McpOAuthProvider, scope: string) {
  ...
  const pending = provider.begin({
    ...
    resource: `${origin}/mcp`, scope, state: "test",
    code_challenge: oauthDigest(secret), code_challenge_method: "S256",
  });
  const redirected = new URL(provider.approve({ transaction: pending.transaction, decision: "approve", pairing_secret: secret }, pending.cookie));
  return provider.token({
    grant_type: "authorization_code", client_id: client.client_id, redirect_uri,
    resource: `${origin}/mcp`, code: redirected.searchParams.get("code"), code_verifier: secret,
  }).access_token;
}
async function fixture() {
  ...
  const app = { jobs, appPaths: getAppPaths(), inpaintingRevisionStore: f.store, getMainWindow: () => null, decodeImage: async () => null };
  ...
  const provider = new McpOAuthProvider(origin, secret, Date.now, { allowProcessing: true });
  ...
  const server = await startMcpHttpServer({
    ...
    tools: createMcpPageEditTools(service, true, true), enforceScopes: true,
  ...
  const request = { ...f.target, revision: createPageRevision(f.after), sourceRect: { x: 250.5, y: 300.25, w: 130.5, h: 140.25 } };
  return { ...f, jobs, errors, notifySaved, request, read, rpc,
    call: async (args: unknown, bearer = full) => (await rpc("tools/call", { name, arguments: args }, bearer)).body,
    close: async () => { await server.close(); stop(); await f.close(); },
  };
}
...
it("saves fractional source bounds over scoped HTTP while retaining image bytes and non-target data", async () => {
  ...
  try {
    ...
    expect(result).toMatchObject({ status: "saved", changed: true, sourceRect: f.request.sourceRect });
    expect(JSON.stringify(response)).not.toMatch(/image\/png|resource_link|dataUrl|sourceText/);
    ...
    expect((await f.call(f.request)).result.structuredContent.error).toBe("revision_conflict");
    const repeated = await f.call({ ...f.request, revision: result.revision });
    expect(repeated.result.structuredContent).toMatchObject({ status: "already_applied", changed: false });
    expect(await f.snapshot()).toEqual(snapshot);
    expect((await f.call({ ...f.request, revision: result.revision, sourceRect: result.previousSourceRect })).result.isError).toBe(false);
    expect((await f.library.openChapter("chapter")).pages[0].blocks).toEqual(before.blocks);
...
it("hides writes from read-only grants and rejects malicious or off-page inputs without mutation", async () => {
  ...
  try {
    ...
    expect(listed.body.result.tools.some((tool: { name: string }) => tool.name === name)).toBe(false);
    expect((await f.call(f.request, f.read)).error.code).toBe(-32601);
    const full = await f.rpc("tools/list");
    const tool = full.body.result.tools.find((item: { name: string }) => item.name === name);
    ...
    const offPage = await f.call({ ...f.request, sourceRect: { x: 999, y: 0, w: 2, h: 10 } });
    expect(offPage.result.structuredContent.error).toBe("invalid_edit");
    expect((await f.call({ ...f.request, blockId: "missing" })).result.structuredContent.error).toBe("not_found");
