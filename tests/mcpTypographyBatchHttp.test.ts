...
async function httpFixture() {
  ...
  const origin = "https://typography-batch.test", secret = "s".repeat(43);
  const provider = new McpOAuthProvider(origin, secret, Date.now, { allowEdits: true, allowProcessing: true });
  ...
  const server = await startMcpHttpServer({
    ...
    tools: f.session.tools, enforceScopes: true,
  ...
  const done = async (batchId: string) => {
    ...
    return (await call("carrot_get_typography_batch", { batchId })).result.structuredContent;
  };
  return { ...f, request, provider, principal, call, done, read, other,
    close: async () => { await f.session.close(); await server.close(); await f.close(); },
  };
}
...
it("returns structured typography plans and completes exact apply/undo/redo through real HTTP", async () => {
  ...
  try {
    ...
    for (const direction of ["apply", "undo", "redo", "undo"]) {
      const receipt = await f.call(`carrot_${direction}_typography_batch`, { batchId, requestId: randomUUID() });
...
it("denies read-only edits, cross-owner observations and invalid payloads without saving", async () => {
  ...
  try {
    ...
    const denied = await f.call("carrot_preview_typography_batch", f.request, f.read);
    expect(denied.error).toMatchObject({ code: -32602, message: "Unknown tool" });
    const wrongOwner = await f.call("carrot_preview_typography_batch", f.request, f.other);
    expect(wrongOwner.result.isError).toBe(true);
    expect(wrongOwner.result.structuredContent.error).toBe("not_found");
    const invalid = await f.call("carrot_preview_typography_batch", { ...f.request, blocks: [] });
