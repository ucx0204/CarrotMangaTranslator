...
async function preparedRecovery() {
  ...
  if (!before.inpaintedImagePath)
    throw new Error("Missing first working image");
  ...
  const id = (await f.list()).items.find(
  ...
  if (!page.inpaintedImagePath)
    throw new Error("Missing restored working image");
  expect(page.inpaintedImagePath).toContain(".mcp-recovered-");
  return {
    ...f,
    id,
    firstBytes,
    restored: page.inpaintedImagePath,
    paths: [page.inpaintedImagePath, page.inpaintMaskPath].filter(
...
it("does not accumulate recovered image bytes when a retained change is repeatedly toggled", async () => {
  ...
  try {
    ...
    expect(
      (await readdir(dirname(f.chapterPath))).filter((name) =>
        name.startsWith(".mcp-recovered-"),
      ),
    ).toHaveLength(1);
  } finally {
    await f.close();
...
it("keeps files while native artifact history leases use them and collects only the released candidates", async () => {
  const f = await preparedRecovery();
  const { retainLibraryArtifacts } =
    await import("../src/main/libraryStore/libraryArtifactRetention");
  const {
    removeUnreferencedInpaintedArtifacts,
    removeUnreferencedInpaintMaskArtifacts,
  } = await import("../src/main/libraryStore/inpaintedArtifacts");
...
it("does not retire an entire candidate directory containing an unrelated file", async () => {
  ...
  try {
    ...
    expect(await readFile(extra, "utf8")).toBe(
      "not an image owned by this recovery",
    );
  ...
  it(`recovers page, action receipt and old working images consistently through ${point}`, async () => {
    ...
    try {
      ...
      if (point === "after-commit-point") {
        await expect(access(f.restored)).rejects.toMatchObject({
          code: "ENOENT",
        });
