...
describe("Windows installer clean uninstall option", () => {
  ...
  it("keeps the user-selected data root instead of recalculating it during install", () => {
    ...
    expect(script).toContain("기존 데이터가 발견되어 해당 위치를 기본값으로 표시합니다");
    expect(script).toContain('StrCpy $MgtDataRoot "$LOCALAPPDATA\\manga-gemma-translator"');
  });

  it("does not let the default uninstaller recursively remove the data folder", () => {
    const script = readFileSync(join(repoRoot, "build", "installer.nsh"), "utf8");

    expect(script).toContain("electron-builder's default uninstaller removes $INSTDIR recursively");
  ...
  it("guards selected data cleanup with marker and root-directory checks", () => {
    ...
    expect(script).toContain('FileOpen $0 "$MgtDataRoot\\.manga-gemma-translator-data.tmp" w');
  });
  ...
  it("stores packaged app data under the configured data root", () => {
    ...
    expect(appPaths).toContain('const ocrRuntimeDir = explicitOcrRuntimeDir || join(dataRoot, "ocr-runtime")');
    ...
    expect(appPaths).not.toContain('join(process.env.LOCALAPPDATA || dataRoot, "manga-gemma-translator", "ocr-runtime")');
  });
