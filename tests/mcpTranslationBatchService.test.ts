...
it("previews without saving then applies/undoes/redoes only explicit translations", async () => {
  ...
  expect(
    mcpTranslationBatchOutputs.carrot_get_translation_batch.safeParse(applied)
      .success,
  ).toBe(true);
  ...
  expect(f.start(preview.batchId, "undo", undo.requestId)).toMatchObject({
    status: "already_started",
    historical: true,
  });
  expect(
    (await f.inspect(preview.batchId)).pages.every(
  ...
  await f.done(preview.batchId);
  expect(f.saved).toEqual(before);
  expect(f.start(preview.batchId, "apply", action.requestId).status).toBe(
    "already_started",
  );
  expect(f.saved).toEqual(before);
...
it("protects changed context before apply/redo but permits page-safe undo", async () => {
  ...
  f.start(next.batchId, "apply");
  await f.done(next.batchId);
  f.saved.styleGuide.rules.honorifics = "adapt";
  f.start(next.batchId, "undo");
  await f.done(next.batchId);
...
it("excludes generated lettering and no-ops without treating them as saved changes", async () => {
  ...
  expect(plan.pages.map((page) => page.state)).toEqual([
    "excluded",
    "unchanged",
    "pending",
  ]);
  ...
  const invalid = [
    { ...base, pages: [base.pages[0], base.pages[0]] },
    {
      ...
      pages: [
        {
          ...base.pages[0],
          edits: [base.pages[0].edits[0], base.pages[0].edits[0]],
        },
      ],
    },
    { ...base, pages: [{ ...base.pages[0], pageId: "missing" }] },
    {
      ...
      pages: [
        {
          ...base.pages[0],
          edits: [{ ...base.pages[0].edits[0], blockId: "missing" }],
        },
      ],
    },
    {
      ...
      pages: [
        {
          ...base.pages[0],
          edits: [{ ...base.pages[0].edits[0], translatedText: " " }],
        },
      ],
    },
    ...
    {
      ...base,
      pages: [{ ...base.pages[0], revision: "page-v1:" + "f".repeat(16) }],
    },
  ];
  for (const value of invalid)
    await expect(f.service.preview(f.owner, value, f.guard)).rejects.toThrow();
  const clearing = { ...invalid[4], allowEmpty: true };
  expect((await f.service.preview(f.owner, clearing, f.guard)).canApply).toBe(
    true,
  );
  expect(f.save).not.toHaveBeenCalled();
  await f.service.close();
...
it("rejects reused IDs across directions/plans and never leaks other owners' history", async () => {
  ...
  await f.done(plan.batchId);
  expect(() => f.start(plan.batchId, "undo", action.requestId)).toThrow(
    /requestId/,
  );
  await f.service.close();
});
