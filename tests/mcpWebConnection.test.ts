...
const { runMcpWebProbe } = require("../scripts/mcp-web-probe.cjs") as {
  runMcpWebProbe: (endpoint: string, password: string, expectedOrigin?: string) => Promise<void>;
};
const { findQuickTunnelOrigin, startQuickTunnel } = require("../scripts/mcp-quick-tunnel.cjs") as {
...
it("extracts only a complete Quick Tunnel HTTPS origin from bounded log text", () => {
  assert.equal(findQuickTunnelOrigin("| https://gentle-carrot-test.trycloudflare.com |"), "https://gentle-carrot-test.trycloudflare.com");
  for (const text of ["http://carrot.trycloudflare.com", "https://carrot.trycloudflare.com.evil.example", "https://carrot.example", "https://carrot.trycloudflare.com?secret=x"])
    assert.equal(findQuickTunnelOrigin(text), null);
});

it("rejects invalid tunnel ports before starting a process", async () => {
  for (const port of [0, -1, 65536, 1.5]) await assert.rejects(startQuickTunnel(port));
});
...
it("runs the real web diagnostic through consent, rotation, reads and revocation", async () => {
  ...
  const server = await startMcpHttpServer({
    ...
    tools: createMcpToolSet({ listLibrary: async () => ({ workOrder: [], works: [] }), openChapter: async () => { throw new Error("No chapter requested"); } }, undefined, true),
