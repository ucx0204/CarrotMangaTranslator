...
it("publishes the workflow record and index owner together without moving page history", async () => {
  ...
  try {
    ...
    const next = {
      ...record,
      owner: "receiving-connection",
      version: record.version + 1,
    };
    await publish(f, plan.id, next, { expected: f.owner, next: next.owner });
    await f.restart();
    await expect(
      f.invoke("carrot_get_workflow", { id: plan.id }),
    ).rejects.toThrow();
    const received = await f.invoke(
      "carrot_get_workflow",
      { id: plan.id },
      f.auth(next.owner),
    );
    ...
    expect(index.entries.find((entry) => entry.id === history)?.owner).toBe(
      f.owner,
    );
