...
it("validates opt-in configuration without accepting short secrets or paths", () => {
  ...
  assert.throws(() =>
    readMcpConfiguration({ CARROT_MCP_ENABLED: "1", CARROT_MCP_TOKEN: "weak" }),
  );
  ...
  assert.equal(
    readMcpConfiguration({
      ...env,
      CARROT_MCP_PUBLIC_ORIGIN: "https://carrot.example/",
    })?.publicOrigin,
    "https://carrot.example",
  );
  for (const port of ["0", "-1", "65536", "1.5", "NaN", " 10", ""]) {
    assert.throws(() =>
      readMcpConfiguration({ ...env, CARROT_MCP_PORT: port }),
    );
  }
  for (const origin of [
    "http://carrot.example",
    "https://carrot.example/mcp",
    "https://u:p@carrot.example",
    "https://carrot.example/?token=a",
    "https://carrot.example/#x",
    "https://*.example",
  ]) {
    assert.throws(() =>
      readMcpConfiguration({ ...env, CARROT_MCP_PUBLIC_ORIGIN: origin }),
    );
  }
});
...
it("pages library results and supports case-insensitive filtering", async () => {
  const reads = service();
  assert.equal(
    (await reads.listWorks({ offset: 0, limit: 1 }, "CAR")).works.length,
    1,
  );
  assert.equal(
    (await reads.listWorks({ offset: 0, limit: 1 }, "missing")).total,
    0,
  );
  assert.equal(
    (await reads.listWorks({ offset: 1, limit: 1 })).works.length,
    0,
  );
  assert.equal(
    (await reads.listChapters("work-1", { offset: 0, limit: 25 })).chapters[0]
      .id,
    "chapter-1",
  );
...
it("projects page metadata without paths, images, masks, source text or raw errors", async () => {
  const first = await service().getChapter("chapter-1", {
    offset: 0,
    limit: 2,
  });
  assert.equal(first.nextOffset, 2);
  assert.deepEqual(
    first.pages.map((item) => item.id),
    ["p1", "p2"],
  );
  ...
  assert.equal(
    (await service().getChapter("chapter-1", { offset: 2, limit: 2 }))
      .nextOffset,
    null,
  );
});
...
it("negotiates protocol versions and only advertises implemented capabilities", async () => {
  for (const version of [
    ...
    const { reply, json } = await message(
      request("initialize", {
...
it("accepts notifications without returning a response body or invoking tools", async () => {
  assert.deepEqual(
    (await message({ jsonrpc: "2.0", method: "notifications/initialized" }))
      .reply,
    { status: 202 },
  );
  assert.equal(
    (await message({ jsonrpc: "2.0", method: "tools/call" })).reply.status,
    400,
  );
});
...
it("returns protocol errors for unknown methods, tools and malformed initialization", async () => {
  ...
  assert.ok(
    (await message(request("tools/call", { name: "__proto__" }))).json.includes(
      "-32602",
    ),
  );
  ...
  assert.ok(
    (
      await message(request("tools/list", { cursor: "unexpected" }))
    ).json.includes("-32602"),
  );
});
...
it("separates tool failures from protocol errors without leaking error details", async () => {
  const errors: unknown[] = [];
  const tool = {
    ...createMcpReadTools(service())[0],
    invoke: async () => {
      throw new Error("/private/key=SECRET");
    },
  };
  const reply = await handleMcpMessage(
    request("tools/call", { name: tool.name }),
    [tool],
    (error) => errors.push(error),
  );
...
it("serves actual Streamable HTTP initialize, tools/list, tools/call and notifications", async () => {
  await withServer(async ({ url }) => {
    ...
    const result = await post(
      url,
      request("tools/call", { name: "carrot_list_works" }),
    );
