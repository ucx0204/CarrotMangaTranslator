...
async function referencePlan(f: Awaited<ReturnType<typeof workflowFixture>>) {
  ...
  return mcpSelectionBatchOutputs.carrot_preview_selection_batch.parse(
    await f.invoke("carrot_preview_selection_batch", {
      ...
      reason: "Native child cancellation",
          ...
          edits: [
            {
              ...
              glossaryEntryIds:
                page.blocks[0].glossaryEntryIds === undefined ? [] : null,
            },
          ],
        },
      ],
...
it("observes pre-aborted child waits but holds native ownership until the actual save boundary settles", async () => {
  ...
  try {
    ...
    await expect(
      wait(f.owner, plan.batchId, requestId, AbortSignal.abort()),
    ).rejects.toThrow();
    ...
    await expect(
      wait("foreign", plan.batchId, requestId, AbortSignal.abort()),
    ).rejects.toThrow();
    await expect(
      wait(f.owner, plan.batchId, randomUUID(), AbortSignal.abort()),
    ).rejects.toThrow();
    ...
    expect(
      (await wait(f.owner, plan.batchId, requestId, AbortSignal.abort()))
        .status,
    ).toBe("cancelled");
    expect(() => retire(f.owner, plan.batchId, requestId)).toThrow();
