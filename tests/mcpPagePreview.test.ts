...
function chapter(): ChapterSnapshot {
  return {
    id: "chapter", workId: "work", title: "Chapter", sourceKind: "images",
    status: "idle", pageOrder: ["page"], createdAt: "now", updatedAt: "now",
    pages: [{
      id: "page", name: "page.png", width: 2000, height: 4000,
      imagePath: "/private/original.png", dataUrl: "PRIVATE DATA URL",
      blocks: [], analysisStatus: "idle", createdAt: "now", updatedAt: "revision",
    }],
  };
}

function previewService(renderApprovedPreview: ConstructorParameters<typeof McpPagePreviewService>[0]["renderApprovedPreview"]) {
  return new McpPagePreviewService({
    openChapter: async (id) => { assert.equal(id, "chapter"); return chapter(); },
...
it("returns only approved image bytes and source/preview dimensions", async () => {
  ...
  assert.deepEqual(content[1], { type: "image", data: "APPROVED", mimeType: "image/png" });
...
it("propagates the approval failure without returning unapproved fallback bytes", async () => {
  ...
  const service = previewService(async () => { throw failure; });
  await assert.rejects(service.getPreview("chapter", "page"), (error) => error === failure);
  const errors: unknown[] = [];
  const reply = await handleMcpMessage({
    jsonrpc: "2.0", id: 1, method: "tools/call",
    params: { name: "carrot_get_page_preview", arguments: { chapterId: "chapter", pageId: "page" } },
  }, [createMcpPagePreviewTool(service)], (error) => errors.push(error));
...
it("rejects caller-supplied paths and identifiers before reaching the preview port", async () => {
  let renders = 0;
  const tool = createMcpPagePreviewTool(previewService(async () => {
    renders++;
    return { data: "PNG", width: 1, height: 1 };
  }));
  for (const args of [
