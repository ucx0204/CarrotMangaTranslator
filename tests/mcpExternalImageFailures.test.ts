...
it.each(["empty", "wrong-size"] as const)(
  ...
  async (kind) => {
    ...
    try {
      ...
      const bad = kind === "empty" ? Buffer.from("invalid native image") : PNG.sync.write(externalPng(1, 1));
      spy.mockImplementation((bytes, options) =>
        decode(bytes.equals(image.bytes) ? bad : bytes, options),
      );
      await expect(f.preview({ kind: "patch-background", imageUploadId: image.uploadId,
        rect: { x: 10, y: 10, w: 8, h: 6 } })).rejects.toThrow(/Native .*dimensions/);
...
it("surfaces staging cleanup failure while still releasing native history and preserving saved images", async () => {
  ...
  try {
    ...
    const failure = Object.assign(new Error("fixture staging directory is locked"), { code: "EACCES" });
    ...
    await expect(f.external.close()).rejects.toMatchObject({
      message: "External image session cleanup failed.", errors: [failure],
    });
